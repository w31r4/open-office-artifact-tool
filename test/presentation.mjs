...
const comboChart = pieSlide.charts.add("combo", {
  ...
  axes: { category: { title: "Quarter" }, value: { title: "Amount" }, secondary: { value: { title: "Margin %" } } },
...
assert.equal((comboChartXml.match(/<c:catAx>/g) || []).length, 2);
assert.equal((comboChartXml.match(/<c:valAx>/g) || []).length, 2);
assert.equal((comboChartXml.match(/<c:axId val="1"\/>/g) || []).length, 2);
assert.equal((comboChartXml.match(/<c:axId val="2"\/>/g) || []).length, 2);
assert.equal((comboChartXml.match(/<c:axId val="3"\/>/g) || []).length, 2);
assert.equal((comboChartXml.match(/<c:axId val="4"\/>/g) || []).length, 2);
assert.match(comboChartXml, /<c:lineChart>[\s\S]*?<c:axId val="3"\/><c:axId val="4"\/><\/c:lineChart>/);
assert.match(comboChartXml, /<c:catAx><c:axId val="3"\/>[\s\S]*?<c:axPos val="t"\/>[\s\S]*?<c:crossAx val="4"\/><c:crosses val="max"\/><\/c:catAx>/);
assert.match(comboChartXml, /<c:valAx><c:axId val="4"\/>[\s\S]*?<c:axPos val="r"\/>[\s\S]*?<a:t>Margin %<\/a:t>[\s\S]*?<c:crossAx val="3"\/><c:crosses val="max"\/><\/c:valAx>/);
...
assert.deepEqual(loadedComboChart.series.map((series) => series.axisGroup || "primary"), ["primary", "secondary", "primary"]);
assert.equal(loadedComboChart.axes.secondary.value.title, "Margin %");
...
assert.equal(alternateComboChart.series[1].axisGroup, "secondary");
assert.equal(alternateComboChart.axes.secondary.value.title, "Margin %");
...
assert.equal((alternateComboSecondXml.match(/<c:catAx>/g) || []).length, 2);
assert.equal((alternateComboSecondXml.match(/<c:valAx>/g) || []).length, 2);
assert.match(alternateComboSecondXml, /<c:lineChart>[\s\S]*?<c:axId val="3"\/><c:axId val="4"\/><\/c:lineChart>/);
...
assert.throws(() => richTextShape.text.set([[{ run: "linked", link: { uri: "https:\/\/example.com", slideId: "slide/2" } }]]), /exactly one of uri, slideId, action, or customShow/);
assert.throws(() => richTextShape.text.set([[{ run: "linked", link: { uri: "https:\/\/example.com", tooltip: "x".repeat(1025) } }]]), /tooltip exceeds 1024/);
assert.throws(() => richTextShape.text.set([[{ run: "linked", link: { targetPart: "ppt/slides/slide2.xml" } }]]), /exactly one of uri, slideId, action, or customShow/);
...
// Structured run hyperlinks own relationships in the slide, group, master, or layout part that emits a:hlinkClick.
const hyperlinkPresentation = Presentation.create({
  master: {
    id: "master/links",
    name: "Hyperlink Master",
    placeholders: [{
      type: "title",
      idx: 1,
      ...
      text: [{ runs: [
        ...
        " or ",
        { run: "open evidence path", link: { customShow: "Evidence Path" } },
      ] }],
    }],
  },
  layouts: [{
    id: "layout/links",
    name: "Hyperlink Layout",
    type: "obj",
    masterId: "master/links",
    placeholders: [{ type: "body", idx: 2, name: "Layout hyperlink", position: { left: 40, top: 96, width: 800, height: 56 }, text: "Layout target" }],
  }],
});
...
const evidenceCustomShow = hyperlinkPresentation.customShows.add({ name: "Evidence Path", slides: [hyperlinkTargetSlide, hyperlinkSlide] });
...
const hyperlinkShape = hyperlinkSlide.shapes.add({
  ...
  position: { left: 40, top: 180, width: 800, height: 80 },
  text: [{ runs: [
    ...
    ", or ",
    { run: "tour the evidence", link: { customShow: evidenceCustomShow, returnToSlide: true, tooltip: "Return after the tour" } },
  ] }],
});
...
assert.deepEqual(hyperlinkShape.text.paragraphs[0].runs[8].link, { customShow: "Evidence Path", returnToSlide: true, tooltip: "Return after the tour" });
...
assert.match(hyperlinkShape.toSvg(), /data-hyperlink="custom-show:Evidence Path"/);
assert.equal(hyperlinkPresentation.customShows.count, 1);
assert.equal(hyperlinkPresentation.customShows.getItem("Evidence Path"), evidenceCustomShow);
assert.equal(hyperlinkPresentation.resolve(evidenceCustomShow.id), evidenceCustomShow);
assert.match(hyperlinkPresentation.inspect({ kind: "customShow" }).ndjson, /"name":"Evidence Path"/);
assert.throws(() => hyperlinkPresentation.customShows.add({ name: "evidence path", slides: [hyperlinkSlide] }), /already exists/);
assert.throws(() => hyperlinkPresentation.customShows.add({ name: "Missing", slides: ["slide/missing"] }), /references missing slide/);
...
assert.throws(() => hyperlinkSlide.shapes.add({ text: [{ runs: [{ run: "Ambiguous", link: { action: "next", uri: "https:\/\/example.com" } }] }] }), /exactly one of uri, slideId, action, or customShow/);
...
const hyperlinkPptx = await PresentationFile.exportPptx(hyperlinkPresentation);
assert.equal((await PresentationFile.inspectPptx(hyperlinkPptx)).ok, true);
const hyperlinkZip = await JSZip.loadAsync(new Uint8Array(await hyperlinkPptx.arrayBuffer()));
const hyperlinkPresentationXml = await hyperlinkZip.file("ppt/presentation.xml").async("text");
const hyperlinkSlideXml = await hyperlinkZip.file("ppt/slides/slide1.xml").async("text");
const hyperlinkSlideRelsXml = await hyperlinkZip.file("ppt/slides/_rels/slide1.xml.rels").async("text");
const hyperlinkMasterXml = await hyperlinkZip.file("ppt/slideMasters/slideMaster1.xml").async("text");
const hyperlinkMasterRelsXml = await hyperlinkZip.file("ppt/slideMasters/_rels/slideMaster1.xml.rels").async("text");
const hyperlinkLayoutXml = await hyperlinkZip.file("ppt/slideLayouts/slideLayout1.xml").async("text");
...
assert.match(hyperlinkSlideXml, /<a:hlinkClick r:id="" action="ppaction:\/\/customshow\?id=0&amp;return=true" tooltip="Return after the tour"\/>/);
assert.match(hyperlinkPresentationXml, /<p:custShowLst><p:custShow name="Evidence Path" id="0"><p:sldLst><p:sld r:id="rId2"\/><p:sld r:id="rId1"\/><\/p:sldLst><\/p:custShow><\/p:custShowLst>/);
...
assert.match(hyperlinkMasterXml, /<a:hlinkClick r:id="" action="ppaction:\/\/customshow\?id=0"\/>/);
...
const hyperlinkLoadedShape = hyperlinkLoaded.slides.items[0].shapes.items.find((shape) => shape.name === "Structured hyperlinks");
...
assert.deepEqual(hyperlinkLoadedShape.text.paragraphs[0].runs[8].link, { customShow: "Evidence Path", returnToSlide: true, tooltip: "Return after the tour" });
...
assert.deepEqual(hyperlinkLoaded.master.placeholders[0].text[0].runs[4].link, { customShow: "Evidence Path" });
...
assert.equal(hyperlinkLoaded.customShows.count, 1);
assert.equal(hyperlinkLoaded.customShows.items[0].nativeId, 0);
assert.equal(hyperlinkLoaded.customShows.items[0].name, "Evidence Path");
assert.deepEqual(hyperlinkLoaded.customShows.items[0].slideIds, [hyperlinkLoaded.slides.items[1].id, hyperlinkLoaded.slides.items[0].id]);
...
assert.match(await hyperlinkSecondZip.file("ppt/slides/slide1.xml").async("text"), /ppaction:\/\/customshow\?id=0&amp;return=true/);
assert.match(await hyperlinkSecondZip.file("ppt/presentation.xml").async("text"), /<p:custShow name="Evidence Path" id="0">/);
...
missingHyperlinkRelationshipZip.file("ppt/slides/_rels/slide1.xml.rels", hyperlinkSlideRelsXml.replace(/<Relationship[^>]*relationships\/hyperlink[^>]*\/>/, ""));
const missingHyperlinkRelationshipPptx = new FileBlob(await missingHyperlinkRelationshipZip.generateAsync({ type: "uint8array", compression: "DEFLATE" }), { type: hyperlinkPptx.type });
await assert.rejects(() => PresentationFile.importPptx(missingHyperlinkRelationshipPptx), /hyperlink references missing relationship/);
const wrongHyperlinkModeZip = await JSZip.loadAsync(new Uint8Array(await hyperlinkPptx.arrayBuffer()));
wrongHyperlinkModeZip.file("ppt/slides/_rels/slide1.xml.rels", hyperlinkSlideRelsXml.replace(/ TargetMode="External"/, ""));
const wrongHyperlinkModePptx = new FileBlob(await wrongHyperlinkModeZip.generateAsync({ type: "uint8array", compression: "DEFLATE" }), { type: hyperlinkPptx.type });
await assert.rejects(() => PresentationFile.importPptx(wrongHyperlinkModePptx), /hyperlink relationship rId\d+ must be external/);
const wrongHyperlinkTypeZip = await JSZip.loadAsync(new Uint8Array(await hyperlinkPptx.arrayBuffer()));
wrongHyperlinkTypeZip.file("ppt/slides/_rels/slide1.xml.rels", hyperlinkSlideRelsXml.replace(/relationships\/hyperlink/, "relationships/image"));
const wrongHyperlinkTypePptx = new FileBlob(await wrongHyperlinkTypeZip.generateAsync({ type: "uint8array", compression: "DEFLATE" }), { type: hyperlinkPptx.type });
await assert.rejects(() => PresentationFile.importPptx(wrongHyperlinkTypePptx), /hyperlink relationship rId\d+ has unsupported type/);
const missingHyperlinkSlideZip = await JSZip.loadAsync(new Uint8Array(await hyperlinkPptx.arrayBuffer()));
missingHyperlinkSlideZip.file("ppt/slides/_rels/slide1.xml.rels", hyperlinkSlideRelsXml.replace('Target="slide2.xml"', 'Target="slide99.xml"'));
const missingHyperlinkSlidePptx = new FileBlob(await missingHyperlinkSlideZip.generateAsync({ type: "uint8array", compression: "DEFLATE" }), { type: hyperlinkPptx.type });
await assert.rejects(() => PresentationFile.importPptx(missingHyperlinkSlidePptx), /targets missing slide part ppt\/slides\/slide99\.xml/);
const alternatePrefixHyperlinkZip = await JSZip.loadAsync(new Uint8Array(await hyperlinkPptx.arrayBuffer()));
alternatePrefixHyperlinkZip.file("ppt/presentation.xml", hyperlinkPresentationXml.replaceAll("<p:", "<deck:").replaceAll("</p:", "</deck:").replace("xmlns:p=", "xmlns:deck=").replaceAll("r:", "rel:").replace("xmlns:r=", "xmlns:rel="));
for (const file of ["ppt/slideMasters/slideMaster1.xml", "ppt/slideLayouts/slideLayout1.xml", "ppt/slides/slide1.xml"]) {
  const xml = await alternatePrefixHyperlinkZip.file(file).async("text");
  alternatePrefixHyperlinkZip.file(file, xml.replaceAll("<a:", "<draw:").replaceAll("</a:", "</draw:").replace("xmlns:a=", "xmlns:draw=").replaceAll("r:", "rel:").replace("xmlns:r=", "xmlns:rel="));
}
for (const file of ["ppt/slideMasters/_rels/slideMaster1.xml.rels", "ppt/slideLayouts/_rels/slideLayout1.xml.rels", "ppt/slides/_rels/slide1.xml.rels"]) {
  const xml = await alternatePrefixHyperlinkZip.file(file).async("text");
  alternatePrefixHyperlinkZip.file(file, xml.replaceAll("http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink", "http://purl.oclc.org/ooxml/officeDocument/relationships/hyperlink").replaceAll("http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide", "http://purl.oclc.org/ooxml/officeDocument/relationships/slide"));
}
...
assert.equal(alternatePrefixHyperlinkLoaded.customShows.items[0].name, "Evidence Path");
assert.equal(alternatePrefixHyperlinkLoaded.slides.items[0].shapes.items.find((shape) => shape.name === "Structured hyperlinks").text.paragraphs[0].runs[8].link.customShow, "Evidence Path");

const missingCustomShowRelationshipZip = await JSZip.loadAsync(new Uint8Array(await hyperlinkPptx.arrayBuffer()));
missingCustomShowRelationshipZip.file("ppt/presentation.xml", hyperlinkPresentationXml.replace('<p:sld r:id="rId2"/>', '<p:sld r:id="rIdMissing"/>'));
await assert.rejects(
  async () => PresentationFile.importPptx(new FileBlob(await missingCustomShowRelationshipZip.generateAsync({ type: "uint8array", compression: "DEFLATE" }), { type: hyperlinkPptx.type })),
  /custom show Evidence Path references missing relationship rIdMissing/,
);
const missingCustomShowDefinitionZip = await JSZip.loadAsync(new Uint8Array(await hyperlinkPptx.arrayBuffer()));
missingCustomShowDefinitionZip.file("ppt/presentation.xml", hyperlinkPresentationXml.replace(/<p:custShowLst>[\s\S]*<\/p:custShowLst>/, ""));
await assert.rejects(
  async () => PresentationFile.importPptx(new FileBlob(await missingCustomShowDefinitionZip.generateAsync({ type: "uint8array", compression: "DEFLATE" }), { type: hyperlinkPptx.type })),
  /references missing custom show ID 0/,
);

const danglingCustomShowDeck = Presentation.create();
danglingCustomShowDeck.slides.add().shapes.add({ text: [{ runs: [{ run: "Missing show", link: { customShow: "Not defined" } }] }] });
await assert.rejects(() => PresentationFile.exportPptx(danglingCustomShowDeck), /references missing custom show Not defined/);
