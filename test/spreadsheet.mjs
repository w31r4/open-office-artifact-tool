...
assert.equal(multiWindowBook.resolve("workbook-window/2"), reviewWindow);
...
assert.equal(multiWindowBook.layoutJson().windows[1].activeWorksheet, "Review");
...
chartFromRange.lineOptions = { grouping: "stacked", smooth: true, varyColors: true };
chartFromRange.dataLabels = { showValue: true, showCategoryName: true, showSeriesName: true, position: "insideEnd" };
chartFromRange.series.items[0].line = { fill: "#2563EB", style: "dash-dot", width: 2 };
chartFromRange.series.items[0].marker = { symbol: "diamond", size: 8, fill: "#FDE68A", line: { fill: "#BE123C", style: "dotted", width: 1.5 } };
...
assert.deepEqual(chartFromRange.lineOptions, { grouping: "stacked", smooth: true, varyColors: true });
assert.deepEqual(chartFromRange.dataLabels, { showValue: true, showCategoryName: true, showSeriesName: true, position: "insideEnd" });
assert.match(chartFromRange.toSvg(), /data-chart-label-position="insideEnd"[\s\S]*data-chart-label-series="0"[\s\S]*>Revenue: Jan: 100<\/text>/);
assert.deepEqual(chartFromRange.series.items[0].line, { fill: "#2563EB", style: "dash-dot", width: 2 });
assert.deepEqual(chartFromRange.series.items[0].marker, { symbol: "diamond", size: 8, fill: "#FDE68A", line: { fill: "#BE123C", style: "dotted", width: 1.5 } });
...
assert.match(spillWorksheetXml, /<c r="A1"[^>]* cm="1"[^>]*><f t="array" ref="A1:C2">SEQUENCE\(2,3,10,2\)<\/f>/);
...
assert.match(await spillZip.file("xl/metadata.xml").async("text"), /<xda:dynamicArrayProperties fDynamic="1" fCollapsed="0"\/>/);
assert.match(await spillZip.file("xl/_rels/workbook.xml.rels").async("text"), /relationships\/sheetMetadata" Target="metadata.xml"/);
assert.match(await spillZip.file("[Content_Types].xml").async("text"), /spreadsheetml\.sheetMetadata\+xml/);
const spillRoundTrip = await SpreadsheetFile.importXlsx(spillXlsx);
assert.equal(spillRoundTrip.worksheets.getItem("Spill").store.get("A1").formulaType, "dynamicArray");
assert.equal(spillRoundTrip.worksheets.getItem("Spill").store.get("A1").dynamicArrayRef, "A1:C2");
assert.deepEqual(spillRoundTrip.worksheets.getItem("Spill").getRange("A1:C2").values, [[10, 12, 14], [16, 18, 20]]);
const prefixedSpillZip = await JSZip.loadAsync(new Uint8Array(await spillXlsx.arrayBuffer()));
prefixedSpillZip.file("xl/worksheets/sheet1.xml", spillWorksheetXml.replace("SEQUENCE(2,3,10,2)", "_xlfn.SEQUENCE(2,3,10,2)"));
const prefixedSpill = await SpreadsheetFile.importXlsx(await prefixedSpillZip.generateAsync({ type: "uint8array" }));
assert.deepEqual(prefixedSpill.worksheets.getItem("Spill").getRange("A1:C2").values, [[10, 12, 14], [16, 18, 20]]);
...
assert.match(previewSvg, /fill="#FDE68A" stroke="#BE123C" stroke-width="1.5" stroke-dasharray="2 4"/);
...
assert.match(chartXml, /<c:marker><c:symbol val="diamond"\/><c:size val="8"\/><c:spPr><a:solidFill><a:srgbClr val="FDE68A"\/><\/a:solidFill><a:ln w="19050"><a:solidFill><a:srgbClr val="BE123C"\/><\/a:solidFill><a:prstDash val="dot"\/><\/a:ln><\/c:spPr><\/c:marker>/);
assert.match(chartXml, /<c:smooth val="1"\/>/);
assert.match(chartXml, /<c:grouping val="stacked"\/>/);
assert.match(chartXml, /<c:varyColors val="1"\/>/);
assert.match(chartXml, /<c:dLbls><c:dLblPos val="inEnd"\/><c:showVal val="1"\/><c:showCatName val="1"\/><c:showSerName val="1"\/><\/c:dLbls>/);
...
assert.deepEqual(nativeOnlySheet.charts.items[0].lineOptions, { grouping: "stacked", smooth: true, varyColors: true });
assert.deepEqual(nativeOnlySheet.charts.items[0].dataLabels, { showValue: true, showCategoryName: true, showSeriesName: true, position: "insideEnd" });
...
assert.deepEqual(nativeOnlyRevenueChart.series.items[0].marker, { symbol: "diamond", size: 8, fill: "#FDE68A", line: { width: 1.5, fill: "#BE123C", style: "dotted" } });
...
assert.deepEqual(relocatedRevenueChart.series.items[0].marker, { symbol: "diamond", size: 8, fill: "#FDE68A", line: { width: 1.5, fill: "#BE123C", style: "dotted" } });
