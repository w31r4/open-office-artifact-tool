...
chartFromRange.dataLabels = { showValue: true, showCategoryName: true, position: "insideEnd" };
...
assert.deepEqual(chartFromRange.dataLabels, { showValue: true, showCategoryName: true, position: "insideEnd" });
assert.match(chartFromRange.toSvg(), /data-chart-label-position="insideEnd"[\s\S]*data-chart-label-series="0"[\s\S]*>Jan: 100<\/text>/);
...
assert.match(chartXml, /<c:dLbls><c:dLblPos val="inEnd"\/><c:showVal val="1"\/><c:showCatName val="1"\/><\/c:dLbls>/);
...
assert.deepEqual(nativeOnlySheet.charts.items[0].dataLabels, { showValue: true, showCategoryName: true, position: "insideEnd" });
