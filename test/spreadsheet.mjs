...
chartFromRange.series.items[0].marker = { symbol: "diamond", size: 8 };
...
assert.deepEqual(chartFromRange.series.items[0].marker, { symbol: "diamond", size: 8 });
...
assert.match(chartXml, /<c:marker><c:symbol val="diamond"\/><c:size val="8"\/><\/c:marker>/);
...
assert.deepEqual(nativeOnlyRevenueChart.series.items[0].marker, { symbol: "diamond", size: 8 });
...
assert.deepEqual(relocatedRevenueChart.series.items[0].marker, { symbol: "diamond", size: 8 });
