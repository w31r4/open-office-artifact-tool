...
assert.match(slideDuplicateDescription, /canonical top-level four-part SmartArt frames.*exactly one canonical dgm:relIds root.*dm\/lo\/qs\/cs relationships.*diagram data, layout, quick-style, and colors parts.*four distinct typed diagram parts.*SmartArt remains source-bound\/read-only.*nested\/noncanonical\/connected SmartArt graphs.*fail closed/i);
