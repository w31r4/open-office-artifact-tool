...
assert.match(HELP_CATALOG.find((item) => item.name === "slide.duplicate")?.schema?.returns?.slide?.description || "", /original imported PPTX.*canonical simple shapes.*canonical inline fixed-grid tables.*recognized closed literal-data charts.*embedded rectangular images.*recursively canonical p:grpSp groups.*numbered ChartPart.*no child, external, hyperlink, or data relationship.*every present connector endpoint.*same copied SlidePart tree.*distinct SlidePart.*byte-copies each accepted chart.*distinct ChartPart.*independent.*formula\/external-data\/embedded-workbook\/connected\/orphan charts.*fail closed/i);
