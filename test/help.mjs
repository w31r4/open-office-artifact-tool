...
assert.match(HELP_CATALOG.find((item) => item.name === "slide.duplicate")?.schema?.returns?.slide?.description || "", /original imported PPTX.*canonical simple shapes.*embedded rectangular images.*exactly one internal layout relationship.*NotesSlide.*NotesMaster.*distinct SlidePart.*immutable ImageParts.*byte-for-byte.*unchanged.*second import.*fail closed/i);
...
assert.match(HELP_CATALOG.find((item) => item.name === "slide.duplicate")?.summary || "", /unchanged graph.*canonical shapes.*embedded rectangular images.*NotesSlide.*NotesMaster.*distinct SlidePart.*byte-for-byte.*untouched.*complex graphs fail closed/i);
