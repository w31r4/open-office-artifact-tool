...
assert.match(HELP_CATALOG.find((item) => item.name === "slide.duplicate")?.schema?.returns?.slide?.description || "", /original imported PPTX.*canonical simple shapes.*canonical inline fixed-grid tables.*embedded rectangular images.*bounded canonical straight\/elbow connectors.*recursively canonical p:grpSp groups.*exactly one internal layout relationship.*NotesSlide.*NotesMaster.*every present connector endpoint.*same copied SlidePart tree.*nested picture.*distinct SlidePart.*immutable ImageParts.*byte-for-byte.*unchanged.*second import.*unsupported connector.*fail closed/i);
...
assert.equal(presentationCatalog.length, 59);
...
assert.match(HELP_CATALOG.find((item) => item.name === "slide.duplicate")?.summary || "", /unchanged graph.*canonical shapes.*canonical inline fixed-grid tables.*embedded rectangular images.*bounded canonical straight\/elbow connectors.*recursively canonical groups.*NotesSlide.*NotesMaster.*every present connector endpoint.*same copied SlidePart tree.*nested picture.*distinct SlidePart.*byte-for-byte.*untouched.*unsupported connector.*complex graphs fail closed/i);
