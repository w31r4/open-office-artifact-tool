...
try {
  ...
  assert.equal(first.qa.pdf.pages.length, 2);
  assert.equal(first.qa.fileInspect.summary.pages, 2);
  assert.equal(first.qa.fileInspect.summary.objects, 9);
  assert.equal(first.qa.fileInspect.summary.hasEmbeddedModel, true);
  assert.equal(first.qa.fileInspect.summary.hasEof, true);
  assert.equal(first.qa.modelRender.pages.length, 2);
  ...
  if (nativeStatus.available) assert.equal(first.qa.nativeRender.pageCount, 2);
