...
assert.deepEqual([...toBinary(PresentationShapeSchema, create(PresentationShapeSchema, { transform: { rotationAngle60000: 1 } }))], [0x6a, 0x02, 0x08, 0x02], "PresentationShape.transform must use additive public wire field 13.");
...
const addedPlaceholderFrameFile = await exportPptxWithOpenChestnut(inheritedPlaceholderFrameEdit);
const addedPlaceholderFrameZip = await JSZip.loadAsync(addedPlaceholderFrameFile.bytes);
assert.match(await addedPlaceholderFrameZip.file(masterPartPath).async("text"), /<a:xfrm\b[^>]*rot="900000"[^>]*><a:off x="762000" y="571500"\s*\/><a:ext cx="6858000" cy="1143000"\s*\/><\/a:xfrm>/);
const addedPlaceholderFrameRoundTrip = await importPptxWithOpenChestnut(addedPlaceholderFrameFile);
assert.deepEqual(addedPlaceholderFrameRoundTrip.master.placeholders[0].position, { left: 80, top: 60, width: 720, height: 120 });
assert.deepEqual(addedPlaceholderFrameRoundTrip.master.placeholders[0].transform, { rotationDegrees: 15 });
const removedPlaceholderFrame = await importPptxWithOpenChestnut(masterStyleSource);
removedPlaceholderFrame.layouts.items[0].placeholders[0].position = undefined;
removedPlaceholderFrame.layouts.items[0].placeholders[0].transform = undefined;
const removedPlaceholderFrameFile = await exportPptxWithOpenChestnut(removedPlaceholderFrame);
const removedPlaceholderFrameZip = await JSZip.loadAsync(removedPlaceholderFrameFile.bytes);
const removedPlaceholderFrameXml = await removedPlaceholderFrameZip.file(layoutPartPath).async("text");
const removedPlaceholderShapeXml = /<p:sp\b[\s\S]*?<\/p:sp>/.exec(removedPlaceholderFrameXml)?.[0] || "";
assert.doesNotMatch(removedPlaceholderShapeXml, /<a:xfrm\b/);
assert.match(removedPlaceholderShapeXml, /<a:prstGeom\b[^>]*prst="rect"/);
assert.match(removedPlaceholderShapeXml, /<a:noFill\s*\/>/);
assert.equal((await importPptxWithOpenChestnut(removedPlaceholderFrameFile)).layouts.items[0].placeholders[0].position, undefined);
const transformWithoutFrame = await importPptxWithOpenChestnut(removedPlaceholderFrameFile);
transformWithoutFrame.layouts.items[0].placeholders[0].transform = { flipHorizontal: false };
await assert.rejects(
  exportPptxWithOpenChestnut(transformWithoutFrame),
  (error) => error instanceof OpenChestnutCodecError && error.code === "invalid_presentation_transform",
);
...
const sourceFreeTransformPresentation = Presentation.create();
sourceFreeTransformPresentation.slides.add().shapes.add({
  name: "Transform roundtrip",
  position: { left: 40, top: 50, width: 320, height: 120 },
  transform: { rotationDegrees: 5, flipHorizontal: false, flipVertical: true },
  text: "Rotated",
});
const sourceFreeTransformPptx = await exportPptxWithOpenChestnut(sourceFreeTransformPresentation);
const sourceFreeTransformZip = await JSZip.loadAsync(sourceFreeTransformPptx.bytes);
assert.match(await sourceFreeTransformZip.file("ppt/slides/slide1.xml").async("text"), /<a:xfrm[^>]*rot="300000"[^>]*flipH="0"[^>]*flipV="1"/);
const sourceFreeTransformImported = await importPptxWithOpenChestnut(sourceFreeTransformPptx);
const sourceFreeTransformShape = sourceFreeTransformImported.slides.getItem(0).shapes.items[0];
assert.deepEqual(sourceFreeTransformShape.transform, { rotationDegrees: 5, flipHorizontal: false, flipVertical: true });
sourceFreeTransformShape.transform = { rotationDegrees: -12, flipHorizontal: true, flipVertical: false };
const editedTransformPptx = await exportPptxWithOpenChestnut(sourceFreeTransformImported);
const editedTransformRoundTrip = await importPptxWithOpenChestnut(editedTransformPptx);
assert.deepEqual(editedTransformRoundTrip.slides.getItem(0).shapes.items[0].transform, { rotationDegrees: -12, flipHorizontal: true, flipVertical: false });
editedTransformRoundTrip.slides.getItem(0).shapes.items[0].transform = undefined;
const clearedTransformPptx = await exportPptxWithOpenChestnut(editedTransformRoundTrip);
