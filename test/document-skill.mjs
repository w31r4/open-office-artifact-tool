...
try {
  ...
  assert.equal(authoredMergedTable?.widthDxa, 9000);
  assert.equal(authoredMergedTable?.indentDxa, 240);
  assert.deepEqual(authoredMergedTable?.columnWidthsDxa, [2400, 3000, 3600]);
  assert.deepEqual(authoredMergedTable?.cellMarginsDxa, { top: 60, bottom: 80, start: 100, end: 140 });
  assert.equal(authoredMergedTable?.borderColor, "445566");
  assert.equal(authoredMergedTable?.borderSize, 8);
  assert.equal(authoredMergedTable?.headerFill, "E2E8F0");
  ...
  assert.match(authoredMergedXml, /<w:tblW w:w="9000" w:type="dxa"\s*\/>/);
  assert.match(authoredMergedXml, /<w:tblGrid><w:gridCol w:w="2400"\s*\/><w:gridCol w:w="3000"\s*\/><w:gridCol w:w="3600"\s*\/><\/w:tblGrid>/);
  assert.match(authoredMergedXml, /<w:top w:val="single" w:color="445566" w:sz="8" w:space="0"\s*\/>/);
  assert.match(authoredMergedXml, /<w:shd w:val="clear" w:color="auto" w:fill="E2E8F0"\s*\/>/);
