...
try {
  ...
  assert.equal(authoredMergedTable?.widthDxa, 9300);
  assert.equal(authoredMergedTable?.indentDxa, 360);
  assert.deepEqual(authoredMergedTable?.columnWidthsDxa, [2500, 3100, 3700]);
  assert.deepEqual(authoredMergedTable?.cellMarginsDxa, { top: 40, bottom: 60, start: 80, end: 100 });
  assert.equal(authoredMergedTable?.borderColor, "884400");
  assert.equal(authoredMergedTable?.borderSize, 10);
  assert.equal(authoredMergedTable?.headerFill, "FFF2CC");
  ...
  assert.match(authoredMergedXml, /<w:tblW w:w="9300" w:type="dxa"\s*\/>/);
  assert.match(authoredMergedXml, /<w:tblGrid><w:gridCol w:w="2500"\s*\/><w:gridCol w:w="3100"\s*\/><w:gridCol w:w="3700"\s*\/><\/w:tblGrid>/);
  assert.match(authoredMergedXml, /<w:top w:val="single" w:color="884400" w:sz="10" w:space="0"\s*\/>/);
  assert.match(authoredMergedXml, /<w:shd w:val="clear" w:color="auto" w:fill="FFF2CC"\s*\/>/);
