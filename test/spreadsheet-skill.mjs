...
try {
  ...
  assert.deepEqual(wasmChart.dataLabels, { showValue: false, showCategoryName: true, position: "top" });
  ...
  assert.match(await fs.readFile(wasmResult.qa.summary.files.inspect, "utf8"), /"drawingType":"chart"[\s\S]*"title":"OpenChestnut edited trend"[\s\S]*"titleTextStyle":\{"fontSize":14\}[\s\S]*"lineOptions":\{"grouping":"percentStacked","smooth":false\}[\s\S]*"dataLabels":\{"showValue":false,"showCategoryName":true,"position":"top"\}[\s\S]*"fill":"#2563EB"[\s\S]*"line":\{"fill":"#7C3AED","style":"dash-dot","width":2\.5\}[\s\S]*"marker":\{"symbol":"triangle","size":10,"fill":"#DCFCE7","line":\{"fill":"#166534","style":"dashed","width":2\}\}[\s\S]*"Edited metric"[\s\S]*"fontSize":11[\s\S]*"Edited value"[\s\S]*"fontSize":8\.5/);
