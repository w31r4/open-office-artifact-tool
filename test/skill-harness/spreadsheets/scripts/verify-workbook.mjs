...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (!args.input) throw new Error("Usage: verify-workbook.mjs --input workbook.xlsx|data.csv|data.tsv [--input-format xlsx|csv|tsv] [--output-dir dir] [--sheet name] [--range A1:D20] [--render-format svg|png|webp|jpeg|pdf] [--all-sheets true] [--native-render auto|required|off] [--baseline-dir dir] [--write-baseline true]");
  const result = await verifyWorkbookFile(args.input, {
    ...
    coerceTypes: args["coerce-types"] === "true",
