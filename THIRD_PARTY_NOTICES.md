...
| PyMuPDF 1.27.2.x | Optional specialist strict scrub, bounded image-backed OCR redaction, legacy high-level imported-PDF edits, and residue inspection not yet covered by the JavaScript path | GNU AGPL-3.0 or an Artifex commercial license |
...
- PyMuPDF `>=1.27.2,<1.28`: optional separately installed specialist provider
  for strict sanitize, image-backed OCR redaction, and decoded/OCR residue
  evidence. Hosted tests pin `1.27.2.3`; Tesseract 5.x language data remains a
  separate Apache-2.0 component. The adapter requires an explicit AGPL or
  commercial-license acknowledgement and neither package is bundled by npm.
