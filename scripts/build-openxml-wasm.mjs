...
const manifest = {
  ...
  sourceProject: "native/OpenXmlWasm/src/OpenOffice.OpenXmlWasm/OpenOffice.OpenXmlWasm.csproj",
  sourceDependencies: {
...
console.log(`OpenXML WASM runtime: ${files.length} files, ${manifest.totalBytes} bytes`);
...
function writeSbom() {
  ...
  const runtimeConfig = JSON.parse(fs.readFileSync(path.join(destination, "OpenOffice.OpenXmlWasm.runtimeconfig.json"), "utf8"));
