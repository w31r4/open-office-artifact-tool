...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "open-chestnut-reproducibility-"));
...
try {
  ...
  if (JSON.stringify(firstSnapshot) !== JSON.stringify(secondSnapshot)) {
    ...
    const changed = [...new Set([...firstByPath.keys(), ...secondByPath.keys()])]
      ...
      .slice(0, 20);
    throw new Error(`OpenChestnut builds differ on the same host: ${changed.join(", ") || "file inventory changed"}.`);
  }
  console.log(`OpenChestnut reproducibility ok: ${firstSnapshot.length} files.`);
} finally {
  fs.rmSync(temporary, { force: true, recursive: true });
}

function build(output) {
  const result = spawnSync(process.execPath, ["scripts/build-open-chestnut.mjs"], {
    cwd: repoRoot,
    env: { ...process.env, OPEN_CHESTNUT_OUTPUT: output },
    stdio: "inherit",
  });
