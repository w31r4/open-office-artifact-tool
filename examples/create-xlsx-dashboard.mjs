...
sheet.getRange("E2:E4").conditionalFormats.add("cellIs", { operator: "greaterThan", formula: 0, format: { fill: "#DCFCE7" } });
workbook.comments.setSelf({ displayName: "Analyst" });
workbook.comments.addThread({ cell: sheet.getRange("E4") }, "Variance depends on latest monthly target.");
workbook.recalculate();

const file = await SpreadsheetFile.exportXlsx(workbook);
await file.save(path.join(outputDir, "xlsx-dashboard.xlsx"));
console.log(workbook.inspect({ kind: "formula,table,chart,thread", maxChars: 6000 }).ndjson);
