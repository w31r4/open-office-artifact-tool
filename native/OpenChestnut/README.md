...
- PPTX: direct RGB/theme solid and native style-reference slide backgrounds, text boxes/round rectangles, basic fill/line/shadow, straight/polyline connectors and arrows, source-free bar/line/pie charts, bounded embedded rectangular images with signed DrawingML `a:srcRect` crop, fixed tables, rich text/lists/links, canonical plain-text speaker notes, and source-bound Master/Layout preservation. Recognized imported direct backgrounds and source rectangles support semantic-hash-bound add/edit/remove without flattening inheritance or surrounding package graphs. JavaScript computes `contain`/`cover` into the native source rectangle before crossing the wire. Advanced background graphs, external or masked/effect-bearing pictures, and rich or irregular notes remain opaque-preserved and fail closed on mutation.
