...
- PPTX imported slide clone: the bounded unchanged SlidePart leaf accepts canonical run-level clicks for an external absolute URI, an internal jump to a retained SlidePart, or one of five relative slide actions. Preflight requires every relationship-backed click to use a recognized inline run property and every hyperlink/slide relationship to be consumed; execution copies the exact `r:id` and target onto the fresh SlidePart without rewriting source XML. Postwrite re-proves that external URIs and internal target parts match exactly. Shape-level and hover clicks, unknown actions, orphan relationships, links in non-shape leaves, and jumps to a removed slide remain fail-closed.
