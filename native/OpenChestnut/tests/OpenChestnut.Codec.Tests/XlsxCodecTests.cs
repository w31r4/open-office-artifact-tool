...
public sealed class XlsxCodecTests
{
    ...
    public void ProtocolAuthorsImportsAndSourcePreservesWorksheetChartsBesidePictures()
    {
        ...
        AssertChartDataLabels(authored.File.ToByteArray(), true, false, "outEnd", true);
        using (var stream = new MemoryStream(authored.File.ToByteArray()))
        using (var document = SpreadsheetDocument.Open(stream, false))
        {
        ...
        Assert.True(chart.DataLabels.HasShowSeriesName);
        Assert.True(chart.DataLabels.ShowSeriesName);
        ...
        AssertChartDataLabels(preserved.File.ToByteArray(), false, true, "t", false);
        ...
        Assert.False(addedLineOptions.Artifact.Workbook.Worksheets[0].Charts[0].LineOptions.HasSmooth);
        ...
        AssertChartDataLabels(withAddedDataLabels.File.ToByteArray(), true, true, "ctr", false);
        ...
        Assert.True(residualDataLabelsChart.DataLabels.HasShowSeriesName);
        Assert.False(residualDataLabelsChart.DataLabels.ShowSeriesName);
        AssertChartDataLabels(preservedDataLabelResidual.File.ToByteArray(), true, true, "outEnd", false);
        ...
        AssertChartDataLabels(withoutDataLabelPosition.File.ToByteArray(), false, true, null, false);
        ...
        AssertChartDataLabels(withAddedDataLabelPosition.File.ToByteArray(), false, true, "bestFit", false);

        var removedSeriesNameLabel = Import(preserved.File.ToByteArray());
        var removedSeriesNameLabelChart = removedSeriesNameLabel.Artifact.Workbook.Worksheets[0].Charts[0];
        removedSeriesNameLabelChart.DataLabels.ClearShowSeriesName();
        var withoutSeriesNameLabel = Export(removedSeriesNameLabel.Artifact);
        Assert.True(withoutSeriesNameLabel.Ok, string.Join("\n", withoutSeriesNameLabel.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
        AssertChartDataLabels(withoutSeriesNameLabel.File.ToByteArray(), false, true, "t", null);
        var addedSeriesNameLabel = Import(withoutSeriesNameLabel.File.ToByteArray());
        var addedSeriesNameLabelChart = addedSeriesNameLabel.Artifact.Workbook.Worksheets[0].Charts[0];
        Assert.False(addedSeriesNameLabelChart.DataLabels.HasShowSeriesName);
        addedSeriesNameLabelChart.DataLabels.ShowSeriesName = true;
        var withAddedSeriesNameLabel = Export(addedSeriesNameLabel.Artifact);
        Assert.True(withAddedSeriesNameLabel.Ok, string.Join("\n", withAddedSeriesNameLabel.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
        AssertChartDataLabels(withAddedSeriesNameLabel.File.ToByteArray(), false, true, "t", true);
        ...
        Assert.Equal(SpreadsheetChartLineGrouping.Stacked, complexSmoothChart.LineOptions.Grouping);
        Assert.False(complexSmoothChart.LineOptions.HasSmooth);
        ...
        var complexGroupingSource = SetChartComplexGrouping(authored.File.ToByteArray());
        var complexGroupingXml = ReadChartXml(complexGroupingSource);
        var complexGrouping = Import(complexGroupingSource);
        Assert.True(complexGrouping.Ok, string.Join("\n", complexGrouping.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
        var complexGroupingChart = Assert.Single(complexGrouping.Artifact.Workbook.Worksheets[0].Charts);
        Assert.False(complexGroupingChart.LineOptions.HasGrouping);
        Assert.True(complexGroupingChart.LineOptions.Smooth);
        Assert.False(complexGroupingChart.Source.Editable);
        var complexGroupingRoundTrip = Export(complexGrouping.Artifact);
        Assert.True(complexGroupingRoundTrip.Ok, string.Join("\n", complexGroupingRoundTrip.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
    ...
    public void ProtocolRejectsInvalidWorksheetChartLineOptions()
    {
        ...
        var unknownGrouping = ChartExportRequest();
        unknownGrouping.Artifact.Workbook.Worksheets[0].Charts[0].LineOptions.Grouping = (SpreadsheetChartLineGrouping)99;
        rejected = CodecResponse.Parser.ParseFrom(CodecProtocol.Invoke(unknownGrouping.ToByteArray()));
        Assert.False(rejected.Ok);
        Assert.Equal("invalid_spreadsheet_chart", Assert.Single(rejected.Diagnostics).Code);
    }
    ...
    [Fact]
    public void ProtocolAuthorsImportsAndSourcePreservesMultiplePivotRowFields()
    {
        ...
        pivot.TargetReference = "A1:D4";
        pivot.ItemFilters.Add(new SpreadsheetPivotItemFilterArtifact
        {
            Field = "Product",
            Mode = SpreadsheetPivotItemFilterMode.Exclude,
            Items = { new SpreadsheetPivotItemArtifact { StringValue = "B" } },
        });
        summary.Cells.Clear();
        var output = new object?[][]
        {
            ["Region", "Product", "Revenue", "Average units"],
            ["East", "A", 10D, 2D],
            ["West", "A", 30D, 6D],
            ["Grand Total", "", 40D, 4D],
        };
        ...
        using (var document = SpreadsheetDocument.Open(stream, false))
        {
            ...
            var definition = pivotPart.PivotTableDefinition!;
            Assert.Equal("A1:D4", definition.Location!.Reference!.Value);
            Assert.Equal(2U, definition.Location.FirstDataColumn!.Value);
            var xml = XDocument.Parse(definition.OuterXml);
            var ns = xml.Root!.Name.Namespace;
            Assert.Equal(["0", "1"], xml.Root.Element(ns + "rowFields")!.Elements(ns + "field").Select(field => field.Attribute("x")!.Value));
            Assert.Equal(3, xml.Root.Element(ns + "rowItems")!.Elements(ns + "i").Count());
            Assert.All(xml.Root.Element(ns + "pivotFields")!.Elements(ns + "pivotField").Take(2), field =>
            {
                Assert.Equal("0", field.Attribute("compact")!.Value);
                Assert.Equal("0", field.Attribute("defaultSubtotal")!.Value);
            });
            Assert.Equal([null, "1"], xml.Root.Element(ns + "pivotFields")!.Elements(ns + "pivotField").ElementAt(1)
                .Element(ns + "items")!.Elements(ns + "item").Select(item => item.Attribute("h")?.Value));
        }

        var imported = Import(authored.File.ToByteArray());
        Assert.True(imported.Ok, string.Join("\n", imported.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
        var importedPivot = Assert.Single(imported.Artifact.Workbook.Worksheets.Single(sheet => sheet.Name == "Summary").PivotTables);
        Assert.Equal(["Region", "Product"], importedPivot.RowFields);
        Assert.Empty(importedPivot.ColumnFields);
        Assert.Equal(["Sales", "Units"], importedPivot.ValueFields.Select(value => value.Field));
        var filter = Assert.Single(importedPivot.ItemFilters);
        Assert.Equal("Product", filter.Field);
        Assert.Equal(SpreadsheetPivotItemFilterMode.Exclude, filter.Mode);
        Assert.Equal("B", Assert.Single(filter.Items).StringValue);
        var preserved = Export(imported.Artifact);
        Assert.True(preserved.Ok, string.Join("\n", preserved.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
        Assert.Equal(ReadEntry(authored.File.ToByteArray(), pivotPath), ReadEntry(preserved.File.ToByteArray(), pivotPath));

        var hostNormalized = RewriteEntry(authored.File.ToByteArray(), pivotPath, xml =>
        {
            var document = XDocument.Parse(xml, LoadOptions.PreserveWhitespace);
            document.Root!.Elements(document.Root.Name.Namespace + "rowItems").Remove();
            return document.ToString(SaveOptions.DisableFormatting);
        });
        var normalizedImport = Import(hostNormalized);
        Assert.True(normalizedImport.Ok, string.Join("\n", normalizedImport.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
        Assert.Equal(["Region", "Product"], Assert.Single(normalizedImport.Artifact.Workbook.Worksheets.Single(sheet => sheet.Name == "Summary").PivotTables).RowFields);

        var compact = RewriteEntry(authored.File.ToByteArray(), pivotPath, xml =>
        {
            var document = XDocument.Parse(xml, LoadOptions.PreserveWhitespace);
            var ns = document.Root!.Name.Namespace;
            document.Root.Element(ns + "pivotFields")!.Elements(ns + "pivotField").First().SetAttributeValue("compact", "1");
            return document.ToString(SaveOptions.DisableFormatting);
        });
        var opaque = Import(compact);
        Assert.True(opaque.Ok, string.Join("\n", opaque.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
    ...
    [Fact]
    public void ProtocolAuthorsImportsAndSourcePreservesExactPivotItemFilters()
    {
        var request = PivotExportRequest();
        var summary = request.Artifact.Workbook.Worksheets.Single(sheet => sheet.Name == "Summary");
        var pivot = Assert.Single(summary.PivotTables);
        pivot.TargetReference = "A1:C3";
        pivot.ItemFilters.Add(new SpreadsheetPivotItemFilterArtifact
        {
            Field = "Region",
            Mode = SpreadsheetPivotItemFilterMode.Include,
            Items = { new SpreadsheetPivotItemArtifact { StringValue = "East" } },
        });
        pivot.ItemFilters.Add(new SpreadsheetPivotItemFilterArtifact
        {
            Field = "Product",
        ...
        {
            var definition = Assert.Single(document.WorkbookPart!.WorksheetParts.SelectMany(part => part.PivotTableParts)).PivotTableDefinition!;
            Assert.Equal("A1:C3", definition.Location!.Reference!.Value);
            var xml = XDocument.Parse(definition.OuterXml);
            var ns = xml.Root!.Name.Namespace;
            var fields = xml.Root.Element(ns + "pivotFields")!.Elements(ns + "pivotField").ToArray();
            Assert.Equal("0", fields[0].Attribute("includeNewItemsInFilter")!.Value);
            Assert.Equal([null, "1"], fields[0].Element(ns + "items")!.Elements(ns + "item").Select(item => item.Attribute("h")?.Value));
            Assert.Equal("1", fields[1].Attribute("includeNewItemsInFilter")!.Value);
            Assert.Equal([null, "1"], fields[1].Element(ns + "items")!.Elements(ns + "item").Select(item => item.Attribute("h")?.Value));
            Assert.Equal(2, xml.Root.Element(ns + "rowItems")!.Elements(ns + "i").Count());
            Assert.Equal(2, xml.Root.Element(ns + "colItems")!.Elements(ns + "i").Count());
        }
        ...
        Assert.True(imported.Ok, string.Join("\n", imported.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
        var importedPivot = Assert.Single(imported.Artifact.Workbook.Worksheets.Single(sheet => sheet.Name == "Summary").PivotTables);
        Assert.Collection(importedPivot.ItemFilters,
            filter =>
            {
                Assert.Equal("Region", filter.Field);
                Assert.Equal(SpreadsheetPivotItemFilterMode.Include, filter.Mode);
                Assert.Equal("East", Assert.Single(filter.Items).StringValue);
            },
            filter =>
            {
                Assert.Equal("Product", filter.Field);
        ...
        Assert.NotNull(importedPivot.Source);
        var preserved = Export(imported.Artifact);
        Assert.True(preserved.Ok, string.Join("\n", preserved.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
        Assert.Equal(
            ReadEntry(authored.File.ToByteArray(), importedPivot.Source.PivotTablePartPath),
            ReadEntry(preserved.File.ToByteArray(), importedPivot.Source.PivotTablePartPath));

        var malformed = RewriteEntry(authored.File.ToByteArray(), importedPivot.Source.PivotTablePartPath, xml =>
        {
            var document = XDocument.Parse(xml, LoadOptions.PreserveWhitespace);
            var ns = document.Root!.Name.Namespace;
            document.Root.Element(ns + "pivotFields")!.Elements(ns + "pivotField")
                .SelectMany(field => field.Element(ns + "items")?.Elements(ns + "item") ?? [])
                .First(item => item.Attribute("h")?.Value == "1")
                .SetAttributeValue("x", "999");
            return document.ToString(SaveOptions.DisableFormatting);
        });
        var opaque = Import(malformed);
        Assert.True(opaque.Ok, string.Join("\n", opaque.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
        Assert.Empty(opaque.Artifact.Workbook.Worksheets.Single(sheet => sheet.Name == "Summary").PivotTables);
        var opaquePreserved = Export(opaque.Artifact);
        Assert.True(opaquePreserved.Ok, string.Join("\n", opaquePreserved.Diagnostics.Select(item => $"{item.Code}: {item.Message}")));
        Assert.Equal(
            ReadEntry(malformed, importedPivot.Source.PivotTablePartPath),
            ReadEntry(opaquePreserved.File.ToByteArray(), importedPivot.Source.PivotTablePartPath));

        importedPivot.ItemFilters[0].Items[0].StringValue = "West";
        var rejected = Export(imported.Artifact);
        Assert.False(rejected.Ok);
        Assert.Equal("unsupported_spreadsheet_pivot_edit", Assert.Single(rejected.Diagnostics).Code);
    }
    ...
    public void PivotTableAuthoringRejectsUnsupportedSemanticProfiles()
    {
        ...
        var unknownFilterRequest = PivotExportRequest();
        unknownFilterRequest.Artifact.Workbook.Worksheets[1].PivotTables[0].ItemFilters.Add(new SpreadsheetPivotItemFilterArtifact
        {
            Field = "Region",
            Mode = SpreadsheetPivotItemFilterMode.Include,
            Items = { new SpreadsheetPivotItemArtifact { StringValue = "North" } },
        });
        var unknownFilter = CodecResponse.Parser.ParseFrom(CodecProtocol.Invoke(unknownFilterRequest.ToByteArray()));
        Assert.False(unknownFilter.Ok);
        Assert.Equal("invalid_spreadsheet_pivot", Assert.Single(unknownFilter.Diagnostics).Code);
        Assert.Contains("outside its source cache", unknownFilter.Diagnostics[0].Message, StringComparison.OrdinalIgnoreCase);

        var emptyFilterRequest = PivotExportRequest();
        emptyFilterRequest.Artifact.Workbook.Worksheets[1].PivotTables[0].ItemFilters.Add(new SpreadsheetPivotItemFilterArtifact
        {
            Field = "Region",
            Mode = SpreadsheetPivotItemFilterMode.Exclude,
            Items =
            {
                new SpreadsheetPivotItemArtifact { StringValue = "East" },
                new SpreadsheetPivotItemArtifact { StringValue = "West" },
            },
        });
        var emptyFilter = CodecResponse.Parser.ParseFrom(CodecProtocol.Invoke(emptyFilterRequest.ToByteArray()));
        Assert.False(emptyFilter.Ok);
        Assert.Equal("unsupported_spreadsheet_pivot_filter", Assert.Single(emptyFilter.Diagnostics).Code);
        Assert.Contains("hides every field item", emptyFilter.Diagnostics[0].Message, StringComparison.OrdinalIgnoreCase);

        var filterBudgetRequest = PivotExportRequest();
        var overBudgetFilter = new SpreadsheetPivotItemFilterArtifact { Field = "Region", Mode = SpreadsheetPivotItemFilterMode.Include };
        overBudgetFilter.Items.Add(Enumerable.Range(0, 1_025).Select(index => new SpreadsheetPivotItemArtifact { StringValue = $"Item {index}" }));
        filterBudgetRequest.Artifact.Workbook.Worksheets[1].PivotTables[0].ItemFilters.Add(overBudgetFilter);
        var filterBudget = CodecResponse.Parser.ParseFrom(CodecProtocol.Invoke(filterBudgetRequest.ToByteArray()));
        Assert.False(filterBudget.Ok);
        Assert.Equal("unsupported_spreadsheet_pivot_filter", Assert.Single(filterBudget.Diagnostics).Code);

        var rowFieldBudgetRequest = PivotExportRequest();
        var rowFieldBudgetPivot = rowFieldBudgetRequest.Artifact.Workbook.Worksheets[1].PivotTables[0];
        rowFieldBudgetPivot.ColumnFields.Clear();
        rowFieldBudgetPivot.RowFields.Add("Product");
        for (var index = 3; index <= 9; index++) rowFieldBudgetPivot.RowFields.Add($"Axis {index}");
        var rowFieldBudget = CodecResponse.Parser.ParseFrom(CodecProtocol.Invoke(rowFieldBudgetRequest.ToByteArray()));
        Assert.False(rowFieldBudget.Ok);
        Assert.Equal("unsupported_spreadsheet_pivot_profile", Assert.Single(rowFieldBudget.Diagnostics).Code);
        Assert.Contains("1 through 8 row fields", rowFieldBudget.Diagnostics[0].Message, StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static byte[] SetChartDataLabelFalseResidual(byte[] bytes)
    {
        ...
        using (var document = SpreadsheetDocument.Open(stream, true))
        {
            ...
            labels.Element(c + "showSerName")!.SetAttributeValue("val", "0");
            using var output = chartPart.GetStream(FileMode.Create, FileAccess.Write);
            chart.Save(output, SaveOptions.DisableFormatting);
        }
        return stream.ToArray();
    ...
    private static byte[] SetChartComplexGrouping(byte[] bytes)
    {
        using var stream = new MemoryStream();
        stream.Write(bytes);
        stream.Position = 0;
        using (var document = SpreadsheetDocument.Open(stream, true))
        {
            ...
            var chart = XDocument.Parse(ReadPartText(chartPart));
            XNamespace c = "http://schemas.openxmlformats.org/drawingml/2006/chart";
            chart.Descendants(c + "lineChart").Single().Element(c + "grouping")!.Attribute("val")!.Remove();
            using var output = chartPart.GetStream(FileMode.Create, FileAccess.Write);
            chart.Save(output, SaveOptions.DisableFormatting);
        }
        return stream.ToArray();
    ...
    private static void AssertChartDataLabels(byte[] bytes, bool? showValue, bool? showCategoryName, string? position = null, bool? showSeriesName = null)
    {
        var chart = XDocument.Parse(ReadChartXml(bytes));
        XNamespace c = "http://schemas.openxmlformats.org/drawingml/2006/chart";
        ...
        Assert.Equal(showSeriesName is null ? null : showSeriesName.Value ? "1" : "0", (string?)labels.Element(c + "showSerName")?.Attribute("val"));
    }
