...
// Owns the direct symbol/size projection of one line-series marker. Marker
// shape properties, picture markers, extensions, and unknown children keep the
// containing source-bound chart read-only so their native graph stays exact.
internal static class XlsxChartSeriesMarkerCodec
{
    ...
    internal static bool TryRead(XElement nativeSeries, SpreadsheetChartSeriesArtifact series, SpreadsheetChartType chartType)
    {
        ...
        if (children.Any(child => child.Name != ChartNs + "symbol" && child.Name != ChartNs + "size") ||
            children.Count(child => child.Name == ChartNs + "symbol") > 1 ||
            children.Count(child => child.Name == ChartNs + "size") > 1) return false;
    ...
    internal static string Semantics(SpreadsheetChartMarkerArtifact? marker) => marker is null
        ? "no-marker"
        : string.Join(':', "marker", (int)marker.Symbol, marker.HasSize ? marker.Size.ToString(CultureInfo.InvariantCulture) : "no-size");
