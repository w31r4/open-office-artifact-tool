...
| Full Presentation API guide | partial | The built-in source-free template is compatible. Plain-text speaker notes now support source-free authoring, stable inspect/resolve, and bounded hash-bound imported edits; rich notes remain preservation-only. Broader reference instructions for advanced Master/Layout, comments, custom shows, groups, notes styling/visibility, and other package graphs still exceed the current canonical export boundary. |
