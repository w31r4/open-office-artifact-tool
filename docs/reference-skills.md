...
| Documents | partial | The ordinary native workflow runs a shipped `DocumentModel` create → OpenChestnut export → import/edit → export → second-import example, with semantic verification, fresh `document.fontFamilies` inventory, and render-backed QA. Bounded whole-block bookmarks/internal hyperlinks, plain-text footnotes/endnotes, whole-paragraph tracked insertions/deletions, and inline plain-text content controls with strict fill-by-tag use the public API and native OpenChestnut markup. Python/OOXML helpers remain explicit for existing-template wrapping, controls outside body runs, irregular note/bookmark/revision graphs, in-paragraph replacement, accept/reject batches, and audits. Full status remains partial because rich/block/cell/data-bound/dropdown/date/checkbox SDTs, complex fields, rich note bodies, and other source-bound features still exceed the public semantic model. |
