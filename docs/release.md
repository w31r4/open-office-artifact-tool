...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `118` tests. The workbook-view corpus now covers coordinated source-free `ActiveTab`/`tabSelected` authoring, grouped selection import/edit, stable worksheet IDs, per-worksheet part/view bindings, explicit-false presence, residual pane/zoom preservation, incomplete selection graphs, nested-binding tamper, legacy active-only edits, opaque multi-window safety, and hidden active/selected rejection; preceding worksheet/calculation/name/sort/connection/table/theme/style/formula/DOCX/PPTX coverage remains green. Local macOS produces 38 inventoried runtime files/13,415,612 bytes. `open-chestnut-basic` crosses active `Icon Rules` plus the selected `Summary`/`Icon Rules` group through two WASM passes and real render QA; fallback and no-local-dotnet clean-install probes use the same public APIs. Hosted run [`29385454113`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29385454113) passes this increment with OpenChestnut 118/118, OfficeBridge 5/5, Chromium/LibreOffice 24.2.7.2/Poppler, the clean install, and two byte-identical 38-file/13,415,614-byte Linux runtime builds.
