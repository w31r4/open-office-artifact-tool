...
`test/package-contents.mjs` also validates the actual dry-run tarball manifest. It requires the project-local runnable skills, native bridge source, public protobuf source/generated binding, source-built XLSX/DOCX/PPTX OpenChestnut codecs/tests and same-host reproducibility verifier, runtime integrity manifest/SBOM/.NET notices, spreadsheet Pivot/date/group/filter/formula/aggregation/coercion/threaded-comment modules, PresentationML grouped-shape, Office 2021 modern-comment, and opaque native-object graph codecs/validators, the split OpenChestnut presentation/error/asset adapters, C# DOCX direct-numbering, merge-aware table/geometry/formatting, numbered-paragraph, and numbering-edit planner codecs, and C# PPTX asset-catalog/slide-context/text/paragraph-layout/paragraph-spacing/color/bullet/bullet-style/hyperlink codecs, the DOCX bibliography, bookmark/internal-link, and section/header-footer planners, the PDF table-grid/accessibility normalizers, the structured-intersection/OpenChestnut fixtures, shared visual-baseline lifecycle, and third-party notices while rejecting reference/handoff material and local dotnet `bin/obj` build output. The current dry-run package contains 209 files and is 4,700,924 bytes compressed/16,099,747 bytes unpacked, below the reviewed 8,000,000/16,250,000-byte gates. The unpacked gate was explicitly raised by 250 KB for the preceding numbering planner source, tests, fixture, and required WASM growth; the direct-numbering and table-formatting builders remain within that reviewed headroom. Most of the footprint remains the bundled .NET/Open XML SDK runtime required for no-local-dotnet Office I/O. The gate catches accidentally bundled fonts, reference assets, fixtures outside the published skills, symbols/maps, and build artifacts while forcing explicit review when legitimate published capability grows the package.
...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `56` tests. The table-format corpus now proves schema-valid fixed authoring plus exact-profile imported edits, semantic reimport, preservation of unknown row/paragraph properties, invalid width/border rejection, explicit no-border output, and fail-closed removal/unrecognized profiles; the direct-numbering and numbering-planner corpus remains green. Local macOS produces 38 runtime files/12,981,948 bytes; two isolated 39-file snapshots are byte-identical; the 209-file no-local-dotnet clean install passes; and the updated OpenChestnut→OpenChestnut merged-table fixture passes real Chromium/LibreOffice/Poppler. Hosted Linux runs [`29343501551`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29343501551) and [`29343983983`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29343983983) pass the same gate with 38 runtime files/12,981,950 bytes, OpenChestnut 56/56, OfficeBridge 5/5, and real Chromium/LibreOffice/Poppler.
