...
- `dotnet test native/OpenXmlWasm/OpenXmlWasm.sln --configuration Release` passes locally (`20` tests), including PPTX character/auto/none marker and explicit/follow-text font/RGB-color/fixed-or-percentage-size authoring and edits; external URI, internal slide, and relative-action run-link authoring/edit/deletion; real relationship-backed picture-marker plus unknown scheme-color/custom-show-link preservation; invalid marker/style/link rejection; basic rich-text roundtrip/editing; unmodeled paragraph/run-property retention; fixed-topology rejection; XLSX/DOCX/PPTX hash/source/opaque-graph or element-binding preservation; unsupported-advanced-edit rejection; and fail-closed Office 2021 validation. Hosted Linux run [`29273423813`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29273423813) passed the complete Chromium/LibreOffice/Poppler, reproducible-runtime, clean-install package, OfficeBridge, and 17-test OpenXmlWasm gate for the preceding marker-style increment; hosted hyperlink evidence is pending this commit's CI.
