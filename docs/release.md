...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux CI with `53` tests. The numbering planner tests prove complete direct-group `numberFormat`/`start`/`levelText` edits through an instance-local override, simultaneous text editing, unchanged shared `abstractNum`, Office 2021 validation/reimport, and partial/style-inherited/cross-part rejection. Local macOS produces 38 runtime files/12,952,252 bytes; hosted run [`29338794373`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29338794373) is green for two byte-identical 38-file/12,952,254-byte Linux builds, the 207-file clean install, real Chromium/LibreOffice/Poppler, the numbering-edit Documents fixture, and OfficeBridge 5/5.
