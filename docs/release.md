...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux CI with `55` tests. The direct-numbering tests prove shared multilevel abstract authoring, multiple instances, Office 2021 validation/reimport, and conflicting/style-linked graph rejection; the numbering planner tests retain complete-group edit, unchanged shared `abstractNum`, and partial/style-inherited/cross-part rejection evidence. Local macOS produces 38 runtime files/12,960,956 bytes. Hosted run [`29340497565`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29340497565) is green for two byte-identical 38-file/12,960,958-byte Linux builds, the 208-file no-local-dotnet clean install, real Chromium/LibreOffice/Poppler including the OpenChestnut→OpenChestnut numbering fixture, and OfficeBridge 5/5.
