...
- The active-content public-sanitize path produced two fixed-candidate live Agent trials at 100/100. Independent pypdf evidence proved root/additional JavaScript, Launch/SubmitForm, five attachments, invisible text, a comment, a populated widget, and personal metadata in the immutable source and zero active structure/canary residue in those outputs. A third repeat had correct final bytes but scored zero after its Agent bypassed the typed primitive to remove null active-content names; that repeat is retained as defect evidence, not a pass. The shipped scrub primitive now closes that provider gap, and the candidate/reference repeat matrix must restart on its fixed tarball.
