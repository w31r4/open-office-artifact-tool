...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `128` tests. The picture corpus authors, imports, and source-preserves one-cell, two-cell, and absolute anchors; retains signed absolute position and optional `editAs` presence; authors/imports/edits/removes bounded signed `a:srcRect` crop plus `a:alphaModFix`/`a:grayscl`/`a:lum` effects while preserving picture locks and opaque effect graphs; replaces uniquely referenced same-format ImagePart bytes without changing the part URI; and rejects out-of-profile crop/effect replacement, anchor-kind conversion, cross-format/shared-part replacement, add/remove, binding tamper, invalid assets, and invalid geometry. The preceding workbook-view/worksheet/calculation/name/sort/connection/table/theme/style/formula/DOCX/PPTX coverage remains green. Local macOS produces 38 inventoried runtime files/13,494,972 bytes, and two isolated builds are byte-identical across all 39 audited files. `open-chestnut-basic` carries all three anchor kinds, crop/effects, source-bound PNG replacement, its `SEQUENCE` spill, and two workbook windows through two WASM exports plus real Chromium/LibreOffice/Poppler QA; JavaScript fallback and no-local-dotnet clean-install probes use the same public model. The current 151-file package is 4,789,568 bytes compressed and 16,097,087 bytes unpacked. Hosted Linux run [`29395054954`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29395054954) remains the preceding crop evidence with OpenChestnut 127/127, OfficeBridge 5/5, real Chromium/LibreOffice 24.2.7.2/Poppler 24.02.0, the 151-file clean install, generated docs/proto, and two byte-identical 38-file/13,485,246-byte runtime builds; the effect increment requires a new hosted run after push.
