...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `140` tests; the last exact-SHA hosted baseline remains `135` until the current milestone is pushed. In addition to the drawing corpus, local coverage now validates content types for every OPC entry, public metadata for every unowned part, per-part relationship adjacency, malformed/duplicate/missing declarations, source-bound exact preservation, and graph-metadata tampering. Local macOS produces 38 runtime files/13,610,684 bytes with two byte-identical 39-file build audits. The 156-file clean-install package and required Chromium/LibreOffice/Poppler gates pass locally; refreshed hosted evidence is pending.
