...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux CI with `103` tests. The QueryTable corpus now covers Office 2021-valid root plus refresh/field/deleted-field/sort import and in-place editing, immutable field/deleted-field/sort topology, formula-fill/data-bound and clipped/data-bound constraints, sort range/column/selector validation, ConnectionsPart/relationship preservation, independently opaque history/sort branches, source-free fabrication rejection, and field/refresh/sort extension retention. The preceding worksheet table, theme/static-style/native-formula, DOCX, and PPTX corpus remains green. Local macOS produces 38 inventoried runtime files/13,311,676 bytes; two isolated builds are byte-identical across 39 audited files. `open-chestnut-query-table` crosses a synthetic public-standard source graph through OpenChestnut import, safe refresh disablement plus root/refresh/field/deleted-field/sort edits, two source-preserving exports, package inspection, opaque-extension checks, and Playwright QA. Native office rendering is intentionally disabled for that fixture to avoid allowing a renderer to execute its external connection. Hosted run [`29373615650`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29373615650) passes OpenChestnut 103/103, OfficeBridge 5/5, all artifact/runnable-skill/docs/package gates, real Chromium/LibreOffice 24.2.7.2/Poppler 24.02.0, the 210-file clean install, and two byte-identical 38-file/13,311,678-byte Linux runtime builds.
