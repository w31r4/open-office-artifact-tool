...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `121` tests. The new corpus authors, imports, edits, detaches, and validates XLDAPR dynamic-array metadata; it also rejects shared-formula misuse and source-bound additions without an owned metadata record. The preceding workbook-view/worksheet/calculation/name/sort/connection/table/theme/style/formula/DOCX/PPTX coverage remains green. Local macOS produces 38 inventoried runtime files/13,431,996 bytes, and two isolated builds are byte-identical across all 39 audited files. `open-chestnut-basic` now carries a `SEQUENCE` spill alongside its two workbook windows, visibility, calculation, names, themes/styles/formulas/tables/filters, and row/column sorts through two WASM passes plus real Chromium/LibreOffice/Poppler QA; fallback and no-local-dotnet clean-install probes use the same public APIs. Hosted Linux run [`29389011985`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29389011985) passes OpenChestnut 121/121, OfficeBridge 5/5, real Chromium/LibreOffice 24.2.7.2/Poppler 24.02.0, the 150-file clean install, and two byte-identical 38-file/13,431,998-byte runtime builds.
