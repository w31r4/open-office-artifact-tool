...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux CI with `105` tests. The corpus now covers Office 2021-valid ordinary-table and fixed-topology QueryTable `none`/`pinYin`/`stroke` plus bounded custom-list sort import/edit/export, invalid vocabulary/selector combinations, and residual extension preservation. The shared workbook-level connection codec still keeps connection count/order and ID/type/version immutable; hides provider strings, commands, credentials, source paths, child graphs, unknown attributes, extensions, and unsupported connection types; and rejects source-free fabrication, binding tamper, removal, and identity changes. The preceding worksheet table, theme/static-style/native-formula, DOCX, and PPTX corpus remains green. Local macOS produces 38 inventoried runtime files/13,333,180 bytes. `open-chestnut-basic` now crosses source-free locale/custom-list sorting; `open-chestnut-query-table` crosses the same semantics through a synthetic public-standard source graph, safe refresh disablement, QueryTable/connection-root edits, two source-preserving exports, package inspection, opaque-extension checks, and Playwright QA. Native office rendering is intentionally disabled for that fixture to avoid allowing a renderer to execute its external connection. Hosted run [`29376600928`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29376600928) passes OpenChestnut 105/105, OfficeBridge 5/5, all artifact/runnable-skill/docs/package gates, Chrome for Testing 149, LibreOffice 24.2.7.2, Poppler 24.02.0, the 211-file clean install, and two byte-identical 38-file/13,333,182-byte Linux runtime builds.
