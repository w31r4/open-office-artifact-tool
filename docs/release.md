...
- The repository-only Agent PromptBench defines 27 black-box cases (19 PDF and 8 Office; 8 ready and 19 asset-required). The seven deterministic ready PDF cases already materialize isolated candidate trials with an npm-packed install and read-only source hashes. The eighth ready case is a generated XLSX threaded-comment direct-reply/resolve workflow with an independent package XML/original-identity/order/no-note-downgrade oracle, byte-bound audit/trace checks, second import, and LibreOffice/Poppler final rendering. Its first clean candidate trial, at `d558a924ad63528f2b2dca5e1bbeb1fb0dc120a7`, passed `100/100` across all machine, visual, security, and trace checks using package SHA-256 `f6889efc2cff246b59bc557761e998299fdb30466d98111096d25d562d72f618`; it is one trial, not a repeat matrix or reference-Skill comparison. A candidate/reference PDF preparation uses byte-identical package tarballs and prompts while varying only the copied Skill. Generic grading covers fail-closed branching, immutable file/directory inputs, read-only prompt/Skill/dependency trees, regular output types, and exact deliverables; the 19 corpus/PKI cases remain explicitly asset-required.
