...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `124` tests. The picture corpus now replaces uniquely referenced same-format ImagePart bytes without changing the part URI, retains unmodeled picture XML, and rejects cross-format or shared-part replacement alongside add/remove, binding tamper, invalid assets, and invalid geometry. The preceding workbook-view/worksheet/calculation/name/sort/connection/table/theme/style/formula/DOCX/PPTX coverage remains green. Local macOS produces 38 inventoried runtime files/13,461,180 bytes, and two isolated builds are byte-identical across all 39 audited files. `open-chestnut-basic` now performs the source-bound PNG replacement alongside its `SEQUENCE` spill and two workbook windows through two WASM exports plus real Chromium/LibreOffice/Poppler QA; fallback and no-local-dotnet clean-install probes use the same public APIs. Hosted Linux run [`29391242886`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29391242886) passes OpenChestnut 124/124, OfficeBridge 5/5, real Chromium/LibreOffice 24.2.7.2/Poppler 24.02.0, the 151-file clean install, and two byte-identical 38-file/13,461,182-byte runtime builds.
