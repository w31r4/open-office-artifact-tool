...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux CI with `84` tests. Eleven worksheet-table tests cover complete source-free authoring with Office 2021 validation, source-bound name/style, calculated-column/totals, and value/custom AutoFilter edits without part/relationship identity churn, unchanged complex/dynamic-filter byte preservation with lossy-replacement rejection, and structured invalid-profile rejection. The preceding workbook-theme/static-style/native-formula and DOCX/PPTX corpus remains green. Local macOS produces 38 runtime files/13,172,924 bytes; the 214-file no-local-dotnet clean install creates and imports value-filter metadata; and the OpenChestnut→OpenChestnut spreadsheet fixture carries both filter kinds with calculated-column/custom-totals metadata, table relationship, custom theme, static styles, shared/array topology, and number formats through modeled, package, Playwright, and real LibreOffice/Poppler QA. Hosted run [`29359575561`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29359575561) passes OpenChestnut 84/84, OfficeBridge 5/5, the 214-file package, real Chromium/LibreOffice/Poppler branches, and two byte-identical 38-file/13,172,926-byte Linux runtime builds.
