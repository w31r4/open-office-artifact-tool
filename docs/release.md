...
`test/package-contents.mjs` validates the actual dry-run tarball manifest. It requires the project-local runnable skills, production native bridge/OpenChestnut source, public protobuf source/generated binding, same-host reproducibility verifier, runtime integrity manifest/SBOM/.NET notices, consumer API/coverage docs, format codecs and validators, published fixtures, shared visual-baseline lifecycle, and third-party notices while rejecting reference/handoff material, development test source, local dotnet `bin/obj` output, and repository-only release/architecture/subsystem notes. C# and JavaScript test sources plus decision logs remain public on GitHub but are not npm runtime inputs. The current dry-run package contains 212 files, is about 4.887 MB compressed and 16,694,626 bytes unpacked. It remains below the unchanged 8,000,000/16,700,000-byte gates with 5,374 bytes of unpacked headroom after adding native worksheet visibility, source binding, active-tab safety, runnable-fixture coverage, generated help, the C# codec, and rebuilt runtime. Most of the footprint remains the bundled .NET/Open XML SDK runtime required for no-local-dotnet Office I/O.
...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `114` tests. The new worksheet-metadata corpus covers source-free visible/hidden/very-hidden state, `WorkbookView.ActiveTab`, source-bound name/state edits, all-hidden and active-tab rejection, binding tamper, unknown-state preservation, and Office 2021 validation; the preceding calculation/defined-name/sort/connection/table/theme/style/formula/DOCX/PPTX corpus remains green. Local macOS produces 38 inventoried runtime files/13,388,476 bytes, and two isolated builds are byte-identical across the 39-file audit. `open-chestnut-basic` crosses visible, hidden, and very-hidden sheets plus its calculation/defined-name/theme/style/formula/table/filter/sort profile through two WASM passes and real render QA; the no-local-dotnet clean-install package gate also passes. Hosted run [`29382260424`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29382260424) passes the same complete milestone with OpenChestnut 114/114, OfficeBridge 5/5, verified Chromium/LibreOffice 24.2.7.2/Poppler, the 212-file clean install, and two byte-identical 38-file/13,388,478-byte Linux runtime builds.
