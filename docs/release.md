...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `112` tests. The new calculation-policy corpus covers source-free `calcPr`, explicit-false scalar presence, source-bound edits, application `calcId` preservation, Office 2021 validation, exact opaque R1C1 preservation, absent/opaque topology rejection, and source binding; the preceding defined-name/sort/connection/table/theme/style/formula/DOCX/PPTX corpus remains green. Local macOS produces 38 inventoried runtime files/13,374,652 bytes, and two isolated builds are byte-identical across the 39-file audit. `open-chestnut-basic` crosses calculation policy plus its defined-name/theme/style/formula/table/filter/sort profile through two WASM passes and real render QA. Hosted run [`29381018453`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29381018453) passes the same calculation-policy gate with OpenChestnut 112/112, OfficeBridge 5/5, verified Chromium/LibreOffice/Poppler, the 212-file clean install, and two byte-identical 38-file/13,375,166-byte Linux runtime builds.
