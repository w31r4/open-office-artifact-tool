...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `53` tests. The new numbering planner tests prove complete direct-group `numberFormat`/`start`/`levelText` edits through an instance-local override, simultaneous text editing, unchanged shared `abstractNum`, Office 2021 validation/reimport, and partial/style-inherited rejection. Local macOS produces 38 runtime files/12,952,252 bytes and the 207-file clean install passes with the new real-render Documents fixture. Hosted run [`29336666891`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29336666891) remains the green preceding 51-test/205-file baseline while this slice awaits its hosted run.
