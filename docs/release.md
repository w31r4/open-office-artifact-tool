...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux (`47` tests). Coverage includes DOCX `w:lvl/w:pStyle`, `styleLink`, recursive `numStyleLink`, numbering-style identity, start overrides, and cyclic-link read-only behavior alongside the existing all-family, hyperlink/simple-field/fixed-table/direct-numbering, PPTX owner-local, exact package-guard, and Office 2021 gates. Hosted run [`29327443007`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29327443007) passes canonical OpenChestnut plus legacy-alias tests, bundled-runtime reproducibility, Chromium, LibreOffice, Poppler, the 202-file npm clean-install, and OfficeBridge tests.
