...
- when explicitly configured, the real optional-provider test covers ReportLab creation, pdfplumber extraction, pypdf forms/annotations, PyMuPDF rewrite/incremental/page/text/image/form/annotation edits, real redaction/scrub/residue scans, capped numerical text-fit behavior, canonical audit byte binding, and Poppler rendering;
...
- The dry-run npm tarball contains 403 files, is 9,107,510 bytes compressed and 22,516,760 bytes unpacked. It includes the rich PDF Skill tasks/references/examples, explicit Python-runtime selector, active-content inert scanner, canonical audit validator/schema, the shipped Spreadsheet Range and sparkline workflows, and the dependency-leaf OOXML package, Range, chart-source, and sparkline modules while excluding Python bytecode/cache files, development harnesses, private reference material, Agent PromptBench oracles, and removed Office codec paths.
...
- The explicit real-provider gate ran with ReportLab 4.4.9, pdfplumber 0.11.9, pypdf 6.10.0, and separately installed PyMuPDF 1.27.2.3 under the approved AGPL route. It proved byte-identical incremental prefixes, non-prefix rewrites, pypdf and PyMuPDF form/annotation operations, bounded text/image/page edits, full redaction/scrub, single-revision/no-residue output, and deliberate reflow rejection. The scrub-only active-content fixture now proves removal of root/additional JavaScript, Launch/SubmitForm actions, attachments, invisible text, comments, populated form values, personal metadata, and the null active-content dictionary names that PyMuPDF can leave after logical deletion; unfamiliar object serialization and invisible text overlapping visible text fail closed. Poppler page rendering passed. An image-bearing strict residue scan failed closed because Tesseract is not installed; qpdf, pyHanko, veraPDF, pikepdf, and OCRmyPDF were not executed and remain external/planned as documented.
...
- The repository-only Agent PromptBench validates 26 black-box cases (19 PDF and 7 Office; 7 ready and 19 asset-required). All seven deterministic ready cases materialized isolated candidate trials with an npm-packed install and read-only source hashes. A candidate/reference PDF preparation used byte-identical package tarballs and prompts while varying only the copied Skill. Generic grading covers fail-closed branching, immutable file/directory inputs, read-only prompt/Skill/dependency trees, regular output types, and exact deliverables. Bounded replacement, overflow refusal, and active-content sanitize additionally have independent semantic/structural, Poppler visual where applicable, residue/revision, audit-provenance, and provider-trace graders; the other 23 cases remain an explicit case-grader/corpus gate.
...
- The active-content public-sanitize path produced two historical candidate live Agent trials at 100/100. Independent pypdf evidence proved root/additional JavaScript, Launch/SubmitForm, five attachments, invisible text, a comment, a populated widget, and personal metadata in the immutable source and zero active structure/canary residue in those outputs. A later repeat had correct final bytes but scored zero after its Agent bypassed the typed primitive to remove null active-content names; the next two restarted repeats safely refused after selecting system Python instead of the configured provider runtime. Those runs are retained as defect evidence, not passes. The shipped scrub primitive now removes null active keys itself, and every Python entry point auto-reexecutes through `OPEN_OFFICE_PDF_PROVIDER_PYTHON`; the candidate/reference repeat matrix must restart on that fixed tarball.
