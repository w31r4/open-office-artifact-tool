...
`test/package-contents.mjs` also validates the actual dry-run tarball manifest. It requires the project-local runnable skills, native bridge source, spreadsheet Pivot/date/group/filter/formula/aggregation/coercion/threaded-comment modules, PresentationML grouped-shape and Office 2021 modern-comment codecs/validators, the DOCX bibliography, bookmark/internal-link, and section/header-footer planners, the PDF table-grid normalizer, the structured-intersection fixture/shared visual-baseline lifecycle, and third-party notices while rejecting reference/handoff material and local dotnet `bin/obj` build output. The current 92-file dry-run package is about 407.9 kB compressed and 1,801,201 bytes unpacked; it remains below the deliberately tight 1,820,000-byte gate. The latest reviewed increase adds deterministic `DATEVALUE`, `TIME`, `TIMEVALUE`, `HOUR`, `MINUTE`, `SECOND`, and `VALUE` evaluation, strict 1900/1904 and invalid/ambiguous-text handling, a third runnable DateTimeText worksheet, all-sheet Playwright rendering, and LibreOffice/Poppler plus ODS→XLSX calculation evidence. The gate catches accidentally bundled fonts, references, fixtures outside the published skills, and build artifacts while forcing explicit review when legitimate published capability grows the package.
