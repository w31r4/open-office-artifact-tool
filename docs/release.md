...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `73` tests. Four workbook-theme tests prove complete source-free authoring and Office 2021 validation, source-bound RGB edits with residual `sysClr`/font-scheme/extension preservation, unchanged complex-theme byte preservation with lossy-replacement rejection, and invalid/tampered-input rejection. Four static-style tests prove complete source-free authoring and Office 2021 validation, source-bound XF/resource cloning with residual `fontScheme`/`quotePrefix` preservation, structured invalid-style rejection, and resource deduplication; the preceding native-formula and DOCX/PPTX corpus remains green. Local macOS produces 38 runtime files/13,116,092 bytes; the 213-file no-local-dotnet clean install passes; and the OpenChestnut→OpenChestnut spreadsheet fixture carries its custom theme plus complete static styles, shared/array topology, and custom/percentage formats through modeled, package, Playwright, and real LibreOffice/Poppler QA. The latest hosted baseline, run [`29351669274`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29351669274), proves the preceding static-style gate, OfficeBridge 5/5, two byte-identical builds around a 38-file/13,096,126-byte Linux runtime, the 212-file clean install, and real Chromium/LibreOffice/Poppler.
