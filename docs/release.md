...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux CI with `110` tests. The new corpus covers source-free workbook/sheet-scoped range names, comment and explicit-false hidden presence, source-bound edits, Office 2021 validation, exact opaque constant-name preservation, and topology/collision/binding rejection; the preceding sort/connection/table/theme/style/formula/DOCX/PPTX corpus remains green. Local macOS produces 38 inventoried runtime files/13,356,732 bytes, and two isolated builds are byte-identical across the 39-file audit. `open-chestnut-basic` crosses defined names plus its existing theme/style/formula/table/filter/sort profile through two WASM passes and render QA. Hosted run [`29379765985`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29379765985) passes all npm/docs/package/runnable-skill gates, OfficeBridge 5/5, OpenChestnut 110/110, verified Chromium, LibreOffice 24.2.7.2, Poppler, the 211-file clean install, and two byte-identical 38-file/13,356,734-byte Linux runtime builds.
