...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `80` tests. Seven worksheet-table tests cover complete source-free authoring with Office 2021 validation, source-bound name/style and calculated-column/totals edits without part/relationship identity churn, unchanged complex-table byte preservation with lossy-replacement rejection, and structured invalid-profile rejection. The preceding workbook-theme/static-style/native-formula and DOCX/PPTX corpus remains green. Local macOS produces 38 runtime files/13,155,516 bytes; the 214-file no-local-dotnet clean install creates and imports calculated-column metadata; and the OpenChestnut→OpenChestnut spreadsheet fixture carries calculated-column/custom-totals metadata together with its table relationship, custom theme, static styles, shared/array topology, and number formats through modeled, package, Playwright, and real LibreOffice/Poppler QA. Hosted run [`29355927675`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29355927675) remains the latest published 77-test worksheet-table baseline while this rich-column increment awaits its hosted run.
