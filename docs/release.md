...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `135` tests. The current drawing corpus covers bounded bar/line/pie charts, direct RGB fill/line, styled direct line markers, chart-level standard/stacked/percent-stacked grouping, smooth true/false/absence, direct vary-colors true and native false normalization, title/tick font sizes, primary axes, all three anchor kinds, source-bound add/change/remove, complex-graph exact preservation, and Office 2021 zero-error validation. Local macOS produces 38 runtime files/13,605,052 bytes with two byte-identical 39-file build audits. The 156-file clean-install package and required Chromium/LibreOffice/Poppler `open-chestnut-basic` gate pass locally; the exact-SHA hosted result for this increment is recorded after CI completes.
...
- The styled line-marker increment's local gates pass: public wire fields, JavaScript fallback, bundled WASM, source-bound add/change/remove, fail-closed complex graphs, SVG preview, Office 2021 validation, clean install, reproducibility, and real browser/native spreadsheet QA all exercise the same marker projection. Exact-SHA hosted evidence is pending the functional commit's CI run.
