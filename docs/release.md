...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and on exact-SHA hosted Linux with `135` tests. The current drawing corpus covers bounded bar/line/pie charts, direct RGB fill/line, styled direct line markers, chart-level standard/stacked/percent-stacked grouping, smooth true/false/absence, direct vary-colors true and native false normalization, title/tick font sizes, primary axes, all three anchor kinds, source-bound add/change/remove, complex-graph exact preservation, and Office 2021 zero-error validation. Local macOS produces 38 runtime files/13,605,052 bytes with two byte-identical 39-file build audits; hosted Linux produces two byte-identical 38-file/13,605,054-byte builds. The 156-file clean-install package and required Chromium/LibreOffice/Poppler `open-chestnut-basic` gates pass in both environments.
