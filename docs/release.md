...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `105` tests. The QueryTable corpus covers Office 2021-valid root plus refresh/field/deleted-field/sort import and in-place editing, and the new shared workbook-level connection codec adds source-bound database/type-5 root metadata edits. It keeps connection count/order and ID/type/version immutable; hides and preserves provider strings, commands, credentials, source paths, child graphs, unknown attributes, extensions, and unsupported connection types; and rejects source-free fabrication, binding tamper, removal, and identity changes. The preceding worksheet table, theme/static-style/native-formula, DOCX, and PPTX corpus remains green. Local macOS produces 38 inventoried runtime files/13,329,596 bytes, and two isolated builds are byte-identical across 39 audited files. `open-chestnut-query-table` crosses a synthetic public-standard source graph through OpenChestnut import, safe refresh disablement plus QueryTable/connection-root edits, two source-preserving exports, package inspection, opaque-extension checks, and Playwright QA. Native office rendering is intentionally disabled for that fixture to avoid allowing a renderer to execute its external connection. The latest hosted predecessor run [`29373615650`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29373615650) passes OpenChestnut 103/103, OfficeBridge 5/5, all artifact/runnable-skill/docs/package gates, real Chromium/LibreOffice 24.2.7.2/Poppler 24.02.0, the 210-file clean install, and two byte-identical 38-file/13,311,678-byte Linux runtime builds; hosted evidence for the 105-test connection increment is pending this commit.
