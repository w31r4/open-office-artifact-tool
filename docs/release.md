...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux CI with `56` tests. The new table-format tests prove schema-valid fixed table/grid/cell widths, indent/margins/borders/header styling, semantic reimport, invalid width/border rejection, explicit no-border output, and source-bound formatting; the direct-numbering and numbering-planner corpus remains green. Local macOS produces 38 runtime files/12,979,388 bytes. Hosted run [`29342355609`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29342355609) is green for two byte-identical 38-file/12,979,390-byte Linux builds, the 209-file no-local-dotnet clean install, real Chromium/LibreOffice/Poppler including the formatted OpenChestnut→OpenChestnut merged-table fixture, and OfficeBridge 5/5.
