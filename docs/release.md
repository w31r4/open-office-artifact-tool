...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `129` tests. The picture corpus authors, imports, and source-preserves one-cell, two-cell, and absolute anchors; retains signed absolute position and optional `editAs` presence; authors/imports/edits/removes bounded signed `a:srcRect` crop, `a:alphaModFix`/`a:grayscl`/`a:lum` effects, and presence-aware `a:xfrm` rotation/horizontal/vertical flips while preserving picture locks, opaque effect/transform graphs, and transform offset/extent children; replaces uniquely referenced same-format ImagePart bytes without changing the part URI; and rejects out-of-profile crop/effect/transform replacement, anchor-kind conversion, cross-format/shared-part replacement, add/remove, binding tamper, invalid assets, and invalid geometry. The preceding workbook-view/worksheet/calculation/name/sort/connection/table/theme/style/formula/DOCX/PPTX coverage remains green. Local macOS produces 38 inventoried runtime files/13,502,140 bytes, and two isolated builds are byte-identical across all 39 audited files. `open-chestnut-basic` carries all three anchor kinds, crop/effects/transform, source-bound PNG replacement, its `SEQUENCE` spill, and two workbook windows through two WASM exports plus real Chromium/LibreOffice/Poppler QA; JavaScript fallback and no-local-dotnet clean-install probes use the same public model. The current 151-file package is 4,795,287 bytes compressed and 16,112,828 bytes unpacked. Hosted Linux run [`29396506905`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29396506905) remains the latest published pre-transform evidence: OpenChestnut 128/128, OfficeBridge 5/5, real Chromium/LibreOffice 24.2.7.2/Poppler 24.02.0, the 151-file clean install, generated docs/proto, and two byte-identical 38-file/13,494,974-byte runtime builds.
