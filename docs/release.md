...
- when explicitly configured, the real optional-provider test covers ReportLab creation, pdfplumber extraction, type-aware pypdf text/radio/checkbox forms and annotations, PyMuPDF rewrite/incremental/page/text/image/form/annotation edits, real redaction/scrub/residue scans, capped numerical text-fit behavior, canonical audit byte binding, and Poppler rendering;
...
- The explicit real-provider gate ran with ReportLab 4.4.9, pdfplumber 0.11.9, pypdf 6.10.0, and separately installed PyMuPDF 1.27.2.3 under the approved AGPL route. It proved byte-identical incremental prefixes, non-prefix rewrites, pypdf text/radio/checkbox value and appearance handling, pypdf and PyMuPDF annotation operations, bounded text/image/page edits, full redaction/scrub, single-revision/no-residue output, and deliberate reflow rejection. The scrub-only active-content fixture now proves removal of root/additional JavaScript, Launch/SubmitForm actions, attachments, invisible text, comments, populated form values, personal metadata, and the null active-content dictionary names that PyMuPDF can leave after logical deletion; unfamiliar object serialization and invisible text overlapping visible text fail closed. Poppler page rendering passed. An image-bearing strict residue scan failed closed because Tesseract is not installed; qpdf, pyHanko, veraPDF, pikepdf, and OCRmyPDF were not executed and remain external/planned as documented.
...
- The AcroForm adapter now distinguishes text/choice strings from radio/checkbox PDF Name states and validates post-write field/widget appearances before transaction promotion. Its complete generated fixture contains five text widgets, two radio widgets, and one pre-checked checkbox. A dirty candidate pilot filled the four requested values, preserved TIN/signature/unselected-radio/checkbox pixels and editability, preserved the exact source prefix with one appended revision, and scored 100/100 across machine, visual, security, and trace. This pilot is defect-discovery evidence only; the fixed clean candidate/reference repeat matrix remains required before release evidence is claimed.
