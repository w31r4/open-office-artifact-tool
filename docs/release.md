...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally (`49` tests) and on the prior hosted Linux milestone (`47` tests). Local coverage adds conforming horizontal/vertical table merges, source-bound grid/span/merge metadata, origin edits, continuation/topology rejection, and malformed-chain read-only behavior to the existing all-family, numbering-style, hyperlink/simple-field/table, PPTX owner-local, exact package-guard, and Office 2021 gates. Hosted run [`29327443007`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29327443007) remains green for canonical OpenChestnut plus legacy-alias tests, bundled-runtime reproducibility, Chromium, LibreOffice, Poppler, the 202-file npm clean-install, and OfficeBridge tests; refreshed hosted evidence is pending this milestone's push.
