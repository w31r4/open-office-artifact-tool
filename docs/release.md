...
- The dry-run npm tarball contains 406 files, is 9,121,236 bytes compressed and 22,573,679 bytes unpacked. It includes the rich PDF Skill tasks/references/examples, the six-page tagged-accessibility report workflow, explicit Python-runtime selector, path-safe attachment quarantine, type-aware AcroForm adapter, active-content inert scanner, canonical audit validator/schema, the shipped Spreadsheet Range and sparkline workflows, and the dependency-leaf OOXML package, Range, chart-source, and sparkline modules while excluding Python bytecode/cache files, development harnesses, private reference material, Agent PromptBench oracles, and removed Office codec paths.
