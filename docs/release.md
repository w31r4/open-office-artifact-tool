...
- The repository-only Agent PromptBench defines 29 black-box cases (19 PDF and 10 Office; 9 ready and 20 asset-required). The seven deterministic ready PDF cases already materialize isolated candidate trials with an npm-packed install and read-only source hashes. The ready Office set includes a generated XLSX threaded-comment direct-reply/resolve workflow with an independent package XML/original-identity/order/no-note-downgrade oracle and a generated DOCX classic-comment text-only workflow with independent `comments.xml`/body-anchor/metadata/no-modern-graph checks. Both bind byte-bound audit/trace checks, second import, and LibreOffice/Poppler final rendering. Its first clean XLSX candidate trial, at `d558a924ad63528f2b2dca5e1bbeb1fb0dc120a7`, passed `100/100` across all machine, visual, security, and trace checks using package SHA-256 `f6889efc2cff246b59bc557761e998299fdb30466d98111096d25d562d72f618`; it is one trial, not a repeat matrix or reference-Skill comparison. One dirty-worktree DOCX candidate trial at `c0be338ccc6c5c37a5cb47567650a57ab9363485` likewise passed `100/100` using package SHA-256 `917617cda7b904f35e71beddcd5486b25244c9585eb8d33b00dad29e8de3bf57`; it is development evidence rather than release evidence until repeated from a clean commit. A candidate/reference PDF preparation uses byte-identical package tarballs and prompts while varying only the copied Skill. Generic grading covers fail-closed branching, immutable file/directory inputs, read-only prompt/Skill/dependency trees, regular output types, and exact deliverables; the 20 corpus/PKI cases remain explicitly asset-required.
