...
- Current PromptBench inventory supersedes those historical counts: 35 cases (21 PDF, 14 Office), 14 deterministic ready cases, and 21 asset-required cases. The new ready `pptx-closed-leaf-slide-clone` fixture drives the shipped duplicate workflow with explicit `--allow-closed-leaves`; an independent OPC oracle permits only the adjacent clone SlidePart plus canonical NotesSlide and legacy SlideComments leaves, checks retained-source byte hashes and shared NotesMaster/CommentAuthors catalogs, requires second import, and compares source/clone/appendix LibreOffice/Poppler pixels. A freshly packed candidate followed by the published workflow scored `100/100` through every case-specific check; it is packaged-primitive/evaluator evidence, not an autonomous-Agent repeat matrix. The added asset-required `pdf-encrypted-owner-policy-boundary` is deliberately fail-closed: user-password-only input must not trigger owner-password bypass, silent decryption, or a replacement unencrypted PDF.
