...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `123` tests. The new corpus authors and imports embedded one-cell PNG/JPEG pictures, edits bounded metadata/anchor geometry while retaining unmodeled picture XML, validates Office 2021 output, and rejects add/remove, byte replacement, binding tamper, invalid assets, and invalid geometry. The preceding workbook-view/worksheet/calculation/name/sort/connection/table/theme/style/formula/DOCX/PPTX coverage remains green. Local macOS produces 38 inventoried runtime files/13,459,644 bytes, and two isolated builds are byte-identical across all 39 audited files. `open-chestnut-basic` now carries an embedded picture and a `SEQUENCE` spill alongside its two workbook windows through two WASM passes plus real Chromium/LibreOffice/Poppler QA; fallback and no-local-dotnet clean-install probes use the same public APIs. Hosted Linux run [`29390388809`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29390388809) passes OpenChestnut 123/123, OfficeBridge 5/5, real Chromium/LibreOffice 24.2.7.2/Poppler 24.02.0, the 151-file clean install, and two byte-identical 38-file/13,459,646-byte runtime builds.
