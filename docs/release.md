...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux CI (`51` tests each). Coverage adds source-free horizontal/vertical merge construction, exact physical-grid/row-span/continuation validation, default `TableGrid` style materialization, invalid authored-graph rejection, and a direct-authoring Documents fixture to the existing all-family, source-bound merge, numbering-style, hyperlink/simple-field/table, PPTX owner-local, exact package-guard, and Office 2021 gates. Local macOS produces 38 runtime files/12,931,772 bytes; hosted run [`29336666891`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29336666891) is green for two byte-identical 38-file/12,931,774-byte Linux builds, the 205-file clean install, Chromium, LibreOffice, Poppler, and OfficeBridge.
