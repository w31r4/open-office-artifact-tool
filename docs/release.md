...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `125` tests. The picture corpus now authors, imports, and source-preserves one-cell and two-cell anchors, retains optional `editAs` presence plus unmodeled crop/lock XML, replaces uniquely referenced same-format ImagePart bytes without changing the part URI, and rejects anchor-kind conversion, cross-format/shared-part replacement, add/remove, binding tamper, invalid assets, and invalid geometry. The preceding workbook-view/worksheet/calculation/name/sort/connection/table/theme/style/formula/DOCX/PPTX coverage remains green. Local macOS produces 38 inventoried runtime files/13,472,444 bytes, and two isolated builds are byte-identical across all 39 audited files. `open-chestnut-basic` now carries both anchor kinds, source-bound PNG replacement, its `SEQUENCE` spill, and two workbook windows through two WASM exports plus real Chromium/LibreOffice/Poppler QA; JavaScript fallback and no-local-dotnet clean-install probes use the same public model. Hosted Linux run [`29392551961`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29392551961) passes OpenChestnut 125/125, OfficeBridge 5/5, real Chromium/LibreOffice 24.2.7.2/Poppler 24.02.0, the 151-file clean install, generated docs/proto, and two byte-identical 38-file/13,472,446-byte runtime builds.
