...
- `dotnet test native/OpenXmlWasm/OpenXmlWasm.sln --configuration Release` passes locally (`11` tests), including XLSX/DOCX hash/source/opaque-graph or block-binding preservation, unsupported-advanced-edit rejection, and fail-closed Office 2021 validation. The bounded and complex XLSX fixtures retain hosted evidence from run [`29257565233`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29257565233); the complex DOCX business-brief now also passes the bundled-runtime, LibreOffice, and Poppler gates locally, with hosted evidence pending for the current commit.
