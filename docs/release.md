...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally with `131` tests. The drawing corpus authors/imports bounded bar, line, and pie charts with title, legend, literal or formula-backed categories, numeric caches, one-cell/two-cell/absolute anchors, and primary category/value axes; preserves residual ChartSpace XML; permits only fixed-topology source-bound name/title/legend/formula/cache/axis edits; imports formula-referenced series names and logarithmic axes read-only; and rejects chart add/remove, anchor/type/series/point/axis topology mutation, binding tamper, external data, combo/multi-plot profiles, invalid ranges, pie axes, text styling, and unsupported axis profiles. Pictures retain their preceding bounded crop/effects/transform and same-format byte-replacement coverage, including simultaneous picture/chart edits against one pre-mutation DrawingPart hash. Local macOS produces 38 inventoried runtime files/13,562,556 bytes, and two isolated builds are byte-identical across all 39 audited files. `open-chestnut-basic` carries a formula-backed line chart with primary-axis edits, all three picture anchor kinds, source-bound chart and image edits, its `SEQUENCE` spill, and two workbook windows through two WASM exports plus real Chromium/LibreOffice/Poppler QA; JavaScript fallback uses the same public model. The current 152-file package dry run is 4,826,164 bytes compressed and 16,207,709 bytes unpacked. The most recent exact-SHA hosted Linux baseline remains chart run [`29400392707`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29400392707); primary-axis hosted evidence follows after the current main push.
