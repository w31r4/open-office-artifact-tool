...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and on hosted Linux (`45` tests). Coverage includes all three Office families plus DOCX explicit paragraph-style-inherited numbering and cyclic-chain read-only behavior, existing hyperlink/simple-field/fixed-table/direct-numbering slices, PPTX owner-local semantics, exact package guards, and Office 2021 validation. Hosted run [`29318756487`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29318756487) also passes canonical OpenChestnut plus legacy-alias tests, two byte-identical 38-file/12,904,638-byte runtime builds, Chromium, LibreOffice, Poppler, the 201-file npm clean-install, and OfficeBridge tests.
