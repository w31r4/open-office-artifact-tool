...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux CI (`49` tests each). Coverage includes conforming horizontal/vertical table merges, source-bound grid/span/merge metadata, origin edits, continuation/topology rejection, and malformed-chain read-only behavior alongside the existing all-family, numbering-style, hyperlink/simple-field/table, PPTX owner-local, exact package-guard, and Office 2021 gates. Hosted run [`29330878218`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29330878218) is green for canonical OpenChestnut plus legacy-alias tests, two byte-identical 38-file/12,925,118-byte runtime builds, Chromium, LibreOffice, Poppler, the 203-file npm clean install, and OfficeBridge tests.
