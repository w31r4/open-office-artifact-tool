...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally and in hosted Linux CI with `102` tests. The QueryTable corpus now covers Office 2021-valid root plus refresh/field import and in-place editing, immutable field identity/topology, formula-fill/data-bound and clipped/data-bound constraints, ConnectionsPart/relationship preservation, opaque-refresh root editing, source-free fabrication rejection, and field/refresh extension retention. The preceding worksheet table, theme/static-style/native-formula, DOCX, and PPTX corpus remains green. Local macOS produces 38 inventoried runtime files/13,298,876 bytes; two isolated builds are byte-identical across 39 audited files. `open-chestnut-query-table` crosses a synthetic public-standard source graph through OpenChestnut import, safe refresh disablement plus root/refresh/field edits, two source-preserving exports, package inspection, opaque-extension checks, and Playwright QA. Native office rendering is intentionally disabled for that fixture to avoid allowing a renderer to execute its external connection. Hosted run [`29371595065`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29371595065) passes OpenChestnut 102/102, OfficeBridge 5/5, all artifact/runnable-skill/docs/package gates, real Chromium/LibreOffice 24.2.7.2/Poppler 24.02.0, the 209-file clean install, and two byte-identical 38-file/13,299,390-byte Linux runtime builds.
