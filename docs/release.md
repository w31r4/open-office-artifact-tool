...
- `dotnet test native/OpenChestnut/OpenChestnut.sln --configuration Release` passes locally (`47` tests) and on the prior hosted Linux milestone (`45` tests). Local coverage adds DOCX `w:lvl/w:pStyle`, `styleLink`, recursive `numStyleLink`, numbering-style identity, start overrides, and cyclic-link read-only behavior to the existing all-family, hyperlink/simple-field/fixed-table/direct-numbering, PPTX owner-local, exact package-guard, and Office 2021 gates. Hosted run [`29318756487`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29318756487) remains the latest completed evidence for canonical OpenChestnut plus legacy-alias tests, bundled-runtime reproducibility, Chromium, LibreOffice, Poppler, the 201-file npm clean-install, and OfficeBridge tests; refreshed hosted evidence is pending this milestone's push.
