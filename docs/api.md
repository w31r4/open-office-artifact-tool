...
Resolve stable document, block, content-control, bookmark, footnote/endnote, bibliography source ID/tag, header/footer, comment, style, and editable text-range IDs.
...
| `slide.duplicate` | api | Clone one original imported PPTX slide only when its unchanged graph contains canonical shapes, canonical inline fixed-grid tables, embedded rectangular images, and recursively canonical groups containing only those same leaf kinds, exactly one layout relationship, picture-bound image relationships, and optional closed NotesSlide-to-NotesMaster/back-to-slide plus bounded legacy-comments leaves. Accepted tables are inline-only and cannot add a fill, link, or another package edge; accepted groups add no relationship themselves and every nested picture must consume one exact verified ImagePart relationship. Export creates a distinct SlidePart and presentation relationship, shares the verified layout, immutable ImageParts, NotesMaster, and presentation-wide CommentAuthorsPart, copies accepted NotesSlide and SlideComments XML byte-for-byte, and repoints only the notes back-reference at the clone while retaining the origin. The clone must remain untouched until export and reimport; legacy comments remain source-bound read-only after that boundary, while connectors/charts/OLE/hyperlinks/external or irregular images/complex graphs fail closed. |
...
Clone one original imported PPTX slide only when its unchanged graph contains canonical shapes, canonical inline fixed-grid tables, embedded rectangular images, and recursively canonical groups containing only those same leaf kinds, exactly one layout relationship, picture-bound image relationships, and optional closed NotesSlide-to-NotesMaster/back-to-slide plus bounded legacy-comments leaves. Accepted tables are inline-only and cannot add a fill, link, or another package edge; accepted groups add no relationship themselves and every nested picture must consume one exact verified ImagePart relationship. Export creates a distinct SlidePart and presentation relationship, shares the verified layout, immutable ImageParts, NotesMaster, and presentation-wide CommentAuthorsPart, copies accepted NotesSlide and SlideComments XML byte-for-byte, and repoints only the notes back-reference at the clone while retaining the origin. The clone must remain untouched until export and reimport; legacy comments remain source-bound read-only after that boundary, while connectors/charts/OLE/hyperlinks/external or irregular images/complex graphs fail closed.

**Schema returns:**

- `slide` (Slide) — A new adjacent Slide. It is available only for an original imported PPTX source slide whose unchanged graph has canonical simple shapes, canonical inline fixed-grid tables, embedded rectangular images, plus recursively canonical p:grpSp groups whose descendants contain only those same leaf kinds, exactly one internal layout relationship, image relationships bound only by those pictures, and optionally one closed NotesSlide -> NotesMaster/back-to-source-slide leaf plus one bounded legacy SlideCommentsPart leaf. Accepted tables are inline-only: table fills, links, or other package edges remain outside this clone profile. Accepted groups add no relationship of their own; every nested picture must bind one exact verified ImagePart relationship. Export allocates a distinct SlidePart and presentation relationship, deliberately shares the verified layout, immutable ImageParts, NotesMaster, and presentation-wide CommentAuthorsPart, copies accepted NotesSlide and SlideComments XML byte-for-byte, repoints only the notes back-reference at the clone, and preserves the required origin parts. The clone must remain semantically unchanged until its first export and second import; supported legacy comments remain source-bound read-only after reimport. Source-free slides, already-cloned slides, same-transaction origin deletion, rich/connected comments, connectors, charts, OLE, hyperlinks, external/data relationships, unbound or irregular images, and every broader graph clone fail closed.
