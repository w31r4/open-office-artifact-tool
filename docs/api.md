...
| `slide.duplicate` | api | Clone one original imported PPTX slide only when its unchanged graph contains canonical shapes, canonical inline fixed-grid tables, embedded rectangular images, bounded canonical straight/elbow connectors, and recursively canonical groups containing only those same leaf kinds, exactly one layout relationship, picture-bound image relationships, canonical run-level external/internal/action click links with exact relationship IDs and retained targets, and optional closed NotesSlide-to-NotesMaster/back-to-slide plus bounded legacy-comments leaves. Every present connector endpoint must resolve to an element in the same copied SlidePart tree. Accepted tables are inline-only and cannot add a fill, link, or another package edge; accepted groups and connectors add no relationship themselves, and every nested picture must consume one exact verified ImagePart relationship. The pending clone resolves connector targets to fresh clone-local elements, while export privately preserves the source-bound endpoint identities. Export creates a distinct SlidePart and presentation relationship, shares the verified layout, immutable ImageParts, NotesMaster, and presentation-wide CommentAuthorsPart, copies accepted NotesSlide and SlideComments XML byte-for-byte, and repoints only the notes back-reference at the clone while retaining the origin. The clone must remain untouched until export and reimport; legacy comments remain source-bound read-only after that boundary, while unsupported connector forms or targets, charts, OLE, shape-level/hover/unknown/orphan hyperlinks, external or irregular images, and complex graphs fail closed. |
