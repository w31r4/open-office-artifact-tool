...
| `exportPptxWithOpenXmlWasm` | api | Experimentally export bounded rectangle/ellipse shapes and basic paragraph/run text through the bundled C# Open XML SDK WebAssembly codec, preserving source-bound unsupported native content fail-closed. |
| `importPptxWithOpenXmlWasm` | api | Experimentally import PPTX bytes with editable fixed-topology basic text, slide/shape-tree source bindings, and opaque part/relationship evidence for loss-aware second export. |
...
Experimentally export bounded rectangle/ellipse shapes and basic paragraph/run text through the bundled C# Open XML SDK WebAssembly codec, preserving source-bound unsupported native content fail-closed.

**Schema parameters:**

- `presentation` (Presentation) required — Presentation facade within the top-level rect/ellipse plus basic paragraph/run text boundary, or carrying validated fixed-topology source bindings from the WASM importer.
- `allowLossy` (boolean) — Explicitly permit discarding detected opaque OPC content when no validated source snapshot is available; defaults to false.
...
Experimentally import PPTX bytes with editable fixed-topology basic text, slide/shape-tree source bindings, and opaque part/relationship evidence for loss-aware second export.
