...
- `inspection` (object) — PDF file summary with sourceSha256, tagged/language/structure evidence, bounded indirect object records, source-bound mupdfAnnotation/mupdfLink/mupdfWidget records, and grouped mupdfFormField snapshots. Native page records include raw MediaBox/CropBox [x,y,width,height] values, normalized right-angle rotation, and an effective visible page-space bbox. Re-inspect after any rewrite because locators cannot identify a later byte sequence.
