...
| `slide.duplicate` | api | Clone one original imported PPTX slide only when its unchanged graph contains canonical shapes and embedded rectangular images, exactly one layout relationship, picture-bound image relationships, and an optional closed NotesSlide-to-NotesMaster/back-to-slide leaf. Export creates a distinct SlidePart and presentation relationship, shares the verified layout, immutable ImageParts, and NotesMaster, copies accepted NotesSlide XML byte-for-byte, and repoints that notes leaf at the clone while retaining the origin. The clone must remain untouched until export and reimport; comments/charts/OLE/hyperlinks/external or irregular images/complex graphs fail closed. |
...
- `slide` (Slide) — A new adjacent Slide. It is available only for an original imported PPTX source slide whose unchanged graph has canonical simple shapes plus embedded rectangular images, exactly one internal layout relationship, image relationships bound only by those pictures, and optionally one closed NotesSlide -> NotesMaster/back-to-source-slide leaf. Export allocates a distinct SlidePart and presentation relationship, deliberately shares the verified layout, immutable ImageParts, and NotesMaster through fresh clone-local relationships, copies the accepted NotesSlide XML byte-for-byte, repoints its back-reference at the clone, and preserves the required origin part. The clone must remain semantically unchanged until its first export and second import; source-free slides, already-cloned slides, same-transaction origin deletion, comments, charts, OLE, hyperlinks, external/data relationships, unbound or irregular images, and every broader graph clone fail closed.
