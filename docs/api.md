...
- `dataLabels` (boolean|object) — Optional plot-level labels. A boolean controls showValue; an object accepts only boolean showValue and showCategoryName. Per-series/per-point labels, positions, number formats, and label text styles remain outside this bounded profile.
