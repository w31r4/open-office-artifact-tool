...
- `calculatedFields` (object[]) — Calculated value fields over grouped source-field sums with arithmetic, percent, concatenation, comparisons, string/boolean constants, bounded ABS/SUM/MIN/MAX/AVERAGE/ROUND, and lazy IF/IFERROR branches. Accepts [Field] or quoted field references; cell references, calculated-field chaining, and non-whitelisted functions are rejected.
