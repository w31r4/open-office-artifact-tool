...
| `slide.duplicate` | api | Clone one original imported PPTX slide only when its unchanged graph contains canonical shapes and embedded rectangular images, exactly one layout relationship, and image relationships bound only by those pictures. Export creates a distinct SlidePart and presentation relationship, sharing the verified layout and immutable ImageParts while retaining the origin; the clone must remain untouched until export and reimport, and notes/comments/charts/OLE/hyperlinks/external or irregular images/complex graphs fail closed. |
...
- `slide` (Slide) — A new adjacent Slide. It is available only for an original imported PPTX source slide whose unchanged graph has canonical simple shapes plus embedded rectangular images, exactly one internal layout relationship, and image relationships bound only by those pictures. Export allocates a distinct SlidePart and presentation relationship, deliberately shares the verified layout and immutable ImageParts through fresh clone-local relationships, and preserves the required origin part. The clone must remain semantically unchanged until its first export and second import; source-free slides, already-cloned slides, same-transaction origin deletion, notes, comments, charts, OLE, hyperlinks, external/data relationships, unbound or irregular images, and every broader graph clone fail closed.
