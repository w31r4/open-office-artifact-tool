...
The previous OpenChestnut paragraph-style-inherited numbered-paragraph milestone passes hosted Linux run [`29318756487`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29318756487): canonical and compatibility APIs, 45 C# tests, the 201-file no-local-dotnet clean install, Chromium/LibreOffice/Poppler, OfficeBridge, and two byte-identical 38-file/12,904,638-byte runtime builds all pass on `main`. Refreshed hosted evidence for `w:lvl/w:pStyle`, `styleLink`, recursive `numStyleLink`, and additive numbering-style wire identity is pending the current milestone push.
