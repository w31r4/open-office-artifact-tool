...
The PPTX slice models slide order/size, a hash-bound Slide → Layout → Master package identity chain, top-level rectangle/ellipse shapes, bounded master `p:txStyles` defaults, and a bounded DrawingML paragraph/inline/text-body subset. Imported master, layout, and slide IDs are stable locators inside the bound source package; they are not a promise of identity after an unrelated file is re-imported. Every slide records its native layout relationship, every layout records its owning master, and source export rejects graph, name, layout-content, or slide-binding changes. From-scratch authoring currently creates one canonical master and its internal blank layout.

Shape paragraphs and shape-local `a:lstStyle` levels 0 through 8 cover alignment, direct margins and signed indents, point/multiplier spacing, tab stops, bounded default-run and inline styles, text/field/line-break inlines, character/auto-number/no-bullet/picture markers, bounded click links, and bounded `a:bodyPr` layout. `PptxParagraphPropertiesCodec` coordinates the shared paragraph-property subset, `PptxListStyleCodec` owns shape-local list-level topology, `PptxBodyPropertiesCodec` owns text-frame properties and AutoFit, and `PptxMasterTextStylesCodec` owns title/body/other master levels without flattening inheritance. `PptxPartContext` generalizes relationship ownership across Slide, Master, and Layout parts while the shared asset catalog owns content-addressed picture bytes.

Missing master-style fields preserve source state. Explicit level deletion removes only modeled properties and drops the native level node only when no residual attributes or children remain; underline, East Asian and complex-script fonts, color transforms, and other unmodeled data stay native. Fixed-topology edits retain old media and relationships, permit only tracked image/link relationship additions, validate against Office 2021, compare the guarded OPC graph, and require byte equality for every package part outside the explicitly changed slide/master XML, relationship part, content-type registration, and tracked media. Transformed colors or picture bullets, custom-show/mouse-over links, field-local paragraph properties, WordArt/3D/compatibility features, layout placeholders and styles, backgrounds, themes, charts, groups, notes, and arbitrary recursive OPC content remain source-preserved and fail closed when a caller attempts to replace them. The runnable `openxml-wasm-preservation` fixture crosses JavaScript authoring, bundled-WASM import, master-title edit, loss-aware master-body-level deletion, Master-owned external picture-marker addition, shape text/body/list edits, JavaScript semantic/package verification, and Playwright plus LibreOffice/Poppler rendering.

The expanded master/layout slice passes 32 local codec tests, a reproducible 38-file/12,836,039-byte runtime build, the 190-file clean-install package, and the real Chromium/LibreOffice/Poppler fixture. Hosted Linux run [`29293011969`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29293011969) remains the preceding 31-test, 189-file advanced text-body-property baseline; fresh hosted evidence for the master/layout slice is pending.
