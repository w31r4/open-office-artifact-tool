...
Query tables use a separate deep module because their package graph and security boundary differ from ordinary table XML. Open XML SDK exposes a TableDefinitionPart's [`QueryTableParts`](https://learn.microsoft.com/en-us/dotnet/api/documentformat.openxml.packaging.tabledefinitionpart.querytableparts), while [`QueryTable`](https://learn.microsoft.com/en-us/dotnet/api/documentformat.openxml.spreadsheet.querytable) selects an existing external connection by `connectionId`; the workbook's [`ConnectionsPart`](https://learn.microsoft.com/en-us/dotnet/api/documentformat.openxml.packaging.connectionspart) owns that definition. Its public [`QueryTableRefresh`](https://learn.microsoft.com/en-us/dotnet/api/documentformat.openxml.spreadsheet.querytablerefresh) and [`QueryTableField`](https://learn.microsoft.com/en-us/dotnet/api/documentformat.openxml.spreadsheet.querytablefield) types define the bounded nested profile. `XlsxQueryTableCodec` accepts exactly one internal TableDefinitionPart → QueryTablePart edge, requires the selected ID to exist in the source ConnectionsPart, and publishes presence-aware root attributes through additive `SpreadsheetTableArtifact.query_table` field 17. Query field 24 independently carries recognized refresh metadata and field semantics; field IDs/order/count and table-column bindings are immutable source topology, while names and bounded flags can change in place. The module enforces the Open XML SDK validity constraints that formula-fill fields are explicitly unbound and clipped fields are explicitly data-bound. It may edit those semantics or rebind the root to another already-declared connection, but it never authors a connection, executes a refresh, or exposes credentials/connection strings. Deleted-field/sort history, extensions, unknown XML, the connection part, and the relationship remain source-preserved and hash-bound. An opaque refresh is omitted from the public projection without hiding an otherwise recognized root; unsupported companion relationships still keep the complete parent table hidden/read-only. The query part path is exempted from opaque digest comparison only when that validated module actually writes it. The JavaScript codec recognizes the same stable projection for inspect/QA but refuses export because it cannot preserve this graph; source-free JavaScript and OpenChestnut authoring both fail closed.
