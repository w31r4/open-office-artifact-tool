...
The direct table-formatting slice raises the local codec gate to 56 tests, the 209-file no-local-dotnet clean install, and a 38-file/12,979,388-byte macOS runtime. Its public wire addition is optional and additive; the existing rows/grid/cell tags are unchanged. The merged-table fixture now proves non-default table/grid/cell widths, indent, margins, borders, and header styling through bundled-WASM authoring, reimport, source-bound text editing, real Chromium, and LibreOffice/Poppler. Hosted Linux run [`29342355609`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29342355609) passes the same 56-test/209-file gate, OfficeBridge, real browser/native render workflow, and two byte-identical 38-file/12,979,390-byte runtime builds.
