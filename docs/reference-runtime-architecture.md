...
The DOCX slice models ordered paragraphs, limited direct run formatting, table text, bounded whole-paragraph hyperlinks, and bounded whole-paragraph simple fields. `DocxHyperlinkCodec` recognizes one direct `w:hyperlink` with one text run, models external http(s) or internal bookmark targets plus text/tooltip/history, and changes those fields in place. `DocxPartContext` owns relationship additions/removals only for the hyperlink being edited. `DocxFieldCodec` recognizes `w:fldSimple`, exposes its instruction and visible result, and edits one-result-run fields only when their command belongs to an explicit non-fetching catalog. PAGE/NUMPAGES, section/page counts, date/time, document-property, and file/word/character-count commands are supported; complex fields, multiple result runs, INCLUDETEXT/DDE and all unrecognized commands remain source-preserved and read-only. Residual hashes protect paragraph properties, run properties, field attributes, extensions, and all other relationships. Imported body blocks carry their original body position, XML digest, semantic digest, residual digest where applicable, and an editability flag. Unchanged advanced WordprocessingML remains in the hash-bound source package; attempts to modify comments/bookmarks, drawings, numbering, sections, tables, complex/unsupported fields, or opaque top-level blocks fail closed until those semantics are modeled. The complex business-brief fixture changes an external hyperlink target/metadata and its PAGE field to NUMPAGES through the bundled WASM codec, preserves its broader Word package, and then runs the existing JS package/semantic plus LibreOffice/Poppler gates.
...
The canonical OpenChestnut identity migration passes hosted Linux run [`29309300830`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29309300830): canonical and compatibility APIs, 39 C# tests, the 199-file no-local-dotnet clean install, Chromium/LibreOffice/Poppler, OfficeBridge, and two byte-identical 38-file/12,886,204-byte runtime builds all pass on `main`.
