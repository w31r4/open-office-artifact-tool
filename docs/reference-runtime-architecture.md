...
The native PDF Skill calls the same `PdfFile` MuPDF.js primitives through a thin JavaScript CLI. Supported operations include read/inspect, PNG/JPEG render, annotations, bounded text/choice/checkbox form values, page delete/reorder, metadata, embedded-file/link deletion, and rewrite redaction. Python adapters remain for capabilities without a trustworthy JavaScript equivalent: strict scrub/residue, typed pypdf attachment/forms/merge/stamp workflows, and complex table extraction. A separate shipped qpdf 11+ adapter provides bounded JSON-v2 structural inspection plus exact-source-bound full-rewrite repair/linearization through an externally installed executable; it has no arbitrary-flag surface and is not a sanitizer, redactor, renderer, strict conformance validator, or signature-trust provider. The shipped pikepdf adapter is a distinct boundary over an explicitly installed pikepdf 10.10.x runtime: it inspects a private SHA-bound snapshot and exposes only two curated full-rewrite profiles for JavaScript/external-action/multimedia removal and optional attachment/thumbnail/search-index/Web-Capture/private-data/portfolio cleanup. It rejects encryption and parser warnings, requires caller trust/isolation plus signature invalidation, and deliberately retains metadata, forms, XFA, annotations, hidden/OCR text, and visible content; it is not strict sanitize or malware isolation. The shipped pyHanko adapter is a separate read-only boundary over an explicitly installed pyHanko 0.35.x core runtime: it validates an immutable SHA-bound snapshot under hard budgets, accepts only caller-supplied trust roots, disables network fetching, and reports integrity, trust, revision coverage, difference level, timestamps, DocMDP, FieldMDP, and policy gates without claiming complete PAdES conformance. The shipped veraPDF adapter is another read-only boundary over an explicitly installed veraPDF 1.30.x CLI: it binds one final PDF to an exact SHA-256 and explicit built-in PDF/A/PDF/UA profile, parses a bounded internally consistent JSON report, and keeps machine-rule compliance separate from operation success and human PDF/UA review. The shipped OCRmyPDF adapter routes one exact complete-document snapshot through separately installed OCRmyPDF 17.8.x, Tesseract 5.x, qpdf, and Poppler under explicit trust/loss gates and a full-rewrite transaction; it is neither sanitize nor host-level malware isolation. Signature creation and private-key/TSA/LTV operations remain external workflows. Poppler remains independent final render QA.
...
- DOCX adds style/default formatting, paragraph/run formatting, bounded inline plain-text content-control identity, section/header/footer, field, and image records.
