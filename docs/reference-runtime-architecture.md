...
Query tables use a separate deep module because their package graph and security boundary differ from ordinary table XML. Open XML SDK exposes a TableDefinitionPart's [`QueryTableParts`](https://learn.microsoft.com/en-us/dotnet/api/documentformat.openxml.packaging.tabledefinitionpart.querytableparts), while [`QueryTable`](https://learn.microsoft.com/en-us/dotnet/api/documentformat.openxml.spreadsheet.querytable) selects an existing external connection by `connectionId`; the workbook's [`ConnectionsPart`](https://learn.microsoft.com/en-us/dotnet/api/documentformat.openxml.packaging.connectionspart) owns that definition. `XlsxQueryTableCodec` accepts exactly one internal TableDefinitionPart → QueryTablePart edge, requires the selected ID to exist in the source ConnectionsPart, and publishes presence-aware root attributes through additive `SpreadsheetTableArtifact.query_table` field 17. It may edit those root attributes or rebind to another already-declared connection, but it never authors a connection, executes a refresh, or exposes credentials/connection strings. Query refresh history/fields, extensions, unknown XML, the connection part, and the relationship remain source-preserved and hash-bound. The query part path is exempted from opaque digest comparison only when that validated module actually writes it. Unsupported companion relationships keep the complete parent table hidden/read-only. The JavaScript codec can recognize the same stable query projection for inspect/QA but refuses export because it cannot preserve this graph; source-free JavaScript and OpenChestnut authoring both fail closed.
