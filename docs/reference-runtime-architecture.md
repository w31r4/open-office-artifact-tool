...
The DOCX slice models ordered paragraphs, limited direct run formatting, table text, bounded whole-paragraph hyperlinks, bounded whole-paragraph simple fields, and direct-numbered single-run paragraphs. `DocxHyperlinkCodec` recognizes one direct `w:hyperlink` with one text run, models external http(s) or internal bookmark targets plus text/tooltip/history, and changes those fields in place. `DocxPartContext` owns relationship additions/removals only for the hyperlink being edited. `DocxFieldCodec` recognizes `w:fldSimple`, exposes its instruction and visible result, and edits one-result-run fields only when their command belongs to an explicit non-fetching catalog. `DocxTableCodec` exposes fixed-topology tables as editable only when every cell contains one paragraph, one run, and one text node; it updates those text nodes in place while residual-hashing all table/row/cell/paragraph/run properties. `DocxNumberedParagraphCodec` resolves a direct `w:numPr` through `numId` and `abstractNum` to format/start/level text using a read-only XML stream, avoiding Open XML SDK AutoSave normalization of the untouched shared numbering part; only the single text node is editable. Row/cell count changes, style or numbering changes, merged/nested tables, and multi-paragraph or multi-run content fail closed. Imported body blocks carry their original body position, XML digest, semantic digest, residual digest where applicable, and an editability flag. The complex business-brief fixture changes an external hyperlink, a PAGE field, one simple-table cell, and one list-item text through OpenChestnut while preserving its bookmark/comment-bearing table, shared numbering graph, sections, citations, drawings, and other advanced WordprocessingML before JS package/semantic plus LibreOffice/Poppler gates.
...
The current OpenChestnut table-editing milestone passes hosted Linux run [`29310339834`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29310339834): canonical and compatibility APIs, 41 C# tests, the 200-file no-local-dotnet clean install, Chromium/LibreOffice/Poppler, OfficeBridge, and two byte-identical 38-file/12,889,790-byte runtime builds all pass on `main`.
