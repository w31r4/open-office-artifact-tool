...
The DOCX slice models ordered paragraphs, limited direct run formatting, table text, bounded whole-paragraph hyperlinks, bounded whole-paragraph simple fields, and direct or explicit paragraph-style-inherited numbered single-run paragraphs. `DocxHyperlinkCodec` recognizes one direct `w:hyperlink` with one text run, models external http(s) or internal bookmark targets plus text/tooltip/history, and changes those fields in place. `DocxPartContext` owns relationship additions/removals only for the hyperlink being edited and caches read-only `styles.xml`/`numbering.xml` documents without materializing Open XML SDK roots. `DocxFieldCodec` recognizes `w:fldSimple`, exposes its instruction and visible result, and edits one-result-run fields only when their command belongs to an explicit non-fetching catalog. `DocxTableCodec` exposes fixed-topology tables as editable only when every cell contains one paragraph, one run, and one text node; it updates those text nodes in place while residual-hashing all table/row/cell/paragraph/run properties. `DocxNumberedParagraphCodec` resolves either direct `w:numPr` or an explicit `w:pStyle`/`w:basedOn` cascade through `numId` and `abstractNum` to format/start/level text; only the single text node is editable. Missing, duplicate, cyclic, over-deep, or unresolved style/definition graphs stay source-preserved and read-only instead of falling back to an editable plain paragraph. Row/cell count changes, style or numbering changes, merged/nested tables, and multi-paragraph or multi-run content fail closed. Imported body blocks carry their original body position, XML digest, semantic digest, residual digest where applicable, and an editability flag. The complex business-brief fixture removes one list item's direct `w:numPr`, resolves it through its authored paragraph style, then changes that text plus an external hyperlink, a PAGE field, and one simple-table cell through OpenChestnut while preserving its bookmark/comment-bearing table, shared numbering graph, sections, citations, drawings, and other advanced WordprocessingML before JS package/semantic plus LibreOffice/Poppler gates.
