...
The PPTX slice models slide order/size, top-level rectangle/ellipse shapes, and a bounded DrawingML paragraph/inline/text-body subset. It authors and imports paragraph level, left/center/right/justified alignment, direct `marL`/signed `indent` list coordinates, direct `lnSpc`/`spcBef`/`spcAft` point-or-multiplier spacing, direct `a:defRPr` bold/italic/size/Latin-font/solid-RGB-or-theme styles, ordered ordinary text, `a:fld`, and `a:br` inlines, left/center/right/decimal `a:tabLst` stops, the direct list-marker choice (character, all standard auto-number schemes with optional start, explicit no-bullet, or a picture), three independent direct marker-style choices, inline styles, bounded click links, and `a:bodyPr` left/top/right/bottom insets, vertical anchor, wrapping, AutoFit mode, ±360-degree rotation, horizontal/vertical/vertical270 text, horizontal/vertical overflow, one through sixteen columns with spacing/RTL flow, and upright text. Shape-local `a:lstStyle` children `lvl1pPr` through `lvl9pPr` reuse that same property subset through Open XML SDK's public `TextParagraphPropertiesType`; a dedicated list-style codec owns level topology, uniqueness, source-preserving add/remove, and unknown-property retention. `PptxBodyPropertiesCodec` independently owns the bounded text-frame attributes and the `noAutofit`/`normAutofit`/`spAutoFit` child choice. Picture sources are either a content-addressed envelope `Asset` containing bounded PNG/JPEG/GIF/safe-SVG bytes or a non-fetched absolute http(s) URI; field 16 adds that choice without changing existing bullet, tab-stop, or text wire fields. Untransformed marker theme colors use additive field 17 and preserve one of the 16 public DrawingML scheme tokens rather than resolving it to an RGB snapshot. Paragraph coordinate value/delete choices use additive fields 18 through 21; three spacing oneofs use additive fields 22 through 30 and retain native `spcPts` versus `spcPct` identity. Default-run style value/delete choices use additive fields 31 and 32. `PresentationTextBody.list_styles` uses additive field 2 with required unique levels and empty runs; additive field 3 carries explicit all-modeled-level deletion intent; additive field 4 carries an independent `PresentationTextBodyProperties` message. Its original seven value/delete choices occupy fields 1 through 14; rotation, vertical text, vertical/horizontal overflow, column count/spacing/direction, and upright choices occupy additive fields 15 through 30 and retain exact native angle/EMU/presence identity. Clearing an imported direct coordinate, spacing slot, modeled default-run style, list level, or body property restores inheritance/native defaults while absence still preserves unknown source state; deletion retains unmodeled underline, script fonts, color transforms, list-level attributes/children, body anchor-centering/compatibility flags, and other native data. A shared C# asset catalog owns durable bytes and media reuse, while each `PptxSlideContext` owns only slide-local relationships. Every imported slide and top-level shape-tree element remains hash-bound to its source. Fixed-topology edits mutate existing PresentationML nodes in place, retain old media/relationships as opaque content, and permit only explicitly tracked image/link relationship and media-part additions. Transformed, unsupported-format, malformed, or unresolved picture bullets remain residual and fail closed if replaced. Transformed or non-sRGB/non-theme marker colors, transformed default-run colors, attributed normal-AutoFit scaling, custom-show/mouse-over links, field-local `a:pPr`, unknown break properties, WordArt vertical modes/warp/3D, body anchor-centering/compatibility flags, master/layout text-style editing, unsupported alignment, pictures, charts, groups, notes, masters, layouts, themes, and arbitrary recursive OPC graphs stay source-preserved. Post-write validation requires Office 2021 zero errors, exact guarded graph equality after removing only modeled additions, and byte equality for every package part except the changed slide XML, its changed relationship part, `[Content_Types].xml` when necessary, and exact tracked new media. The runnable `openxml-wasm-preservation` fixture crosses JS export, WASM body/list/layout/spacing/default-run-style/text/link/field/break/tab-stop/picture-bullet/theme-color edits, JS semantic/package verification, and Playwright plus LibreOffice/Poppler rendering.

Hosted Linux run [`29293011969`](https://github.com/w31r4/open-office-artifact-tool/actions/runs/29293011969) passes the 31-test codec, reproducible runtime, 189-file clean-install package, and real Chromium/LibreOffice/Poppler gates for the complete advanced text-body-property slice.
