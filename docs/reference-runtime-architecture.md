...
The PPTX slice models slide order/size, top-level rectangle/ellipse shapes, and a bounded basic DrawingML paragraph/run subset. It authors and imports paragraph level, left/center/right/justified alignment, the direct list-marker choice (character, all standard auto-number schemes with optional start, or explicit no-bullet), three independent direct marker-style choices (font/follow-text, solid RGB color/follow-text, and point/percentage/follow-text size), run text/bold/italic/point size/Latin font family/solid RGB color, and click links targeting an absolute external URI, an internal slide ID, or one of the five standard relative slide-show actions. Link tooltip, target frame, history, and click-highlight flags roundtrip. Protobuf `oneof` groups make contradictory marker, marker-style, and link targets unrepresentable; an unset link preserves unknown native source actions while explicit `no_hyperlink` records deletion intent. Every imported slide binds to its source part and presentation relationship; every top-level shape-tree element binds to its ordinal, XML digest, semantic digest, and editability state. Source-bound text edits require the original paragraph/run topology and mutate the existing PresentationML nodes in place. Picture bullets, non-RGB or transformed marker colors, custom-show/mouse-over links, underline, East Asian font, unsupported alignment, and other residual XML survive under a separate hash guard; replacing an unknown marker, marker-style choice, or click action fails closed. Pictures, charts/graphic frames, groups, connectors, content parts, notes, masters, layouts, themes, media, and arbitrary recursive OPC graphs remain source-preserved and read-only. The codec may add external-hyperlink or internal-slide relationships with IDs allocated by Open XML SDK, but every pre-existing relationship—including unknown hyperlink relationships—remains in the opaque inventory and must retain its exact signature. Post-write validation requires Office 2021 zero errors, exact guarded graph equality after removing only those explicitly tracked additions, and byte equality for every package part except explicitly changed slide XML and, when a modeled link adds a relationship, that slide's relationship part. The runnable `openxml-wasm-preservation` fixture crosses JS export, WASM list/style/text/link edits, JS semantic/package verification, and Playwright plus optional LibreOffice/Poppler rendering.
