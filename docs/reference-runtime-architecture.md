...
The DOCX slice models ordered paragraphs, limited direct run formatting, table text, bounded whole-paragraph hyperlinks, and bounded whole-paragraph simple fields. `DocxHyperlinkCodec` recognizes one direct `w:hyperlink` with one text run, models external http(s) or internal bookmark targets plus text/tooltip/history, and changes those fields in place. `DocxPartContext` owns relationship additions/removals only for the hyperlink being edited. `DocxFieldCodec` recognizes `w:fldSimple`, exposes its instruction and visible result, and edits one-result-run fields only when their command belongs to an explicit non-fetching catalog. `DocxTableCodec` exposes fixed-topology tables as editable only when every cell contains one paragraph, one run, and one text node; it updates those text nodes in place while residual-hashing all table/row/cell/paragraph/run properties. Row/cell count changes, style changes, merged/nested tables, and multi-paragraph or multi-run cells fail closed. Imported body blocks carry their original body position, XML digest, semantic digest, residual digest where applicable, and an editability flag. The complex business-brief fixture changes an external hyperlink, a PAGE field, and one simple-table cell through OpenChestnut while preserving its bookmark/comment-bearing table, numbering, sections, citations, drawings, and other advanced WordprocessingML before JS package/semantic plus LibreOffice/Poppler gates.
