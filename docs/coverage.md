...
| Agent black-box PromptBench | partial | The repository-only v1 suite defines 26 realistic prompts (19 PDF, 7 Office), isolated candidate/reference Skill trials against the same packed npm candidate, dirty-worktree plus tarball provenance, immutable input fingerprints including directory trees, exact deliverables, fail-closed outcomes, and generic hard gates. Bounded contract-ID replacement, overflow refusal, and active-content public sanitize now have independent pypdf/pdfplumber structural/semantic evidence, Poppler/Pillow all-page comparison where applicable, residue/revision gates, audit provenance, and Codex trace discipline grading. The active-content fixture proves root/additional JavaScript, Launch/SubmitForm actions, attachments, invisible text, comments, populated forms, and personal metadata existed before requiring every channel to clear. One fixed replacement candidate trial, one same-tarball replacement reference-Skill trial, and two active-content candidate repeats scored 100/100; a third active-content repeat correctly failed the trace gate after a direct object-level bypass exposed null active-content names left by the typed scrub. The primitive is fixed and the three-repeat candidate/reference matrix must restart on its new tarball. Four other generated/inline PDF cases still have generic gates only; 19 corpus/PKI cases await pinned licensed assets. |
