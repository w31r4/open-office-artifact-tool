...
| Agent black-box PromptBench | partial | The repository-only v1 suite defines 26 realistic prompts (19 PDF, 7 Office), isolated candidate/reference Skill trials against the same packed npm candidate, dirty-worktree plus tarball provenance, immutable input fingerprints including directory trees, exact deliverables, fail-closed outcomes, and generic hard gates. Bounded contract-ID replacement, overflow refusal, and active-content public sanitize now have independent pypdf/pdfplumber structural/semantic evidence, Poppler/Pillow all-page comparison where applicable, residue/revision gates, audit provenance, and Codex trace discipline grading. The active-content fixture proves root/additional JavaScript, Launch/SubmitForm actions, attachments, invisible text, comments, populated forms, and personal metadata existed before requiring every channel to clear. Its fixed candidate and reference-Skill matrices each pass `3/3` at 100/100 against one clean commit/tarball, identical prompt/input/oracle fingerprints, fixed within-subject Skill fingerprints, and intentionally different candidate/reference Skill fingerprints. Earlier failures remain defect evidence for null-key bypass and provider-runtime drift; evaluator regressions distinguish help from mutation and require both independently scheduled preflights to complete before edit. Four other generated/inline PDF cases still have generic gates only; 19 corpus/PKI cases await pinned licensed assets. |
...
| PDF | partial | The native plugin is now a reference-compatible provider-routing superset: greenfield `PdfArtifact`, ReportLab creation, pdfplumber/pypdf basics, direct-original PyMuPDF imported-PDF edits and sanitize, mandatory pre-mutation probe/route plan, explicit rewrite/incremental/sanitize policy, a versioned canonical audit schema plus byte-binding validator, residue scanning, and Poppler QA. Local real-provider tests and the fully graded active-content same-prompt candidate/reference `3/3 + 3/3` matrix cover those shipped adapters. qpdf/pyHanko/veraPDF external execution, OCRmyPDF/pikepdf adapters, hosted optional-provider tests, other-case repeat matrices, and broader corpus QA remain open. |
