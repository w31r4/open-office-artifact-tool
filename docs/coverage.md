...
| Render/preview loop | partial | `renderArtifact(...)` plus per-artifact render/export adapters return SVG `FileBlob` previews with metadata for workbooks, presentations, documents, and PDFs. `renderArtifact(..., { format, renderer })` now supports pluggable PNG/WebP/JPEG/PDF conversion adapters over the SVG/FileBlob source. Built-in rasterization and external Poppler/LibreOffice adapters remain roadmap. |
