...
| JavaScript source layering | partial | The root entry preserves the 36-symbol public API while Help, Compose, binary/image/PNG/render primitives, the cross-format raster registration/diff/visual-QA engine, `FileBlob`, inspection, the complete PDF domain, the shared OOXML inspect/patch engine, pure Spreadsheet range-address/translation/copy-shape/chart-source/sparkline model rules, and Presentation native-object ownership live in dependency-leaf modules. Visual QA receives artifact-kind inference through one explicit factory boundary, so it has no model back-edge or circular import; the root retains only the public wrappers. The OOXML module owns JSZip package loading, safety budgets, OPC paths/content types/relationships, recipes/source references, validation, and transactional regeneration behind four internal exports; the root owns only family policy and File facades. Root `PdfArtifact`/`PdfFile` bindings are strict-identical re-exports and the cross-format ID allocator remains a single shared module. `src/index.mjs` is now 6,723 of 28,021 JavaScript source lines (24.0%). Stateful Worksheet/Range/Chart behavior, the remaining Presentation model, and Document remain to be extracted only at cohesive ownership boundaries. |
