...
| PDF | partial | The native plugin is now a reference-compatible provider-routing superset: greenfield `PdfArtifact` including a runnable six-page CJK accessible-report workflow, ReportLab creation, pdfplumber/pypdf basics, typed attachment quarantine, type-aware AcroForm filling, complete-source merge/reorder/selective watermarking, direct-original PyMuPDF imported-PDF edits and sanitize, mandatory probe/route preflight, explicit read-only/rewrite/incremental/sanitize policy, canonical byte-bound audits, residue scanning, and typed Poppler source/output comparison. Local real-provider tests and fully graded same-prompt matrices cover attachment `3/3 + 2/3`, AcroForm `3/3 + 2/3`, active-content `3/3 + 3/3`, greenfield accessibility `3/3 + 3/3`, and merge/stamp `3/3 + 3/3`. The retained reference misses are intentional workflow-discipline evidence, not rerun targets. qpdf/pyHanko/veraPDF external execution, OCRmyPDF/pikepdf adapters, hosted optional-provider tests, remaining matrices, and broader corpus QA remain open. |
