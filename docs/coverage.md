...
| JavaScript source layering | partial | The root entry preserves the 36-symbol public API while Help, Compose, binary/image/PNG/render/text-range primitives, the cross-format raster registration/diff/visual-QA engine, `FileBlob`, inspection, the complete PDF domain, the complete Document model/layout/render/file-facade domain, the shared OOXML inspect/patch engine, pure Spreadsheet range-address/translation/copy-shape/chart-source/sparkline model rules, and Presentation native-object ownership live in dependency-leaf modules. Visual QA receives artifact-kind inference through one explicit factory boundary, so it has no model back-edge or circular import; the root retains only public wrappers and exact binding re-exports. The OOXML module owns JSZip package loading, safety budgets, OPC paths/content types/relationships, recipes/source references, validation, and transactional regeneration behind four internal exports. Root `PdfArtifact`/`PdfFile` and `DocumentModel`/`DocumentFile` bindings are strict-identical re-exports, and the cross-format ID allocator remains a single shared module. `src/index.mjs` is now 5,694 of 28,043 JavaScript source lines (20.3%). Stateful Worksheet/Range/Chart behavior and the remaining Presentation model remain to be extracted only at cohesive ownership boundaries. |
...
| Bookmarks, bibliography, advanced settings, complex fields, content controls, drawings | partial | Preserve imported content when source evidence remains valid; creation/editing is unsupported. Only modeled section/header behavior and `evenAndOddHeaders` are writable settings; `trackRevisions`, `updateFields`, `mirrorMargins`, and `documentProtection` fail closed. |
