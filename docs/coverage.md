...
| `PresentationFile.exportPptx` / `importPptx` | partial | Minimal PPTX round trip for text shapes works; table facades now export as native DrawingML table `p:graphicFrame`/`a:tbl` XML, and `dataUrl` image facades export as native `ppt/media/image*.ext` parts with slide `p:pic` XML and relationships. Needs true PPTX chart OOXML, themes, masters, layouts, notes, comments, connectors, and image/table import restoration. |
