...
| Presentations | partial | The native plugin and complete 26-slide built-in template pass canonical OpenChestnut export/import, including bounded custom geometry. Plain-text speaker notes now cross the public Skill/model/protobuf/Open XML path with stable inspect/resolve anchors and fail-closed rich-notes preservation. The broader reference API guide still exceeds the current PPTX boundary. |
