...
| Agent black-box PromptBench | partial | The repository-only v1 suite defines 26 realistic prompts (19 PDF, 7 Office), isolated candidate/reference Skill trials against the same packed npm candidate, immutable input fingerprints including directory trees, exact deliverables, fail-closed outcomes, and generic hard gates. The bounded contract-ID replacement and overflow-refusal pilots now have independent pypdf/pdfplumber semantic evidence, Poppler/Pillow all-page comparison where applicable, residue/revision gates, audit provenance, and Codex trace discipline grading. Five other generated/inline PDF cases still have generic gates only; 19 corpus/PKI cases await pinned licensed assets. |
