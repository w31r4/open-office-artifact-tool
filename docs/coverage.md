...
| `SpreadsheetFile.exportXlsx` / `importXlsx` | partial | Minimal OOXML XLSX round trip implemented with inline strings/formulas. Clean-room metadata part preserves comments, data validations, conditional formatting, worksheet tables, and worksheet charts across roundtrip; worksheet XML also emits simple dataValidation/conditionalFormatting nodes. Needs styles, shared strings, native table/chart OOXML, pivots, images, and native threaded-comment OOXML. |
...
| `workbook.inspect` table/formula/match/sheet/workbook | partial | Core records implemented for workbook, sheet, table/structured table, formula, match, drawing/chart, thread, dataValidation, and conditionalFormat with simple search filtering. Needs computedStyle, definedName, images, and richer include/exclude. |
| `workbook.render` visual verification | partial | SVG grid preview includes cell values plus worksheet table outlines and simple chart previews. Needs PNG output and full drawing/image rendering. |
