...
| JavaScript source layering | partial | The root entry preserves the 36-symbol public API while Help, Compose, binary/image/PNG/render/text-range primitives, the cross-format raster registration/diff/visual-QA engine, `FileBlob`, inspection, the complete PDF, Document, and Presentation model/layout/SVG/file-facade domains, the shared OOXML inspect/patch engine, and pure Spreadsheet range-address/translation/copy-shape/chart-source/sparkline model rules live in dependency-leaf modules. Visual QA receives artifact-kind inference through one explicit factory boundary, so it has no model back-edge or circular import; the root retains only public wrappers and exact binding re-exports. The OpenChestnut Presentation adapter imports the Presentation leaf directly instead of creating a root-entry back-edge. The OOXML module owns JSZip package loading, safety budgets, OPC paths/content types/relationships, recipes/source references, validation, and transactional regeneration behind four internal exports. Root PDF, Document, and all eight Presentation bindings are strict-identical leaf re-exports, and the cross-format ID allocator remains a single shared module. `src/index.mjs` is now 4,305 of 28,107 JavaScript source lines (15.3%). Only stateful Spreadsheet Worksheet/Range/Chart/Workbook behavior remains in the root and will be extracted at cohesive ownership boundaries. |
