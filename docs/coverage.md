...
| JavaScript source layering | partial | `src/index.mjs` is now a 42-line composition root: it preserves the 36-symbol public API, cross-format kind inference, verify/render/visual-QA/help wrappers, and exact binding re-exports while the complete PDF, Document, Presentation, and Spreadsheet model/layout/render/file-facade domains live in dependency-leaf modules. Visual QA receives artifact-kind inference through one explicit factory boundary, so it has no model back-edge or circular import. Both OpenChestnut Office adapters import their Presentation or Spreadsheet leaf directly instead of creating root-entry back-edges. The shared OOXML module owns JSZip package loading, safety budgets, OPC paths/content types/relationships, recipes/source references, validation, and transactional regeneration behind four internal exports. Root PDF, Document, all eight Presentation, and all four Spreadsheet bindings are strict-identical leaf re-exports, and the cross-format ID allocator remains a single shared module. The root is 42 of 28,109 JavaScript source lines (0.1%); future decomposition concerns the large but cohesive Spreadsheet leaf, not the public composition root. |
...
| Bibliography, advanced settings, complex fields, content controls, drawings | partial | Preserve imported content when source evidence remains valid; creation/editing is unsupported. Only modeled section/header behavior and `evenAndOddHeaders` are writable settings; `trackRevisions`, `updateFields`, `mirrorMargins`, and `documentProtection` fail closed. |
