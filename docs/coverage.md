...
| `PresentationFile.exportPptx` / `importPptx` | partial | Minimal PPTX round trip for text shapes works; table/chart facades export as visible PPTX text placeholders, and `dataUrl` image facades export as native `ppt/media/image*.ext` parts with slide `p:pic` XML and relationships. Needs true PPTX table/chart OOXML, themes, masters, layouts, notes, comments, connectors, and image import restoration. |
