...
description: "Control an open or active Microsoft Excel workbook through the spreadsheet add-in or connected session. Use when the user tags the Microsoft Excel app in agent or follows up on an established live Excel task. Do not use for standalone spreadsheet files or Google Sheets."
---
...
Keep user-facing language to "Microsoft Excel", "spreadsheet add-in for Excel", "open workbook", "connected Excel session", or "live Excel control"; avoid internal connector/backend names.

## Setup State Machine

Complete these gates in order. A later gate cannot prove that an earlier gate passed.

Checklist: Excel app open -> intended workbook active and unambiguous -> spreadsheet add-in installed -> pane open -> signed in -> connected-document tools available -> target workbook registered.
...
### 2. Determine Whether The Spreadsheet add-in Is Installed

Inspect the Home ribbon only after a workbook grid is active.

- A visible `the spreadsheet add-in` button on the ribbon is positive evidence that the add-in is installed.
- If the button is absent, open **Home > Add-ins** and look for **the spreadsheet add-in** published by **w31r4, LLC**. Do not infer that the add-in is missing only because its task pane is closed.
- If the spreadsheet add-in is not present in the ribbon or the installed add-ins list, treat the add-in as not installed.
...
If the add-in is installed but its pane is not visible, click **the spreadsheet add-in** on the Home ribbon. Allow a few seconds for the task pane to load, then inspect the pane again.

- A ribbon button without a visible task pane means installed but not open.
- A task pane titled **the spreadsheet add-in** means the add-in is open, but it does not by itself prove that the user is signed in.
...
### 4. Verify the spreadsheet add-in Sign-in
...
After Excel, the target workbook, the open add-in pane, and sign-in are all verified, check whether `list_document_sessions` is available in the current agent thread.

- If the tool is unavailable, do not report that the workbook failed to register. The current agent thread did not load the connected-document tool surface.
- Tell the user to confirm that the Spreadsheets plugin is installed or reinstalled, then start a new agent thread and retry the Microsoft Excel request. A thread does not necessarily acquire newly installed plugin tools after it has started.
...
- If sessions exist only for other workbooks, do not send commands to them. Activate the intended workbook, open its spreadsheet add-in pane, keep the pane visible, and retry discovery.
- If no Excel session exists, keep the target workbook active, select a cell in it, keep the signed-in spreadsheet add-in pane open, wait briefly, and retry once.
- If no session appears, close and reopen the spreadsheet add-in pane once, wait for the normal composer, and retry once.
- If the workbook still does not register, tell the user that Excel and the add-in are ready but agent cannot see a connected session. Ask the user to reopen the target workbook or restart Excel, then reopen the spreadsheet add-in and sign in if prompted. Do not loop indefinitely.
