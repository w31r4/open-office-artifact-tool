...
Prefer the direct `chart` tool and follow `charts.md` for chart-source, orientation, and visual verification. For live Excel, confirm chart objects with `list_items` and inspect the output with `read_range_image` when layout matters.
