...
- `assets/builtin_templates/grid-layout-library/`: Blob-managed static assets for the built-in Grid template, including 26 rendered previews, a model-facing registry, structured content tokens, and 26 exact plain-JavaScript artifact-tool Compose reconstructions with no JSX. This directory contains no Markdown, prompts, or reusable runners.
...
## Grid Artifact-Tool Compose Layout Reference
...
Use the chat mode supplied by agent. If the chat is not projectless, use the
project-backed layout.

Set:

- `SKILL_DIR=<absolute path to this skill>`
- `THREAD_ID=${ARTIFACT_THREAD_ID:-manual-<timestamp-or-short-random-suffix>}`
...
| Project-backed | `$SCRATCH_ROOT/artifact-presentations/$THREAD_ID/$TASK_SLUG` | User-requested path, repository convention, or `<project-root>/outputs/$TOPIC_SLUG.pptx` |
...
1. **User reference or template skill**: if the user supplies a reference deck,
   ...
   reference. Do not mix in Grid or another template.
2. **Explicit custom formatting**: if there is no reference and the user asks
   ...
   create the deck from scratch. Do not use Grid.
3. **No visual direction**: use the bundled Grid Artifact.md layout
   library as the composition reference. Select and adapt layouts using the
   Grid instructions above; do not run PPTX template-following mode.
...
`mcp__office_apps__google_drive_import_presentation`, with
...
`google-drive`. If the plugin is available but presentation
...
## App final response citations

Use the inline form `:file-citation{...}` and place each citation immediately after the claim it supports.
...
```text
:file-citation{path="/abs/path/deck.pptx" artifact_kind="presentation" slide_number="3"}
```

Include `slide_id` only when artifact-tool inspection provides the exact stable `sl/...` ID and stable navigation matters:

```text
:file-citation{path="/abs/path/deck.pptx" artifact_kind="presentation" slide_number="1" slide_id="sl/gs5z1kshq0xv"}
```
...
```text
:file-citation{path="/abs/path/deck.pptx" artifact_kind="presentation" slide_number="1" slide_id="sl/gs5z1kshq0xv" object_id="ch/pz9t1r3ka8vn" label="ARR by segment chart"}
```
