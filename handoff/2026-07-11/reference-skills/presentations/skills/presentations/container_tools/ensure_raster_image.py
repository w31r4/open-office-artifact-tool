...
- agent runtime Node + sharp: SVG/SVGZ rasterization
- agent runtime Poppler + pdf2image: PDF rasterization (first page)
- agent runtime Pillow: TIFF/JPEG XR output bridging
...
def ensure_raster_image(path: str, out_dir: str | None = None) -> str:
    ...
    if ext_lower in (".emf", ".wmf"):
        _unsupported_format(
            path, "No equivalent standalone EMF/WMF converter exists in the agent runtime bundle."
        )
    ...
    if ext_lower in (".eps", ".ps"):
        _unsupported_format(
            path, "No equivalent EPS/PS converter exists in the agent runtime bundle."
        )
