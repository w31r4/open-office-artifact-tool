...
OpenChestnut now owns one bounded classic-comment profile through `DocxClassicCommentCodec`: an exact whole top-level paragraph anchor (`w:commentRangeStart`/`w:commentRangeEnd`/`w:commentReference`) and one CommentsPart paragraph/run/text body. Source-free export authors the package relationship and matching native IDs. Import keeps the native decimal ID, body ordinal, and comment/semantic/residual/anchor hashes in source-only wire evidence; source-bound export may update author, optional initials/date, and text while target/count/order/anchor topology remain immutable. Rich bodies, partial/cross-paragraph anchors, replies, resolved state, durable identities, and people/extended parts remain opaque and fail closed. When no modeled comment edit is requested, the editable package never materializes an unsupported CommentsPart, preventing Open XML SDK AutoSave from normalizing opaque bytes.
