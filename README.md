...
OpenChestnut also owns a bounded worksheet-picture path. `sheet.images.add(...)` may author an embedded base64 PNG/JPEG with `{ from, extent }` one-cell geometry, `{ type: "twoCell", from, to, editAs? }` two-cell geometry, or `{ type: "absolute", position: { leftPx, topPx }, extent }` page-relative geometry. `editAs` may be `twoCell`, `oneCell`, or `absolute`, and omission remains distinct from an explicit value. Absolute positions retain signed native EMUs, so off-canvas origins roundtrip without pixel drift. The JavaScript adapter derives a content-addressed asset ID, while `XlsxDrawingCodec` owns the worksheet Drawing relationship, DrawingPart, picture relationship, and ImagePart through the public Open XML SDK. Import restores the existing image facade and native-precision EMU geometry. A recognized source-bound picture may edit its name, alt text, same-kind anchor, and embedded bytes without flattening crop/lock or other residual picture XML. Byte replacement retains the existing ImagePart path, requires the same PNG/JPEG content type, and fails closed when another blip shares that part. Imported add/remove, anchor-kind conversion, cross-format or shared-part byte replacement, external links, crop/effect edits, charts, groups, and shapes remain fail-closed or opaque. Relationship IDs and part paths are source locators, not durable image identity.
