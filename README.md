...
Worksheet visibility and active selection are shared by the JavaScript and OpenChestnut XLSX paths. Create or update sheets with `visible`, `hidden`, or `veryHidden`, then call `workbook.worksheets.setActiveWorksheet(nameOrIndexOrWorksheet)` to choose the visible sheet opened by default and used by APIs that omit a sheet. Inspect/layout expose the selection; verification/export require at least one visible sheet and prevent hiding an explicitly active sheet until another is selected. Source-free output writes the selected sheet's zero-based position to `workbookView activeTab` (falling back to the first visible sheet when no selection was made). Imported OpenChestnut workbooks bind each `<sheet>` and one recognized primary `<workbookView>` with source hashes, edit only `activeTab`, and preserve the view's window geometry, first-sheet scroll position, visibility, extensions, and other residual XML. Absent, invalid, or multi-window view profiles remain opaque; their graph cannot be fabricated or replaced, and visibility edits cannot hide a source-active tab. Grouped `sheetView tabSelected` state and multi-window editing are not yet modeled.
