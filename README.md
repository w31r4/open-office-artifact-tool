...
qpdf、Python 专项工具、OCR、veraPDF/JRE 等由显式的 `open-office-artifact-tool/pdf/providers` 路由：先根据任务和检查结果返回 `ready`、`installable` 或 `blocked`，再由项目策略决定是否可安装。默认策略禁止下载；qpdf、Python foundation、Python specialists、veraPDF/JRE `1.30.2-oat.1`、OCR core `17.8.1-oat.1` 以及英语/简体中文语言包 `4.1.0-oat.1` 都已有双平台、哈希固定且带 SBOM、许可证与 attestation 的能力包。foundation 只提供隔离 CPython 上的 ReportLab、pdfplumber、pypdf 与 Pillow；specialists 提供 PyMuPDF、pikepdf、pyHanko 与证书校验器，依赖 qpdf 并需要显式接受 AGPL 或商业许可；veraPDF pack 自带隔离 JRE；OCR core 携带隔离 OCRmyPDF、Tesseract 5、Ghostscript 与 `pdftotext`，语言包仍须逐个获准。Poppler QA pack 仍会明确 `blocked`，不会偷偷降级到其它工具。已由部署方管理的运行时可使用显式 `system-only` 策略。详见 [PDF Provider Setup](skills/pdf/skills/pdf/tasks/provider_setup.md)。
...
| PPTX | OpenChestnut C# WASM | 形状与富文本、图片及可逆裁剪、表格、连接线、图表、直接背景、纯文本及固定拓扑富文本演讲者备注、经典评论和有界 Office 2021 现代评论线程；Master/Layout 仅保真、不可编辑。 |
