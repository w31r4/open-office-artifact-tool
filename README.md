...
The experimental OpenChestnut subpath authors a bounded workbook feature set directly, including native shared-formula groups and legacy array-formula anchors. Shared formulas cross the public wire as expanded per-cell formulas plus explicit group index/range metadata; malformed, incomplete, overlapping, data-table, or otherwise unsupported topology fails before package mutation. For imported XLSX files, OpenChestnut also carries a budget-checked, hash-bound source-package snapshot and updates modeled workbook/sheet/cell fields in place, so unmodeled styles, tables, pivots, drawings, comments, and arbitrary legal OPC targets survive the second export byte-for-byte at the opaque-part boundary. Cached-value and number-format-only edits retain the original formula XML exactly. Writing a new formula into one shared member deliberately detaches the complete group to ordinary formulas instead of leaving a broken native group. New advanced workbooks still use the JavaScript codec until their semantics enter the public schema. Consumption uses only bundled runtime assets; a local .NET SDK is not required:
