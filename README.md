...
OpenChestnut also has separate bounded worksheet-chart, series-style, and primary-axis codecs. `sheet.charts.add("bar"|"line"|"pie", ...)` authors a relationship-backed native ChartPart with title, legend, shared categories, ordered numeric caches, optional category/value formulas, and optional per-series `fill: "#RRGGBB"` solid colors. Bar and line charts expose `xAxis` text-category title/number format/tick-label interval plus `yAxis` linear value title/number format/min/max/major unit; `yAxis.tickLabelInterval` remains a compatibility alias for `majorUnit`. `WorksheetArtifact.charts` additive field 14 carries those semantics plus one native-EMU anchor and a hash-bound DrawingPart/ChartPart source locator, chart fields 12/13 carry independently versioned axis messages, and series field 5 reuses the public color message. A recognized import may add, remove, or change an owned RGB series fill and edit name, title, legend, formulas, categories, cached values, and bounded axis fields while anchor, chart type, series count, point count, literal/reference topology, and primary-axis identity stay fixed. Other series shape properties and unknown ChartSpace XML survive those edits. Theme/gradient/no-fill or otherwise unrecognized series-fill graphs, formula-referenced series names, and logarithmic/date/secondary/multi-axis profiles are read-only; external data, combo/multi-plot charts, line/marker styles, data labels, chart add/remove, and anchor/topology changes remain opaque or fail closed. `XlsxDrawingCodec` and `XlsxChartCodec` share one pre-mutation DrawingPart hash, so picture and chart edits can safely occur in the same source-preserving export.
