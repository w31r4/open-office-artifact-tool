...
OpenChestnut's public opaque-OPC contract is loss-aware rather than a second copy of every ZIP entry. Each unmodeled imported part exposes its normalized package path, exact resolved content type, SHA-256, and unowned outgoing relationships; the raw bytes live once in the bounded, hash-bound source-package snapshot. Import parses `[Content_Types].xml` with the OPC namespace, rejects duplicate defaults/overrides and OPC parts without a matching declaration, and binds content-type plus per-part relationship metadata into source-preserving export validation. DOCX, XLSX, and PPTX therefore share one fail-closed package boundary while their semantic codecs continue to migrate independently.

## Current status

OpenChestnut now projects unsupported PPTX drawing objects as bounded source-bound graphs instead of anonymous XML only. The public wire classifies `contentPart`, OLE, SmartArt/diagram, picture, group, connector, and generic graphic-frame elements; records namespace-qualified relationship attributes and the transitive reachable part paths; and keeps content types, relationship metadata, hashes, and package bytes single-owned by `opaque_opc`. The JavaScript adapter materializes those paths into ordinary `nativeObject` facades, so `inspect` exposes safe relationship/part metadata and `resolve` returns the facade. Recognized top-level OLE, SmartArt/diagram, and `contentPart` groups may change only their display name and complete outer frame through `setName(...)`/`setPosition(...)`; nested coordinates, raw XML, relationships, native payload bytes, and graph topology remain immutable. Both OpenChestnut and JavaScript exports preserve that graph, while generic profiles, missing relationships/parts, cycles, traversal budgets, unsafe paths, hash mismatches, and graph tampering fail closed.

One deliberately narrower exception exists for an eligible top-level OLE object that owns exactly one non-shared internal package relationship to an ordinary XLSX part. `nativeObject.getEmbeddedWorkbook()` returns a defensive workbook `FileBlob`, and `nativeObject.replaceEmbeddedWorkbook(...)` replaces only those validated XLSX bytes. The part path, content type, relationship ID, raw OLE XML, preview image, and every other recursive OPC node stay source-bound; the retained preview may therefore be stale until an Office host regenerates it. OLE creation, arbitrary part replacement, non-XLSX payloads, and shared embedded packages remain unsupported.
...
The experimental OpenChestnut DOCX path additionally treats bounded whole-paragraph hyperlinks, simple fields, table cell text, and direct or paragraph/numbering-style-linked numbered single-run paragraph text as editable content. Its C# Open XML SDK codec can change hyperlink text, tooltip, history, and external/internal target mode while changing only relationships owned by that hyperlink. It can edit one-run `w:fldSimple` instructions/results for a bounded non-fetching catalog such as `PAGE`, `NUMPAGES`, dates, document properties, and counts. `DocxTableGeometry` maps physical cells onto `w:tblGrid`, exposes horizontal `w:gridSpan`, vertical `w:vMerge` restart/continuation state, effective row spans, row offsets, and per-cell editability through additive public wire fields. Source-free `document.addTable({ gridColumns, cells, values, widthDxa, indentDxa, columnWidthsDxa, cellMarginsDxa, borderColor, borderSize, headerFill })` can author complete physical-cell geometry plus a fixed-layout direct-formatting profile. `DocxTableFormatting` emits exact dxa table/grid/cell widths, four logical cell margins, six uniform RGB borders, and first-row fill/bold formatting; logical column widths must sum to the table width, merged-cell widths are derived from the covered grid columns, and unsupported theme/conditional/per-cell formatting fails closed. OpenChestnut emits the default `TableGrid` style definition and rejects unmodeled custom style graphs. Imported geometry and direct formatting remain source-bound. `DocxTableCodec` may edit simple unmerged, horizontally merged, or vertical-origin cells containing exactly one paragraph/run/text node while retaining merge topology and all table/row/cell/paragraph/run properties behind a residual hash. Vertical continuations are read-only; incomplete/mismatched merge chains and nested, multi-paragraph, or multi-run tables fail closed. `DocxNumberedParagraphCodec` resolves the paragraph assignment while the dedicated `DocxNumberingResolver` follows the source `w:lvl/w:pStyle`, `w:styleLink`, and `w:numStyleLink` graph through cached numbering/styles XML. In accordance with WordprocessingML, an `ilvl` inside a paragraph style is ignored; the associated numbering level comes from the level's `w:pStyle`, while direct paragraph `ilvl` remains authoritative. `DocxNumberingEditPlanner` adds a bounded definition-edit slice: every editable top-level paragraph directly assigned to one `numId`/level must request the same `numberFormat`, `start`, and `levelText`; the codec writes an instance-local `w:lvlOverride` and leaves the shared `w:abstractNum` untouched. Numbering identity, level, paragraph style, and style linkage remain source-bound. Partial groups, nested uses, references from another package part, incompatible existing overrides, and `styleLink`/`numStyleLink` graphs fail closed. Missing, duplicate, cyclic, over-deep, ambiguous, or otherwise unresolved definition graphs remain source-preserved but not editable. Unsupported structures fail closed instead of being flattened or executed.

For imported direct table formatting, “source-bound” means the codec must first recognize the complete profile and retains its original topology; it no longer means every modeled value is immutable. Recognized width, indent, grid/cell widths, margins, uniform borders, and header fill can be changed in place. The residual hash canonicalizes exactly those fields while continuing to guard table styles, row properties, paragraph/run properties, and other unknown XML. Missing, partial, theme, conditional, per-cell, and style-effective formatting still rejects replacement.

OpenChestnut now owns one bounded classic-comment profile through `DocxClassicCommentCodec`: an exact whole top-level paragraph anchor (`w:commentRangeStart`/`w:commentRangeEnd`/`w:commentReference`) and one CommentsPart paragraph/run/text body. Source-free export authors the package relationship and matching native IDs. Import keeps the native decimal ID, body ordinal, per-comment/semantic hashes, whole-comments-part residual hash, and anchor hash in source-only wire evidence; source-bound export may update author, optional initials/date, and text while target/count/order/anchor topology and root-level residual XML remain immutable. A derived public initials value does not materialize a natively absent `w:initials` during an unrelated edit. Rich bodies, partial/cross-paragraph anchors, replies, resolved state, durable identities, and people/extended parts remain opaque and fail closed. When no modeled comment edit is requested, the editable package never materializes an unsupported CommentsPart, preventing Open XML SDK AutoSave from normalizing opaque bytes.

For source-free DOCX, `DocxDirectNumbering` now preflights the whole public list graph and authors direct text-marker `w:abstractNum`/`w:lvl`/`w:num` definitions through Open XML SDK. Items may share a numbering instance and multiple levels or explicitly share one compatible abstract definition; omitted package-local IDs are allocated by the JavaScript adapter. Conflicting definitions fail before package mutation. Picture-bullet assets and style-linked numbering graphs still use the JavaScript codec until their complete style/asset semantics enter the public wire schema.
...
Workbook calculation policy is part of both XLSX codec paths. `Workbook.create({ calculation })` or `workbook.setCalculation(...)` accepts `automatic`, `automaticExceptTables`, or `manual` mode; presence-aware calculate-on-save/full-on-load/force-full/full-precision flags; and optional iterative limits. OpenChestnut writes these as native `workbook.xml` `calcPr` attributes and can edit a recognized imported element without erasing its application `calcId`. R1C1, concurrent, completed-state, extended, or otherwise unsupported profiles remain source-preserved and fail closed; this policy asks a host to recalculate but does not execute Excel.

Worksheet visibility, active selection, grouped tab selection, and multiple workbook windows are shared by the JavaScript and OpenChestnut XLSX paths. Legacy `workbook.worksheets.setActiveWorksheet(...)`, `setSelectedWorksheets([...])`, and `getSelectedWorksheets()` operate on primary window 0. `workbook.windows.add({ activeWorksheet, selectedWorksheets })` appends an independent window; every non-empty visible tab group must include that window's active sheet. Inspect exposes the ordered windows, verification/export reject hidden active/selected tabs in any window, and source-free output coordinates each zero-based `workbookView@activeTab` with one worksheet `sheetView@tabSelected` for the matching `workbookViewId`. Public protobuf field 8 remains the primary window and additive field 9 carries only later windows, so primary state has one source of truth. Imported OpenChestnut workbooks become editable only when every window has a complete one-per-worksheet `sheetView` graph; source-bound edits keep window count/order fixed and change only the corresponding `activeTab`/`tabSelected` attributes while preserving window geometry, panes, zoom, first-sheet scroll position, extensions, and residual XML. Missing or incomplete single-window selection graphs remain readable but fail closed on selection changes; incomplete multi-window graphs remain fully opaque.

OpenChestnut also owns a bounded worksheet-picture path. `sheet.images.add(...)` may author an embedded base64 PNG/JPEG with `{ from, extent }` one-cell geometry, `{ type: "twoCell", from, to, editAs? }` two-cell geometry, or `{ type: "absolute", position: { leftPx, topPx }, extent }` page-relative geometry. `editAs` may be `twoCell`, `oneCell`, or `absolute`, and omission remains distinct from an explicit value. Absolute positions retain signed native EMUs, so off-canvas origins roundtrip without pixel drift. An optional `crop: { leftPercent, topPercent, rightPercent, bottomPercent }` maps to DrawingML `a:srcRect`: every signed edge is bounded to -100 through 100 percent, positive values inset, negative values outset, and each opposing sum must leave a positive source rectangle. Optional `effects: { grayscale, brightnessPercent, contrastPercent, opacityPercent }` maps to the bounded `a:grayscl`, `a:lum`, and `a:alphaModFix` blip effects; brightness/contrast are -100 through 100 percent and opacity is 0 through 100. Optional `transform: { rotationDegrees, flipHorizontal, flipVertical }` maps to presence-aware `a:xfrm` attributes; rotation is bounded to -360 through 360 degrees at native 1/60000-degree precision, while explicit false flips remain distinct from omission. The JavaScript SVG preview approximates those visual transforms, while native Office renderers remain authoritative. The JavaScript adapter derives a content-addressed asset ID, while `XlsxDrawingCodec` owns the worksheet Drawing relationship, DrawingPart, picture relationship, and ImagePart through the public Open XML SDK. Import restores the existing image facade, native-precision EMU geometry, recognized crop/effects/transform semantics. A recognized source-bound picture may edit its name, alt text, same-kind anchor, crop, effects, transform, and embedded bytes without flattening picture locks, blip extensions, transform offsets/extents, or other residual XML. Byte replacement retains the existing ImagePart path, requires the same PNG/JPEG content type, and fails closed when another blip shares that part. Imported add/remove, anchor-kind conversion, cross-format or shared-part byte replacement, external links, advanced/duplicate picture-effect graphs, charts, groups, and shapes remain fail-closed or opaque; out-of-profile native crop/effect/transform graphs are preserved but cannot be replaced through the bounded model. Relationship IDs and part paths are source locators, not durable image identity.

The OpenChestnut codec authors a bounded workbook feature set directly, including ordinary workbook- and worksheet-scoped range defined names with optional comment/hidden state, native shared-formula groups, legacy array-formula anchors, XLDAPR-marked dynamic-array anchors, worksheet-level row/column sort state, embedded one-cell/two-cell PNG/JPEG pictures, and worksheet tables with calculated-column, totals-row, AutoFilter, and sort metadata. Shared formulas cross the public wire as expanded per-cell formulas plus explicit group index/range metadata; dynamic arrays reuse that formula-metadata field with an independent kind and spill reference. Source-free dynamic arrays receive a canonical workbook cell-metadata graph, while recognized imported graphs are retained across safe edits; adding into an opaque or absent source graph and exporting a blocked spill fail closed. Malformed, incomplete, overlapping, data-table, or otherwise unsupported topology fails before package mutation. A bounded table owns its name/range, header/totals/filter choices, rich column definitions, style name, and first/last/row/column stripe toggles while cell values and formulas remain in the worksheet. Rich columns add leading-`=` calculated formulas, built-in/custom totals functions, labels, totals formulas, and array flags while retaining `columnNames` as a compatibility projection. Table filters model exact values/blanks, calendar-aware grouped dates, one/two comparison criteria, SpreadsheetML dynamic types with optional calculated thresholds, top/bottom item-or-percent filters, the 17 standard main-namespace icon sets, and stable cell-fill or font-color selectors, keyed by zero-based table column index. Exact values and date groups intentionally cannot coexist in one native `<filters>` branch. Color selectors use `{ kind: "color", target: "cell"|"font", color }`; `dxfId` stays an internal package address. `sortState` models a contained sort range, case sensitivity, optional SpreadsheetML `sortMethod` (`none`, `pinYin`, or `stroke`), and ordered ascending/descending value, icon, or color conditions; value conditions may carry a bounded native `customList`, and optional icon IDs preserve the native “no icon” choice. Ordinary row sorts use unique single-column conditions. Worksheet-level and QueryTable refresh sorts may set presence-aware `columnSort: true` and use unique single-row conditions for left-to-right sorting; table AutoFilter sorts reject the attribute because SpreadsheetML limits it to non-AutoFilter ranges. For imported XLSX files, OpenChestnut also carries a budget-checked, hash-bound source-package snapshot and updates modeled workbook/sheet/cell/table/image fields in place. Recognized table edits retain the original TableDefinitionPart path and worksheet relationship; color edits append/deduplicate an exact color-only differential format rather than mutating an existing shared entry. Recognized defined names retain their ordinal and source hashes, may edit bounded name/range/scope/comment/hidden fields in place, and cannot be added, removed, reordered, or collided with hidden names. Formula/constant, reserved built-in, macro, and extended defined-name profiles remain opaque and exact-preserved. An existing one-to-one QueryTable relationship can expose `table.queryTable` root semantics and a recognized `queryTableRefresh`: presence-aware refresh metadata, field names and bounded flags, source deleted-field names, and a refresh-local sort state with the same locale/custom-list/value/icon/color/direction vocabulary. That path is source-bound only: the connection ID must exist in the hash-bound ConnectionsPart; query fields retain immutable IDs/order/count/table-column bindings, deleted-field count/order stays fixed, and sort presence/condition count stays fixed. Safe names, flags, sort range/case/method/direction/selectors can change in place while extensions, relationships, and unknown nested profiles stay preserved. A separately source-bound `workbook.connections` projection exposes only database/type-5 root metadata and refresh policy: name, optional description, keep-alive, bounded interval, background, refresh-on-load, and save-data. Connection ID/type/refreshed-version, count/order, provider strings, commands, credentials, source paths, child graphs, unknown attributes, and extensions remain immutable or hidden; unsupported connection types remain opaque and exact-preserved. Graph add/remove, field/deleted-field/sort topology changes, connection creation/execution, and source-free QueryTable or connection authoring are rejected. The JavaScript XLSX fallback can inspect the same bounded projection but refuses to export it because it cannot preserve the graph. Multi-property differential styles, opaque refresh profiles/geometries, unsupported QueryTable companion relationships, pivots, non-picture drawings, comments, other unmodeled objects, and arbitrary legal OPC targets survive the second export behind the same opaque-part boundary. Cached-value and number-format-only edits retain the original formula XML exactly. Writing a new formula into a native group detaches stale topology before recalculation and export. New advanced workbooks still use the JavaScript codec until their semantics enter the public schema. Consumption uses only bundled runtime assets; a local .NET SDK is not required:

OpenChestnut also has separate bounded worksheet-chart, series-fill, series-line, series-marker, chart-line-options, text-style, and primary-axis codecs. `sheet.charts.add("bar"|"line"|"pie", ...)` authors a relationship-backed native ChartPart with title, legend, shared categories, ordered numeric caches, optional category/value formulas, optional per-series `fill: "#RRGGBB"` solid colors, bounded `line: { fill, style, width }` direct lines, line-only `marker: { symbol, size, fill, line }`, line-only chart-level `lineOptions: { grouping?, smooth?, varyColors? }`, and optional `{ fontSize }` title/tick-label styles. Grouping is `standard`, `stacked`, or `percentStacked`; omission authors the native `standard` default, and deterministic SVG previews stack or normalize every line series accordingly. `smooth` preserves explicit true/false presence, while `varyColors: true` authors direct `<c:varyColors val="1"/>` and false or omission removes that optional node. `stroke: { color, style, weight }` is a compatibility alias for the series `line`; the five line styles are `solid`, `dashed`, `dotted`, `dash-dot`, and `dash-dot-dot`, and width is 0 through 1,584 points. Marker symbols are `none`, `dot`, `circle`, `square`, `diamond`, `triangle`, `x`, `star`, `plus`, or `dash`; size is an integer from 2 through 72, marker `fill` is a direct `#RRGGBB` color, and marker `line` reuses the bounded fill/style/width outline shape. Bar and line charts expose `xAxis` text-category title/number format/tick-label interval plus `yAxis` linear value title/number format/min/max/major unit; `yAxis.tickLabelInterval` remains a compatibility alias for `majorUnit`. Chart field 15 carries line options, chart field 14 carries `titleTextStyle`, axis field 7 carries tick-label `textStyle`, chart fields 12/13 carry independently versioned axis messages, series fields 5–7 carry fill/direct-line/marker, and marker fields 3/4 carry its RGB fill/bounded outline. A recognized import may add, remove, or change an owned RGB series fill, direct line, complete bounded direct marker, chart-level grouping/smooth/vary-colors option, or bounded font size and edit name, title, legend, formulas, categories, cached values, and bounded axis fields while anchor, chart type, series count, point count, literal/reference topology, and primary-axis identity stay fixed. Other series shape properties and unknown ChartSpace XML survive those edits. Theme/gradient/no-fill or otherwise unrecognized series-fill graphs, theme/transformed/compound/cap/join/arrow/custom-dash line graphs, theme/transformed/gradient/picture marker graphs, marker extensions, complex grouping/smooth/vary-colors nodes, rich title runs, complex/conflicting axis `txPr`, formula-referenced series names, and logarithmic/date/secondary/multi-axis profiles are read-only and exact-preserved; external data, combo/multi-plot charts, axis-title styles, data labels, chart add/remove, and anchor/topology changes remain opaque or fail closed. `XlsxDrawingCodec` and `XlsxChartCodec` share one pre-mutation DrawingPart hash, so picture and chart edits can safely occur in the same source-preserving export.

The three Office file facades share one explicit codec policy. `OFFICE_CODEC_IDS` is the frozen list `['javascript', 'open-chestnut']`. Omitting `options.codec` keeps the JavaScript codec as the compatibility and migration-oracle default; pass `{ codec: 'open-chestnut' }` to lazily start the bundled WebAssembly runtime. Import and any source-bound second export must use the same codec. Invalid IDs fail closed, and the old `openxml-wasm` name is accepted only by its deprecated compatibility subpath. The direct `codecs/open-chestnut` helpers remain available for advanced byte-boundary tooling, but normal application and agent code should use the file facades.
...
summary.getRange("A1:B2").values = [["Metric", "Value"], ["Revenue", 42.5]];
summary.tables.add({ range: "A1:B2", name: "MetricsTable", style: "TableStyleMedium4" });
const details = workbook.worksheets.add("Details");
workbook.worksheets.setActiveWorksheet(details);
workbook.worksheets.setSelectedWorksheets([summary, details]);
const xlsx = await SpreadsheetFile.exportXlsx(workbook, { codec: "open-chestnut" });
const roundtripped = await SpreadsheetFile.importXlsx(xlsx, { codec: "open-chestnut" });
```

The same facade policy selects the bounded DOCX semantic slice. Imported advanced blocks are hash-bound to the original package: unchanged content is preserved, while edits to not-yet-modeled structures fail explicitly.
...
document.addListItem("Direct OpenChestnut numbering", { listType: "number", numberFormat: "upperLetter", numberingId: 7 });
...
PPTX uses the same loss-aware boundary. A simple deck can be authored directly; importing an advanced deck carries hash-bound master/layout/slide/shape bindings so unsupported native objects survive safe modeled edits. The WebAssembly slice exposes every Slide Master and Slide Layout as a stable package locator, preserves the Slide → Layout → Master chain, and permits bounded title/body/other `p:txStyles`, direct Master/Layout solid or theme-reference background add/replace/remove operations, owner-local Master/Layout placeholder text/body/list/link edits, and add/remove/move/resize/rotate/flip edits for a recognized placeholder transform slot. A presence change is allowed only when `p:spPr` has no transform or has one fully owned `a:xfrm` with exactly one `a:off` and `a:ext`; unknown attributes, extra children, and out-of-range transforms stay source-bound. `placeholder.position = undefined` removes that recognized direct frame without flattening inheritance. `placeholder.transform` uses optional `rotationDegrees`, `flipHorizontal`, and `flipVertical`; native attribute absence stays distinct from explicit zero/false, and assigning the transform itself to `undefined` removes only those bounded attributes. On an imported slide, a `p:ph` without owner-local `a:xfrm` resolves its effective position and `shape.transform` from the linked layout placeholder with the same unsigned `idx`; layout resolution may in turn inherit the complete atomic frame from the master. The shape reports `placeholder.geometrySource` as `slide`, `layout`, `master`, or `unresolved`. Those effective position/rotation/flip values are a read-only projection: unchanged export reuses the exact source wire and never writes the derived frame into slide XML, while mutation and source-free slide-placeholder authoring fail closed. For ordinary non-placeholder rectangle/ellipse Shapes, both JavaScript and OpenChestnut author/import `shape.transform`, preserve explicit false, and allow recognized source-bound rotation/flip attributes to be changed or cleared; unknown attributes, duplicate/extra transform children, and out-of-range rotation keep the whole Shape read-only. Inspect, layout, and SVG preview consume the same model transform. Top-level embedded rectangular pictures have a bounded name/alt/frame/direct-transform/same-content-type-byte profile. Fixed rectangular tables accept one plain-text run per cell; source-bound edits may change name, complete frame, and existing cell strings while topology, row/column proportions, style flags, merges, rich text, and residual formatting remain guarded. `background` reports only direct `p:bg`; `effectiveBackground()` resolves the theme/master inheritance fallback. Fill, shape style, identity/topology changes, and slide rebinding remain read-only or fail closed.
...
const pptx = await PresentationFile.exportPptx(deck, { codec: "open-chestnut" });
const importedDeck = await PresentationFile.importPptx(pptx, { codec: "open-chestnut" });
```
...
The current XLSX slice roundtrips complete static `range.format` profiles, the workbook's 12-slot DrawingML color scheme, native shared/legacy/dynamic-array formula topology, bounded worksheet row/column sort state and tables, and source-bound database connection metadata through additive public wire fields and the bundled C# Open XML SDK codec. Static profiles cover font, pattern fill, all nine border edges, alignment, protection, and built-in/custom number formats with RGB, theme, indexed, or automatic colors plus tint. `XlsxThemeCodec` creates a complete source-free theme and can update its name and twelve RGB slots in place; recognized source themes retain their font/format schemes and other residual XML, and system-color XML remains intact while its slot is unchanged. Unsupported HSL/preset/scheme color profiles are preserved unchanged and reject lossy replacement. Source-free export creates a valid minimal stylesheet; source-bound edits clone the current XF and referenced resources before changing owned fields, leaving unmodeled style content and formula XML intact. `XlsxFormulaCodec` validates complete shared groups, expands follower formulas using bounded A1 translation, and emits master/follower `t/si/ref` records. `XlsxDynamicArrayCodec` owns the workbook cell-metadata part, creates a canonical XLDAPR record, maps one-based `c@cm` references, and preserves recognized source metadata without flattening opaque graphs. Shared `XlsxSortStateCodec` owns direction-aware parsing, validation, authoring, patching, selectors, and semantic hashing across worksheet, Table, and QueryTable contexts. `WorksheetArtifact.sort_state` field 10 and `SpreadsheetTableSortStateArtifact.column_sort` field 5 preserve explicit direction; worksheet and QueryTable refresh column sorts require unique single-row conditions, while table AutoFilter sorts remain row-oriented. `XlsxTableCodec` owns the worksheet `tableParts` locator and its TableDefinitionPart relationship: it authors tables, edits bounded calculated-column/totals-row metadata plus exact/grouped-date/custom/dynamic/Top10/icon/color filters and multi-condition custom-list/value/icon/color row sorts with optional `none`/`pinYin`/`stroke` methods without changing package identity, and delegates a recognized source QueryTable relationship to `XlsxQueryTableCodec`. The query module edits presence-aware root, recognized refresh/field semantics, fixed-topology deleted-field history, and fixed-topology refresh-local row/column sorts against an existing connection ID. Shared workbook-level `XlsxConnectionCodec` parses and writes the ConnectionsPart exactly once, exposing only hash-bound database/type-5 root metadata while leaving provider/command/credential/source-path/child/extension content hidden and preserved. Connection count/order and ID/type/version are immutable; source-free authoring and opaque-profile replacement fail closed. `XlsxQuerySortStateCodec` patches owned sort attributes in place and shares the stable color model/differential-format allocator without exposing `dxfId`. Invalid nested history/sort geometry stays independently opaque, so safe root/field edits do not erase residual XML. `XlsxCellStyleCodec` owns differential-format allocation, recognizes only color-only records, deduplicates authored resources, and appends source-bound replacements while preserving the original shared record order and XML. Imported table add/remove, QueryTable graph or field/deleted-field/sort topology/source-free creation, multi-property differential styles, and unsupported extension graphs remain fail-closed. Gradient fills, HSL/preset/scheme theme color choices, theme font/format-scheme editing, named/cell style inheritance, data-table formulas, and non-XLDAPR or multi-record dynamic-array metadata graphs remain fail-closed on this path; the JavaScript XLSX codec is still the explicit fallback for broader features that do not require QueryTable graph preservation.
