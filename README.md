...
The experimental OpenXML-WASM DOCX path additionally treats bounded whole-paragraph hyperlinks as source-bound editable content. Its C# Open XML SDK codec can change text, tooltip, history, and external/internal target mode while retaining native run/paragraph formatting behind a residual hash and changing only the relationship IDs owned by that hyperlink. Mixed-content and multi-run hyperlink paragraphs remain read-only instead of being flattened.
