...
The bundled codec also has bounded DOCX and PPTX slices. The complex Documents fixture crosses DOCX with source-preserving advanced WordprocessingML and real page-render QA. PPTX models top-level rectangle/ellipse shapes plus a bounded paragraph subset: ordered text, field, and styled line-break inlines; paragraph tab stops; level/alignment; character/auto-number/no-bullet markers; direct marker font, solid RGB marker color, fixed/percentage/follow-text marker size; bold/italic, font size/family, solid RGB text color; and bounded click links. Source-bound edits retain picture bullets, non-RGB or transformed marker colors, unknown field paragraph properties, unknown break run properties, and other unmodeled DrawingML when paragraph/inline kinds are unchanged; attempts to replace an unknown choice or change text/field/break topology fail closed. Pictures, charts, groups, connectors, content parts, notes, masters, themes, and their relationship graphs remain read-only and preserved until their editable semantics land. Direct authoring outside each modeled subset fails closed.
