...
PPTX uses the same loss-aware boundary. A simple deck can be authored directly; importing an advanced deck carries hash-bound master/layout/slide/shape bindings so unsupported native objects survive safe modeled edits. The WebAssembly slice exposes every Slide Master and Slide Layout as a stable package locator, preserves the Slide → Layout → Master chain, and permits bounded title/body/other `p:txStyles`, direct Master/Layout solid or theme-reference background add/replace/remove operations, owner-local Master/Layout placeholder text/body/list/link edits, and move/resize/rotate/flip edits for an already-present direct placeholder `a:xfrm` with exactly one `a:off` and `a:ext`. `placeholder.transform` uses optional `rotationDegrees`, `flipHorizontal`, and `flipVertical`; native attribute absence stays distinct from explicit zero/false, and assigning `undefined` removes the bounded attributes without removing the frame. `background` reports only direct `p:bg`; `effectiveBackground()` resolves the theme/master inheritance fallback. Missing placeholder transforms remain inherited, while adding/removing frames, complex or out-of-range transforms, fill, shape style, identity/topology changes, and slide rebinding remain read-only or fail closed.
