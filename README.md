...
OpenChestnut also has separate bounded worksheet-chart, series-fill, series-line, text-style, and primary-axis codecs. `sheet.charts.add("bar"|"line"|"pie", ...)` authors a relationship-backed native ChartPart with title, legend, shared categories, ordered numeric caches, optional category/value formulas, optional per-series `fill: "#RRGGBB"` solid colors, bounded `line: { fill, style, width }` direct lines, and optional `{ fontSize }` title/tick-label styles. `stroke: { color, style, weight }` is a compatibility alias for `line`; the five line styles are `solid`, `dashed`, `dotted`, `dash-dot`, and `dash-dot-dot`, and width is 0 through 1,584 points. Bar and line charts expose `xAxis` text-category title/number format/tick-label interval plus `yAxis` linear value title/number format/min/max/major unit; `yAxis.tickLabelInterval` remains a compatibility alias for `majorUnit`. Chart field 14 carries `titleTextStyle`, axis field 7 carries tick-label `textStyle`, and their shared public message owns only `fontSize` from 1 through 4,000 points; it does not style axis titles. `WorksheetArtifact.charts` field 14 still carries the chart collection, chart fields 12/13 carry independently versioned axis messages, series field 5 reuses the public color message, and additive series field 6 carries `SpreadsheetChartLineStyleArtifact`. A recognized import may add, remove, or change an owned RGB series fill, direct line, or bounded font size and edit name, title, legend, formulas, categories, cached values, and bounded axis fields while anchor, chart type, series count, point count, literal/reference topology, and primary-axis identity stay fixed. Other series shape properties and unknown ChartSpace XML survive those edits. Theme/gradient/no-fill or otherwise unrecognized series-fill graphs, theme/transformed/compound/cap/join/arrow/custom-dash line graphs, rich title runs, complex/conflicting axis `txPr`, formula-referenced series names, and logarithmic/date/secondary/multi-axis profiles are read-only and exact-preserved; external data, combo/multi-plot charts, marker styles, axis-title styles, data labels, chart add/remove, and anchor/topology changes remain opaque or fail closed. `XlsxDrawingCodec` and `XlsxChartCodec` share one pre-mutation DrawingPart hash, so picture and chart edits can safely occur in the same source-preserving export.
