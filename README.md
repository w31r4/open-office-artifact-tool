...
The experimental OpenChestnut subpath authors a bounded workbook feature set directly, including native shared-formula groups, legacy array-formula anchors, and simple worksheet tables. Shared formulas cross the public wire as expanded per-cell formulas plus explicit group index/range metadata; malformed, incomplete, overlapping, data-table, or otherwise unsupported topology fails before package mutation. A bounded table owns its name/range, header/totals/filter choices, column names, style name, and first/last/row/column stripe toggles while cell data remains in the worksheet. For imported XLSX files, OpenChestnut also carries a budget-checked, hash-bound source-package snapshot and updates modeled workbook/sheet/cell/table fields in place. Recognized table edits retain the original TableDefinitionPart path and worksheet relationship; complex table/query/filter/extension profiles remain byte-exact and reject semantic replacement. Pivots, drawings, comments, other unmodeled objects, and arbitrary legal OPC targets survive the second export behind the same opaque-part boundary. Cached-value and number-format-only edits retain the original formula XML exactly. Writing a new formula into one shared member deliberately detaches the complete group to ordinary formulas instead of leaving a broken native group. New advanced workbooks still use the JavaScript codec until their semantics enter the public schema. Consumption uses only bundled runtime assets; a local .NET SDK is not required:
