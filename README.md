...
`npm run eval:agents -- validate` checks the repository-only Agent PromptBench. Its isolated trials compare the candidate and reference Skills against the same packed npm candidate; two PDF pilots have independent semantic/visual/security/trace graders, while ungraded cases remain explicitly partial. The benchmark and its locked corpus are intentionally excluded from the consumer tarball. See [Agent black-box evaluations](docs/agent-evals.md).
