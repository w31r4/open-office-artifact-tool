...
Worksheet visibility, active selection, and grouped tab selection are shared by the JavaScript and OpenChestnut XLSX paths. Use `setActiveWorksheet(...)` for the one visible sheet opened by default; it collapses the selected group to that sheet. Use `setSelectedWorksheets([...])` and `getSelectedWorksheets()` for a visible multi-tab group, which must include the active sheet. Inspect/layout expose both states, verification/export reject hidden active/selected tabs, and source-free output coordinates zero-based `workbookView@activeTab` with worksheet `sheetView@tabSelected` for `workbookViewId=0`. Imported OpenChestnut workbooks hash-bind the workbook view plus every participating worksheet part/view and edit only those two owned attributes, preserving window geometry, panes, zoom, first-sheet scroll position, extensions, and residual XML. Missing or incomplete worksheet-view graphs remain readable but selection edits fail closed; multi-window graphs remain opaque.
