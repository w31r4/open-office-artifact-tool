...
The current XLSX slice roundtrips `range.format.numberFormat` and native shared/legacy-array formula topology through additive public wire fields and the bundled C# Open XML SDK codec. It authors built-in or custom formats from scratch and, for imported workbooks, clones the cell's original XF before changing only the number-format reference so unrelated font, fill, border, alignment, protection, and unknown style content remain intact. `XlsxFormulaCodec` validates complete shared groups, expands follower formulas using bounded A1 translation, emits master/follower `t/si/ref` records, and keeps formula XML untouched when only cached values or formats change. Other direct style authoring, data-table formulas, and dynamic-array extension semantics remain fail-closed on this path; the JavaScript XLSX codec is still the explicit fallback for broader style/theme/formula features.
