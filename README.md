...
The long-term Office I/O architecture is documented in [docs/reference-runtime-architecture.md](docs/reference-runtime-architecture.md). The agent-facing model remains JavaScript, while high-fidelity DOCX/PPTX/XLSX package I/O is planned as a source-built, bundled C# Open XML SDK codec running in .NET WebAssembly. The existing JavaScript codecs remain the current implementation and migration fallback; the Windows Microsoft Office bridge is optional render/compatibility QA, not the core file codec.
