...
Worksheet visibility is shared by the JavaScript and OpenChestnut XLSX paths. Create or update sheets with `visible`, `hidden`, or `veryHidden`; inspect/layout expose the normalized state and verification/export require at least one visible worksheet. Source-free output activates the first visible sheet. Imported OpenChestnut workbooks bind each `<sheet>` by ordinal, relationship ID, native sheet ID, and hashes, preserve the existing workbook-view graph, and reject hiding its active tab instead of silently retargeting it. Unknown native state profiles remain opaque and read-only.
