...
OpenChestnut also has separate bounded worksheet-chart and primary-axis codecs. `sheet.charts.add("bar"|"line"|"pie", ...)` authors a relationship-backed native ChartPart with title, legend, shared categories, ordered numeric caches, and optional category/value formulas. Bar and line charts expose `xAxis` text-category title/number format/tick-label interval plus `yAxis` linear value title/number format/min/max/major unit; `yAxis.tickLabelInterval` remains a compatibility alias for `majorUnit`. `WorksheetArtifact.charts` additive field 14 carries those semantics plus one native-EMU anchor and a hash-bound DrawingPart/ChartPart source locator, while additive chart fields 12/13 carry independently versioned axis messages. A recognized import may edit name, title, legend, formulas, categories, cached values, and those bounded axis fields while anchor, chart type, series count, point count, literal/reference topology, and primary-axis identity stay fixed. Unknown ChartSpace XML survives those edits. Formula-referenced series names and logarithmic/date/secondary/multi-axis profiles are read-only; external data, combo/multi-plot charts, richer styling/data labels, chart add/remove, and anchor/topology changes remain opaque or fail closed. `XlsxDrawingCodec` and `XlsxChartCodec` share one pre-mutation DrawingPart hash, so picture and chart edits can safely occur in the same source-preserving export.
