...
The experimental OpenChestnut DOCX path additionally treats bounded whole-paragraph hyperlinks, simple fields, fixed-topology table cell text, and direct or explicit paragraph-style-inherited numbered single-run paragraph text as source-bound editable content. Its C# Open XML SDK codec can change hyperlink text, tooltip, history, and external/internal target mode while changing only relationships owned by that hyperlink. It can edit one-run `w:fldSimple` instructions/results for a bounded non-fetching catalog such as `PAGE`, `NUMPAGES`, dates, document properties, and counts. `DocxTableCodec` edits only cells containing exactly one paragraph, one run, and one text node while retaining table/row/cell/paragraph/run properties behind a residual hash; merged, nested, multi-paragraph, and multi-run tables remain read-only. `DocxNumberedParagraphCodec` resolves the paragraph assignment while the dedicated `DocxNumberingResolver` follows the source `w:lvl/w:pStyle`, `w:styleLink`, and `w:numStyleLink` graph through cached read-only styles and numbering XML. In accordance with WordprocessingML, an `ilvl` inside a paragraph style is ignored; the associated numbering level comes from the level's `w:pStyle`, while direct paragraph `ilvl` remains authoritative. The wire/JS list model exposes format, start, level text, and numbering-style identity, but only the single text node is editable; numbering identity, level, style chain, links, overrides, and shared definitions remain read-only. Missing, duplicate, cyclic, over-deep, ambiguous, or otherwise unresolved definition graphs remain source-preserved but not editable. Unsupported structures fail closed instead of being flattened or executed.
