...
The current XLSX slice roundtrips `range.format.numberFormat` through an additive public wire field and the bundled C# Open XML SDK codec. It authors built-in or custom formats from scratch and, for imported workbooks, clones the cell's original XF before changing only the number-format reference so unrelated font, fill, border, alignment, protection, and unknown style content remain intact. Other direct style authoring remains fail-closed on this path; the JavaScript XLSX codec is still the explicit fallback for broader style/theme features.
