...
The current XLSX slice roundtrips complete static `range.format` profiles and native shared/legacy-array formula topology through additive public wire fields and the bundled C# Open XML SDK codec. Static profiles cover font, pattern fill, all nine border edges, alignment, protection, and built-in/custom number formats with RGB, theme, indexed, or automatic colors plus tint. Source-free export creates a valid minimal stylesheet; source-bound edits clone the current XF and referenced resources before changing owned fields, leaving unmodeled style content and formula XML intact. `XlsxFormulaCodec` validates complete shared groups, expands follower formulas using bounded A1 translation, and emits master/follower `t/si/ref` records. Custom workbook themes, gradient fills, named/cell style inheritance, data-table formulas, and dynamic-array extension semantics remain fail-closed on this path; the JavaScript XLSX codec is still the explicit fallback for broader features.
