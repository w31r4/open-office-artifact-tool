...
OpenChestnut also has a separate bounded worksheet-chart codec. `sheet.charts.add("bar"|"line"|"pie", ...)` authors a relationship-backed native ChartPart with title, legend, shared categories, ordered numeric caches, and optional category/value formulas. `WorksheetArtifact.charts` additive field 14 carries those semantics plus one native-EMU anchor and a hash-bound DrawingPart/ChartPart source locator. A recognized import may edit name, title, legend, formulas, categories, and cached values while anchor, chart type, series count, point count, and literal/reference topology stay fixed. Unknown ChartSpace XML survives those edits. Formula-referenced series names are read-only; external data, combo/multi-plot charts, richer axes/styles/labels, chart add/remove, and anchor/topology changes remain opaque or fail closed. `XlsxDrawingCodec` and `XlsxChartCodec` share one pre-mutation DrawingPart hash, so picture and chart edits can safely occur in the same source-preserving export.
