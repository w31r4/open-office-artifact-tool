...
PPTX comments support two native contracts. Legacy export (the compatibility default) writes `ppt/commentAuthors.xml`, per-author indexes, initials, display colors, and `lastIdx`. `Presentation.create({ commentFormat: "modern" })` writes Office 2021 `p188` Comment/Author parts with GUID author/comment identities, nested replies, dates, and active/resolved/closed status. Relationship-driven import follows arbitrary valid targets and alternate namespace prefixes, then preserves modern identities and author metadata on second export without private metadata. Package inspection and atomic patch validation reject invalid roots/content types/sources, orphan or duplicate relationships, invalid/duplicate identities, missing authors, invalid dates/status, and missing anchors. Modern shape/text monikers remain a fidelity target; export currently uses the standards-defined `unknownAnchor`. `presentation.masters` supports multiple editable Slide Masters while `presentation.master` remains the first-master compatibility alias. Layouts bind to a stable `masterId`; export writes every presentation master ID, per-master layout list, master↔layout and slide→layout relationship, plus a Theme relationship from each master. Metadata-free import preserves the graph, unused layouts, native IDs, and standard master-owned Theme targets through a second export.
