...
export async function main(argv = process.argv.slice(2)) {
  ...
  const result = await verifyPdfFile(args.input, { outputDir: args["output-dir"], nativeRender: args["native-render"], pdfjs: args.pdfjs, baselineDir: args["baseline-dir"], writeBaseline: args["write-baseline"] === "true", pixelThreshold: args["pixel-threshold"] ? Number(args["pixel-threshold"]) : undefined, maxChars: args["max-chars"] ? Number(args["max-chars"]) : undefined });
