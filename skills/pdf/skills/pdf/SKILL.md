...
`duplicate_page` is the narrow direct-original route for copying one ordinary
page inside the same PDF. It requires the exact source SHA-256 and inspected
`mupdfPage` bbox/rotation snapshot. `insertAt` is a 1-based output position and
defaults to immediately after the source page. The operation must be the only
operation in a full rewrite, supports right-angle page rotation, and accepts
only a source page with no annotations, links, widgets/form fields, structure
tree, page actions, associated files, article beads, transitions, or template
steps. It does not synthesize outlines, named destinations, or other navigation
for the new page, and the projected output must remain inside the configured
page/object budgets. Re-inspect the output, then Poppler-render the source page,
retained originals, and duplicate to prove exact page mapping and pixel identity.
