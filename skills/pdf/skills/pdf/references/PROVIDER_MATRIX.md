...
| pypdf | Read/inspect, path-safe read-only attachment quarantine, merge/split/stamp, basic AcroForm and annotation operations | Open original PDF directly | `read-only`, `rewrite`, or explicit `incremental` | Attachment extraction preserves source bytes and never executes payloads. Inspect signatures/DocMDP before mutation; incremental preserves prior bytes but does not prove a permitted signed-document change. |
