# PDF mutation audit schema

Every imported-PDF mutation emits one canonical `open-office-artifact-tool.pdf-audit.v1` JSON record. Do not invent aliases such as `outputs.pdf`, `actual_provider`, `provider_version`, `save_strategy`, or `silent_fallback`; downstream Agent and evaluator code reads the stable camelCase fields below.
