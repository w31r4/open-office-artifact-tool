...
- Public `document.addSection({ columns: { count: 2, spacing: 720, separator: true } })` authors canonical equal-width `w:cols`. Count is 1–45 and spacing must leave positive text width; custom-width `w:col` children and irregular column graphs remain source-owned/read-only → `tasks/sections_layout.md`, `artifact_tool/API_QUICK_START.md`
