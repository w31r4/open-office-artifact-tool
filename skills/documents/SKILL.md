...
- The checked-in `business-brief` fixture deliberately crosses the experimental `open-chestnut` DOCX roundtrip. It edits one hyperlink, one bounded field, one fixed-topology table cell, and one explicit paragraph-style-inherited numbered single-run list item's text while preserving the separate bookmark/comment-bearing table and all other advanced WordprocessingML from a hash-bound source package. Numbering identity/level/style chain/definitions and merged, nested, multi-paragraph, multi-run, resized, or restyled source tables remain read-only and must fail closed.
