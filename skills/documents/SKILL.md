...
- The checked-in `business-brief` fixture deliberately crosses the experimental `open-chestnut` DOCX roundtrip. The adapter preserves unchanged advanced WordprocessingML from a hash-bound source package and rejects unsupported edits; normal advanced authoring continues through `DocumentFile` until the C# semantic slice expands.
