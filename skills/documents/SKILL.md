...
- The checked-in `business-brief` fixture deliberately crosses the experimental `open-chestnut` DOCX roundtrip. It edits one hyperlink, one bounded field, and one fixed-topology table cell while preserving the separate bookmark/comment-bearing table and all other advanced WordprocessingML from a hash-bound source package. Merged, nested, multi-paragraph, multi-run, resized, or restyled source tables remain read-only and must fail closed.
