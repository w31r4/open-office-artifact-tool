...
- Use `importPptxWithOpenXmlWasm` / `exportPptxWithOpenXmlWasm` when a workflow explicitly requests the bundled C# codec. Its current editable boundary is simple top-level rectangle/ellipse shapes. Imported pictures, charts, groups, connectors, content parts, notes, masters, layouts, themes, and recursive part graphs are source-bound and read-only; preserve them unchanged and treat `unsupported_presentation_edit` as a hard stop, never as permission for a lossy fallback.
...
- The checked-in `openxml-wasm-preservation.json` fixture crosses JavaScript authoring → bundled C# Open XML SDK import/edit/export → JavaScript package/semantic/render QA. It must retain its chart, image, notes, master, theme, parts, and relationships while changing only the source-bound simple title shape.
