...
- Master/layout evidence must restore native `p:bg` solid or scheme references and merge placeholders by `type` plus unsigned `idx` (native default `0` is valid); slide backgrounds override layout backgrounds, which override the linked master. `background` is direct state, while `effectiveBackground()` reports the inherited result. `clearBackground()` may remove only a recognized direct Master/Layout `p:bg`; complex backgrounds remain exact-preserved and fail closed. The OpenChestnut path may edit an imported placeholder's owner-local text/body/list/link semantics with fixed owner, identity, shape-tree, paragraph, and inline topology. An already-present direct `a:xfrm` with exactly one `a:off` and `a:ext` may also move/resize through `placeholder.position` and edit `placeholder.transform` with optional -360..360 `rotationDegrees` plus presence-aware `flipHorizontal`/`flipVertical`; assigning `undefined` clears only these attributes. Absence remains inherited, while complex/out-of-range transforms and frame add/remove reject edits. Fill, shape style, and inherited/effective values remain source-bound. Inspect and render must report the same effective background and direct transform.
