...
- The checked-in `modern-comments.json` fixture emits Office 2021 `p188` Comment and Author parts, preserves GUID identities, native reply nesting, dates, authors, and resolved state across metadata-free import/export, and passes semantic plus LibreOffice/Poppler gates. Supported drawing targets use typed monikers tied to persistent `a16:creationId` identities; text-range and group-shape monikers remain fidelity targets, while unsupported targets use the standards-defined `unknownAnchor`.
