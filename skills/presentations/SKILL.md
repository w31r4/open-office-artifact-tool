...
- Use `importPptxWithOpenXmlWasm` / `exportPptxWithOpenXmlWasm` when a workflow explicitly requests the bundled C# codec. Its current editable boundary is top-level rectangle/ellipse shapes and basic paragraph/run text: level, left/center/right/justify alignment, character/auto-number/explicit-no-bullet markers, direct marker font, solid RGB marker color, fixed/percentage/follow-text marker size, bold/italic, font size/family, and solid RGB text color. Imported text may be edited only without changing paragraph/run topology. Picture bullets, non-RGB/transformed marker colors, links, fields, body/list styles, and other native properties stay source-bound; replacing an unknown marker or marker-style choice is unsupported. Pictures, charts, groups, connectors, content parts, notes, masters, layouts, themes, and recursive part graphs are read-only; preserve them unchanged and treat `unsupported_presentation_edit` or topology errors as hard stops, never as permission for a lossy fallback.
...
- The checked-in `openxml-wasm-preservation.json` fixture crosses JavaScript authoring → bundled C# Open XML SDK import/list-style-text edit/export → JavaScript package/semantic/render QA. It changes character, auto-number, and explicit no-bullet markers plus explicit/follow-text font, RGB color, and fixed/percentage size while retaining its chart, image, notes, master, theme, parts, and relationships.
