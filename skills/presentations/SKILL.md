...
- The checked-in `modern-comments.json` fixture emits Office 2021 `p188` Comment and Author parts, anchors its review to the resolved `shapeId/text` facade with a native `oac:txMkLst` character range, preserves GUID identities, native reply nesting, dates, authors, range offsets, and resolved state across metadata-free import/export, and passes semantic plus LibreOffice/Poppler gates. Supported drawing and shape-text targets use typed monikers tied to persistent `a16:creationId` identities; group-shape monikers remain a fidelity target, while unsupported targets use the standards-defined `unknownAnchor`.
