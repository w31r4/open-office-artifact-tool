...
- That fixture also injects an Office 2021-valid OLE package+preview, four-part SmartArt graph, and nested `contentPart`→custom XML→properties graph. Before editing it requires OpenChestnut `nativeObject` facades with `oleObject` 2 roots/2 parts, `diagram` 4 roots/4 parts, and `contentPart` 1 root/2 parts; every ID must resolve, inspect must expose bounded graph summaries, and the final PPTX must remain package-valid and renderable.
