...
- Use `importPptxWithOpenXmlWasm` / `exportPptxWithOpenXmlWasm` when a workflow explicitly requests the bundled C# codec. Its current editable boundary is top-level rectangle/ellipse shapes and bounded paragraphs: ordered text, `{ field: { id?, type, text } }`, and `{ break: true }` inlines; strictly increasing left/center/right/decimal `tabStops`; level and left/center/right/justify alignment; character/auto-number/explicit-no-bullet markers; direct marker font, solid RGB marker color, fixed/percentage/follow-text marker size; bold/italic, font size/family, solid RGB text color; and click links for absolute external URIs, internal slide IDs, or `nextSlide`/`previousSlide`/`firstSlide`/`lastSlide`/`endShow`. Imported text may be edited only without changing paragraph count, inline count, or text/field/break kinds; an empty imported `tabStops` list explicitly deletes the native list. Picture bullets, non-RGB/transformed marker colors, custom-show/mouse-over links, body/list styles, and other native properties stay source-bound; unknown field paragraph and break run properties are preserved. Replacing an unknown marker, marker-style choice, click action, or inline kind is unsupported. Pictures, charts, groups, connectors, content parts, notes, masters, layouts, themes, and recursive part graphs are read-only; preserve them unchanged and treat `unsupported_presentation_edit` or topology errors as hard stops, never as permission for a lossy fallback.
...
- The checked-in `openxml-wasm-preservation.json` fixture crosses JavaScript authoring → bundled C# Open XML SDK import/list-style-text/link/field/break/tab-stop edit/export → JavaScript package/semantic/render QA. It changes character, auto-number, and explicit no-bullet markers; explicit/follow-text font, RGB color, and fixed/percentage size; an external URI plus relative action while explicitly deleting another modeled link; field cached text; line-break styling; and tab-stop position/alignment. It retains its chart, image, notes, master, theme, and every unmodeled part/relationship.
