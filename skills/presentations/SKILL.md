...
- Master/layout evidence must restore native `p:bg` solid or scheme references and merge placeholders by `type` plus unsigned `idx` (native default `0` is valid); slide backgrounds override layout backgrounds, which override the linked master. The OpenXML-WASM path may edit only an imported placeholder's owner-local text/body/list/link semantics with fixed owner, identity, shape-tree, paragraph, and inline topology. Geometry, fill, shape style, and inherited/effective values remain source-bound. Inspect and render must report the same effective background.
