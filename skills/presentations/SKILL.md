...
3. Inspect the relevant slides, text ranges, tables, charts, images, notes, and comments before editing.
   Imported unsupported native objects appear in `presentation.inspect({ kind: "nativeObject" })` and resolve by ID. Recognized top-level OLE, SmartArt/diagram, and `contentPart` group facades report `editable:true` with `editableFields:["name","position"]`; use only `setName(...)` and `setPosition(...)`. Generic graphic frames and every other unsupported profile report `editable:false`. In both cases, preserve and never replace raw XML, relationship metadata, part bytes, or graph topology.
...
- Master/layout evidence must restore native `p:bg` solid or scheme references and merge placeholders by `type` plus unsigned `idx` (native default `0` is valid); slide backgrounds override layout backgrounds, which override the linked master. The OpenChestnut path may edit only an imported placeholder's owner-local text/body/list/link semantics with fixed owner, identity, shape-tree, paragraph, and inline topology. Geometry, fill, shape style, and inherited/effective values remain source-bound. Inspect and render must report the same effective background.
