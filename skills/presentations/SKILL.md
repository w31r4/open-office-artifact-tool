...
- The checked-in `modern-comments.json` fixture emits Office 2021 `p188` Comment and Author parts, creates a real native `p:grpSp`, anchors one review to `shapeId/text` through `oac:txMkLst` and another to the group through `grpSpMk`, preserves GUID identities, native reply nesting, dates, authors, range offsets, group transforms, and resolved state across metadata-free import/export, and passes semantic plus LibreOffice/Poppler gates. Nested group children use complete ancestor moniker paths; grouped tables/charts/images remain fidelity work, while unsupported targets use `unknownAnchor`.
