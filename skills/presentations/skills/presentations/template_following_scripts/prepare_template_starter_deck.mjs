...
async function main() {
  ...
  // `slide.duplicate()` now has a deliberately tiny shape/inline-table/image/recursive-group/closed-notes
  // leaf profile, but this starter workflow needs arbitrary graph duplication,
  // immediate inherited-object edits, and broad source deletion. Keep the
  // future implementation below as a design skeleton without letting a newly
  // exposed narrow primitive accidentally masquerade as template fidelity.
  ...
  if (!broadTemplateGraphCloneAvailable) {
    throw new Error(
      ...
      "The current OpenChestnut codec has only an isolated layout-only delete profile and an unchanged shape/inline-table/image/recursive-group/closed-notes clone that requires export/reimport; it will not reconstruct or share a broad clone graph.",
    );
  }
