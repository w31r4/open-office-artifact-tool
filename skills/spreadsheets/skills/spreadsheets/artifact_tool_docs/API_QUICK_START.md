...
- `PMT`, `IPMT`, `PPMT`, `RATE`, `SLN`, `DB`, `DDB`, `NPV`, `XNPV`, `IRR`, and `XIRR` calculate in the JavaScript model as well as being written to XLSX. Put rates, loan terms, cost/salvage/life assumptions, cash-flow vectors, and dates in visible input cells; do not hide them as magic numbers in a long formula.
