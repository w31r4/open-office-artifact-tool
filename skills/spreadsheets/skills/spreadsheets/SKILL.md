...
- `examples/openchestnut-loan-amortization-workflow.mjs`: Run or adapt this three-sheet `Inputs` / `Amortization` / `Checks` example for a visible loan schedule using PMT/IPMT/PPMT, payment-timing-aware first-period checks, finance color conventions, SVG review, and canonical OpenChestnut roundtrip.
