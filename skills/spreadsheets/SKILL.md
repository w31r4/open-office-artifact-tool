...
XLSX import follows worksheet and drawing relationships instead of assuming conventional filenames. Metadata-free embedded images and basic bar/line/pie charts are restored from native SpreadsheetDrawingML, including one-cell, two-cell, and absolute anchors, chart caches, range formulas, bounded direct RGB series lines, direct line-marker symbol/size/RGB fill/bounded outline, and bounded primary text/value-axis titles, number formats, intervals, and linear bounds. Always inspect and verify imported drawings; logarithmic/date/secondary axes, theme/compound/custom-dash lines, theme/transformed/gradient/picture marker graphs, data labels, and grouped shapes remain fidelity work.
...
The source-built Open XML SDK WebAssembly codec is available for the bounded workbook features: primitive/cached-formula cells, native shared-formula groups, legacy array-formula anchors, the complete 12-slot workbook theme, complete static `range.format` profiles, ordinary range defined names, rich worksheet tables/filters/sorts, source-bound QueryTable and database-connection root metadata, embedded PNG/JPEG pictures with all three anchors plus crop/effects/transform, and native single-plot bar/line/pie worksheet charts with title, legend, shared categories, numeric caches, optional category/value formulas, plot-level value/category data labels, per-series solid RGB fills, direct RGB line color/dash/width, direct line-marker symbol/size/RGB fill/bounded outline, chart-level line smoothing, bounded title/tick-label font sizes, and primary text/value axes. The checked-in fixture performs OpenChestnut authoring, import, same-format source-bound picture-byte replacement, fixed-topology chart/data-label/fill/line/marker/smooth/font-size/axis editing, and a second export, so its custom theme, defined names, static styles, shared/array topology, native sorting/color selectors, three picture anchor kinds, formula-backed ChartPart, edited data labels, series fill/line/marker/smooth/font sizes, primary axes, shared DrawingPart, and table relationships cross the public wire twice before QA:
...
For this OpenChestnut picture slice, use `sheet.images.add({ name, alt, dataUrl, anchor, crop?, effects?, transform? })` with a base64 `image/png` or `image/jpeg` data URL. A one-cell anchor is `{ from: { row, col, rowOffsetPx?, colOffsetPx? }, extent: { widthPx, heightPx } }`; a two-cell anchor is `{ type: "twoCell", from: {...}, to: {...}, editAs?: "twoCell"|"oneCell"|"absolute" }`; an absolute anchor is `{ type: "absolute", position: { leftPx, topPx }, extent: { widthPx, heightPx } }`. Marker rows/columns are zero-based, and `to` must be strictly after `from` on both axes. Absolute positions are signed and bounded; their extents must be positive. Omitted `editAs` retains native-default attribute absence. `crop` is `{ leftPercent, topPercent, rightPercent, bottomPercent }`; each signed offset is -100 through 100, positive values inset, negative values outset, and opposing sums must remain below 100. `effects` is `{ grayscale?, brightnessPercent?, contrastPercent?, opacityPercent? }`; brightness/contrast use -100 through 100 and opacity uses 0 through 100. `transform` is `{ rotationDegrees?, flipHorizontal?, flipVertical? }`; rotation uses -360 through 360 degrees at 1/60000-degree native precision, and explicit false flips remain distinct from omission. The SVG preview approximates these native DrawingML visuals. The adapter content-addresses the bytes; OpenChestnut owns the worksheet Drawing relationship, DrawingPart, picture relationship, and ImagePart. A recognized imported picture may edit `name`, `alt`, same-kind anchor geometry/`editAs`, `crop`, `effects`, `transform`, and `dataUrl` in place. Byte replacement retains the ImagePart path and is accepted only when the MIME type stays PNG or JPEG exactly as imported and the part has one blip reference; shared-part and cross-format replacements fail closed. Imported image add/remove, anchor-kind conversion, external URI/prompt sources, advanced or duplicate picture-effect graphs, shapes, and groups remain fail-closed or opaque. Out-of-profile native crop/effect/transform graphs are retained but cannot be replaced through this bounded model. Relationship IDs are package locators, never durable image IDs.

For the OpenChestnut chart slice, use `sheet.charts.add("bar"|"line"|"pie", { name, title, titleTextStyle?, lineOptions?, dataLabels?, hasLegend, categories, series, xAxis?, yAxis?, position })`. `lineOptions` is line-chart-only and accepts `{ grouping?, smooth?, varyColors? }`. Grouping is `standard`, `stacked`, or `percentStacked`; omission authors native `standard`, while import exposes the native value. Explicit smooth false is preserved as `c:smooth val="0"`, while omission removes that optional node. Set `varyColors: true` to author direct `c:varyColors val="1"`; false or omission removes that optional node. `dataLabels` accepts a boolean shorthand for `showValue`, or `{ showValue?, showCategoryName?, showSeriesName?, position? }`; `position` is `bestFit`, `bottom`, `center`, `insideBase`, `insideEnd`, `left`, `outsideEnd`, `right`, or `top`. At least one field must be present. Omission removes the plot-level label block, while an explicit all-false object retains a native block with the value/category flags false; `showSeriesName: false` and explicit `bestFit` remain distinct from omission. Deterministic SVG preview renders every line series and applies the selected stacking mode, value/category/series-name label text, and bounded label placement. `titleTextStyle` and each primary axis `textStyle` accept only `{ fontSize }` from 1 through 4,000 points; axis `textStyle` controls tick labels, not the axis title. Each series accepts `{ name, values, categoryFormula?, formula?, fill?, line?, marker? }`; formulas omit the leading equals sign, `fill` is an explicit `#RRGGBB` solid color, and cached values/categories remain available for deterministic inspect/render. `line` is `{ fill?, style?, width? }`: fill is `#RRGGBB`, style is `solid`, `dashed`, `dotted`, `dash-dot`, or `dash-dot-dot`, and width is 0 through 1,584 points. `stroke: { color?, style?, weight? }` is a compatibility alias for the series line; do not provide conflicting `line` and `stroke` values. On line charts, `marker` is `{ symbol?, size?, fill?, line? }`: symbol is `none`, `dot`, `circle`, `square`, `diamond`, `triangle`, `x`, `star`, `plus`, or `dash`, size is an integer from 2 through 72, fill is a direct `#RRGGBB` color, and line reuses the same bounded fill/style/width outline profile. Bar/line `xAxis` accepts `{ axisType: "textAxis", title: { text }, textStyle?, numberFormatCode?, tickLabelInterval? }`; `yAxis` accepts `{ axisType: "valueAxis", title: { text }, textStyle?, numberFormatCode?, min?, max?, majorUnit? }`, with `tickLabelInterval` accepted as a compatibility alias for `majorUnit`. Source-free charts use a bounded absolute anchor derived from `position`. A recognized import may add, remove, or change an owned series RGB fill, direct line, complete bounded direct marker, chart-level grouping/smooth/vary-colors nodes, plot-level value/category/series-name/position labels, and bounded title/tick-label font sizes, and edit name, title, legend, formulas, categories, cached values, and these primary-axis fields while anchor, chart type, series count, point count, literal/reference topology, and axis identity remain unchanged. Native theme/gradient/no-fill, rich title runs, complex/conflicting axis `txPr`, or otherwise unrecognized fill/text graphs become read-only and exact-preserved. Theme/transformed line colors, compound lines, caps, joins, arrows, custom dash patterns, or conflicting line graphs likewise remain read-only and exact-preserved. Theme/transformed/gradient/picture marker graphs, marker extensions, duplicates, unknown marker children, non-canonical grouping/smooth/vary-colors nodes, series/point label overrides, label number/text formatting or styles, leader lines, visible legend/percent/bubble label flags, and other complex label graphs also keep the whole chart read-only and exact-preserved. Pie axes, axis-title styles, inverted/non-finite ranges, logarithmic/date/secondary/multi-axis profiles, imported chart add/remove or movement, combo/multi-plot charts, external data, and formula-referenced series names remain fail-closed or opaque. Pictures and charts safely share one worksheet DrawingPart; relationship IDs and part paths remain source locators rather than persistent model identity.
