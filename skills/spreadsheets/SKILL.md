...
The source-built Open XML SDK WebAssembly codec is available for the bounded workbook features: primitive/cached-formula cells, native shared-formula groups, legacy array-formula anchors, the complete 12-slot workbook theme, complete static `range.format` profiles (font, pattern fill, nine border edges, alignment, protection, and built-in/custom number formats), worksheet tables with calculated-column/totals-row, exact/grouped-date/custom/dynamic/Top10/icon/color AutoFilter, value/icon/color sort metadata, source-bound QueryTable root plus refresh/field semantics, date-system selection, sheets, merges, dimensions, gridlines, and frozen panes. The checked-in fixture performs OpenChestnut authoring, import, and source-preserving second export, so its custom theme, static styles, shared/array topology, custom/percentage formats, native color selectors, and `tableParts`/TableDefinitionPart relationship cross the public wire twice before QA:
...
An existing QueryTable is a source-bound graph, not a source-free table option. The dedicated fixture first creates an ordinary workbook, injects a standards-based synthetic ConnectionsPart/QueryTablePart as external-source input, then uses only OpenChestnut to import, disable refresh, edit root and recognized refresh/field policy in place, export, and roundtrip before package/model/Playwright QA:
...
Imported recognized tables expose `table.queryTable` with `name`, `connectionId`, presence-aware root policy, optional `growShrinkType`/`autoFormatId`, and `refresh`. The refresh object exposes presence-aware `preserveSortFilterLayout`, `fieldIdWrapped`, `headersInLastRefresh`, `minimumVersion`, `nextId`, unbound-column counters, and `fields`. A field exposes its immutable positive `id` and optional immutable `tableColumnId` plus editable `name`, `dataBound`, `rowNumbers`, `fillFormulas`, and `clipped`; field count, order, IDs, and table bindings cannot change. Formula-fill fields must set `dataBound: false`, while clipped fields must set `dataBound: true`. OpenChestnut requires the connection ID to exist in the unchanged source ConnectionsPart. It preserves connection definitions, deleted-field/sort history, relationships, field/refresh extensions, and unknown XML, and it never executes a refresh. An unrecognized refresh remains hidden while a separately recognized root can still be edited. Do not set `queryTable` on a newly authored table: both OpenChestnut and the JavaScript codec reject source-free fabrication, and the JavaScript codec also rejects export of an imported query graph it cannot preserve. The fixture deliberately sets `disableRefresh: true` and keeps native office rendering off so QA does not invoke an external-data consumer; Playwright still verifies the modeled render.
...
Bounded sorting uses `{ reference, caseSensitive, conditions: [{ reference, descending, kind?, iconSet?, iconId?, target?, color? }] }`; each ordered condition is a unique single column with the same row span as the contained sort range. Omit `kind` for a value sort, use `kind: "icon"` for an icon selector, or `kind: "color"` with `target` and `color` for a stable color selector. Imported recognized tables may edit these bounded semantic fields while retaining their part/relationship identity. Complex table attributes, unsupported QueryTable companion relationships/refresh profiles, QueryTable field topology, custom-list/locale-specific sorts, multi-property differential styles, and extensions remain exact-preserved and read-only, and imported table add/remove or QueryTable graph topology change fails closed. Direct WebAssembly authoring also remains fail-closed for QueryTable/external-connection creation, gradient fills, HSL/preset/scheme theme colors, theme font/format-scheme editing, named/cell style inheritance, defined names, pivots, drawings, comments, validations, conditional formatting, data-table formulas, and dynamic-array extension semantics. On imported workbooks a cached-value-only edit leaves `<f>` untouched, while a modeled theme edit preserves the imported font/format schemes and other residual XML, and a modeled static-style change clones the current XF and its referenced resources before changing only owned fields; unknown XF/font/fill/border/alignment/protection content and formula XML remain source-preserved. All other unsupported package content stays behind the bounded, hash-bound source package and opaque-graph checks. Use the default `javascript` codec to create advanced workbooks only when no source-bound QueryTable graph must survive; never discard or fabricate preservation state merely to make an export pass.
