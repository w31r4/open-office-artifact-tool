...
For the OpenChestnut chart slice, use `sheet.charts.add("bar"|"line"|"pie", { name, title, hasLegend, categories, series, position })`. Each series accepts `{ name, values, categoryFormula?, formula? }`; formulas omit the leading equals sign and cached values/categories remain available for deterministic inspect/render. Source-free charts use a bounded absolute anchor derived from `position`. A recognized import may edit name, title, legend, formulas, categories, and cached values only while anchor, chart type, series count, point count, and literal/reference topology remain unchanged. Imported add/remove or movement, combo/multi-plot charts, external data, formula-referenced series names, and richer axes/styles/labels remain read-only, opaque, or fail closed. Pictures and charts safely share one worksheet DrawingPart; relationship IDs and part paths remain source locators rather than persistent model identity.
