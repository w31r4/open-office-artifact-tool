...
The Pivot calculated-field whitelist currently contains 37 scalar functions. Its deterministic date subset also includes `EDATE`/`EOMONTH` month shifting, `DAYS(end, start)`, and `WEEKDAY` return types `1`, `2`, `3`, and `11` through `17`. Month offsets are truncated, short target months clamp to month end, and invalid serials or return types produce `#NUM!`. The `formula-summary` fixture exercises these functions under a 1904 workbook and verifies metadata-free second export.
