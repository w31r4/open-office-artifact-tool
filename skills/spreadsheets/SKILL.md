...
The Pivot calculated-field whitelist currently contains 41 scalar functions. Its deterministic date subset also includes `EDATE`/`EOMONTH` month shifting, `DAYS(end, start)`, and `WEEKDAY` return types `1`, `2`, `3`, and `11` through `17`. `TIME` builds a wrapped clock fraction from truncated `0..32767` hour/minute/second inputs, while `HOUR`/`MINUTE`/`SECOND` extract components from numeric serials or strict time text. Month offsets are truncated, short target months clamp to month end, and invalid serials, time inputs, or return types produce explicit Excel errors. The `formula-summary` fixture exercises these functions under a 1904 workbook and verifies metadata-free second export.
