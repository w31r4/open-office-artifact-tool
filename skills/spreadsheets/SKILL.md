...
An existing QueryTable is a source-bound graph, not a source-free table option. The dedicated fixture first creates an ordinary workbook, injects a standards-based synthetic ConnectionsPart/QueryTablePart as external-source input, then uses only OpenChestnut to import, disable refresh, edit the query root, export, and roundtrip before package/model/Playwright QA:
