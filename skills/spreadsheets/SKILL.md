...
The source-built Open XML SDK WebAssembly codec is available for the bounded workbook features: primitive/cached-formula cells, native shared-formula groups, legacy array-formula anchors, complete static `range.format` profiles (font, pattern fill, nine border edges, alignment, protection, and built-in/custom number formats), date-system selection, sheets, merges, dimensions, gridlines, and frozen panes. The checked-in fixture performs OpenChestnut authoring, import, and source-preserving second export, so its static styles, shared/array topology, and custom/percentage formats cross the public wire twice before QA:
