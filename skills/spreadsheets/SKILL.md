...
For the OpenChestnut chart slice, use `sheet.charts.add("bar"|"line"|"pie", { name, title, titleTextStyle?, hasLegend, categories, series, xAxis?, yAxis?, position })`. `titleTextStyle` and each primary axis `textStyle` accept only `{ fontSize }` from 1 through 4,000 points; axis `textStyle` controls tick labels, not the axis title. Each series accepts `{ name, values, categoryFormula?, formula?, fill? }`; formulas omit the leading equals sign, `fill` is an explicit `#RRGGBB` solid color, and cached values/categories remain available for deterministic inspect/render. Bar/line `xAxis` accepts `{ axisType: "textAxis", title: { text }, textStyle?, numberFormatCode?, tickLabelInterval? }`; `yAxis` accepts `{ axisType: "valueAxis", title: { text }, textStyle?, numberFormatCode?, min?, max?, majorUnit? }`, with `tickLabelInterval` accepted as a compatibility alias for `majorUnit`. Source-free charts use a bounded absolute anchor derived from `position`. A recognized import may add, remove, or change an owned series RGB fill and bounded title/tick-label font sizes, and edit name, title, legend, formulas, categories, cached values, and these primary-axis fields while anchor, chart type, series count, point count, literal/reference topology, and axis identity remain unchanged. Native theme/gradient/no-fill, rich title runs, complex/conflicting axis `txPr`, or otherwise unrecognized style graphs become read-only and exact-preserved. Pie axes, axis-title styles, inverted/non-finite ranges, logarithmic/date/secondary/multi-axis profiles, imported chart add/remove or movement, combo/multi-plot charts, external data, formula-referenced series names, line/marker styles, and data labels remain read-only, opaque, or fail closed. Pictures and charts safely share one worksheet DrawingPart; relationship IDs and part paths remain source locators rather than persistent model identity.
