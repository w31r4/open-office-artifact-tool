...
For the OpenChestnut chart slice, use `sheet.charts.add("bar"|"line"|"pie", { name, title, hasLegend, categories, series, xAxis?, yAxis?, position })`. Each series accepts `{ name, values, categoryFormula?, formula? }`; formulas omit the leading equals sign and cached values/categories remain available for deterministic inspect/render. Bar/line `xAxis` accepts `{ axisType: "textAxis", title: { text }, numberFormatCode?, tickLabelInterval? }`; `yAxis` accepts `{ axisType: "valueAxis", title: { text }, numberFormatCode?, min?, max?, majorUnit? }`, with `tickLabelInterval` accepted as a compatibility alias for `majorUnit`. Source-free charts use a bounded absolute anchor derived from `position`. A recognized import may edit name, title, legend, formulas, categories, cached values, and these primary-axis fields only while anchor, chart type, series count, point count, literal/reference topology, and axis identity remain unchanged. Pie axes, inverted/non-finite ranges, logarithmic/date/secondary/multi-axis profiles, imported add/remove or movement, combo/multi-plot charts, external data, formula-referenced series names, styles, and data labels remain read-only, opaque, or fail closed. Pictures and charts safely share one worksheet DrawingPart; relationship IDs and part paths remain source locators rather than persistent model identity.
