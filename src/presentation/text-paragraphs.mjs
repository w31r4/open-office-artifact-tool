...
export function inheritPresentationParagraphs(paragraphs = [], inheritedByLevel = {}) {
  return paragraphs.map((paragraph) => {
    ...
    if (paragraph.bulletCharacter != null || paragraph.autoNumber || paragraph.bulletNone) {
...
export function presentationParagraphsXml(paragraphs = [], defaultStyle = {}) {
  return paragraphs.map((paragraph) => {
    ...
    return `<a:p>${paragraphPropertiesXml(paragraph)}${runs}<a:endParaRPr lang="en-US"${endStyle.fontSize ? ` sz="${Math.round(endStyle.fontSize * HUNDREDTH_POINTS_PER_PIXEL)}"` : ""}/></a:p>`;
  }).join("");
}
