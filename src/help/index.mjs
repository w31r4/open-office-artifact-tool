...
const DOCUMENT_HELP_SCHEMAS = {
  "DocumentModel.create": helpSchema({
    ...
    blocks: { type: "object[]", description: "Ordered paragraph/list/table/link/field/citation/image/section/change block models. Canonical bibliography-backed citations and one-paragraph complex TOC placeholders cross OpenChestnut; other complex field graphs remain source-bound." },
  ...
  "document.addParagraph": helpSchema({
    ...
    runs: { type: "object[]", description: "Optional run spans whose style may include runStyleId plus direct/theme formatting. A run may carry contentControl { id, tag, alias, nativeId? } for the bounded inline plain-text SDT profile." },
  ...
  "exportDocxWithOpenChestnut": helpSchema({
    document: { type: "DocumentModel", required: true, description: "Document facade within the OpenChestnut paragraph/run/style, section, header/footer, image, list, hyperlink, whole-block bookmark, plain-text footnote/endnote, simple-field, comment, and fixed-table boundary. Advanced imported content remains source-bound; unsupported edits fail closed." },
    limits: { type: "object", description: "Optional maxInputBytes, maxUncompressedBytes, maxParts, maxCells, and maxCompressionRatio codec budgets." },
