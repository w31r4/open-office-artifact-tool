...
export const HELP_CATALOG = [
  ...
  { artifactKind: "presentation", kind: "api", name: "slide.duplicate", summary: "Clone one original imported PPTX slide only when its unchanged graph is shape-only and has exactly one layout relationship. Export creates a distinct SlidePart and presentation relationship while sharing that verified layout and retaining the origin; the clone must remain untouched until export and reimport, and every media/note/comment/chart/OLE/hyperlink/complex graph fails closed." },
...
const PRESENTATION_HELP_SCHEMAS = {
  ...
  "slide.duplicate": helpSchema({}, "slide", "Slide", "A new adjacent Slide. It is available only for an original imported PPTX source slide whose unchanged graph has shape-only elements and exactly one internal layout relationship. Export allocates a distinct SlidePart and presentation relationship, deliberately shares that verified layout, and preserves the required origin part. The clone must remain semantically unchanged until its first export and second import; source-free slides, already-cloned slides, same-transaction origin deletion, notes, comments, media, charts, OLE, hyperlinks, external/data relationships, and every broader graph clone fail closed."),
