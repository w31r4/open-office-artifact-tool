...
export function computePivotValues(matrix = [], config = {}) {
  ...
  const rows = matrix.slice(1).filter((row) => (config.filters || []).every((filter) => pivotItemVisible(config.filters, filter.field, row[headers.indexOf(filter.field)], config.dateSystem)));
