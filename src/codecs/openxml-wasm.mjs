...
import { presentationEnvelope, presentationFromEnvelope } from "./openxml-wasm-presentation.mjs";

export { OpenXmlWasmCodecError } from "./openxml-wasm-error.mjs";
...
export async function exportPptxWithOpenXmlWasm(presentation, options = {}) {
  const response = await invokeOpenXmlWasm({
    ...
    artifact: presentationEnvelope(presentation, OPEN_XML_WASM_PROTOCOL_VERSION),
    limits: codecLimits(options.limits),
    allowLossy: options.allowLossy === true,
  });
  return new FileBlob(response.file, {
