...
import { DocumentModel, FileBlob, Presentation, Workbook } from "../index.mjs";
import {
  ArtifactFamily,
  CodecOperation,
  CodecRequestSchema,
...
function simplePresentationText(shape) {
  const paragraphs = shape.text?.paragraphs || [];
  const simple = paragraphs.length <= 1 && paragraphs.every((paragraph) =>
    Number(paragraph.level || 0) === 0 &&
    Object.keys(paragraph.style || {}).length === 0 &&
    Object.keys(paragraph).every((key) => ["runs", "level", "style"].includes(key)) &&
    (paragraph.runs || []).length <= 1 &&
    (paragraph.runs || []).every((run) => Object.keys(run.style || {}).length === 0 && Object.keys(run).every((key) => ["text", "style"].includes(key))));
  if (!simple || Object.keys(shape.text?.style || {}).length || Object.keys(shape.text?.inheritedParagraphStyles || {}).length) {
    throw new OpenXmlWasmCodecError(`Presentation shape ${shape.id} uses rich text that is outside the PPTX WebAssembly vertical slice.`, [], { code: "unsupported_presentation_features" });
  }
  ...
  return {
    ...
    content: {
      ...
      value: {
        ...
        text: simplePresentationText(shape),
...
function presentationEnvelope(presentation) {
...
export async function exportPptxWithOpenXmlWasm(presentation, options = {}) {
  const response = await invokeOpenXmlWasm({
    protocolVersion: OPEN_XML_WASM_PROTOCOL_VERSION,
    operation: CodecOperation.EXPORT_PPTX,
    family: ArtifactFamily.PRESENTATION,
    artifact: presentationEnvelope(presentation),
    limits: codecLimits(options.limits),
    allowLossy: options.allowLossy === true,
  });
  return new FileBlob(response.file, {
    type: PPTX_MIME,
    metadata: { artifactKind: "presentation", codec: "openxml-wasm", diagnostics: response.diagnostics },
  });
}
...
function presentationFromEnvelope(envelope) {
  ...
  const presentation = Presentation.create({
    slideSize: {
      width: Number(source.slideWidthEmu) / EMU_PER_PIXEL,
      height: Number(source.slideHeightEmu) / EMU_PER_PIXEL,
    },
  });
...
export async function importPptxWithOpenXmlWasm(input, options = {}) {
  const response = await invokeOpenXmlWasm({
    protocolVersion: OPEN_XML_WASM_PROTOCOL_VERSION,
