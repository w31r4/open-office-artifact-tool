...
function wireImage(image, assets, source) {
  ...
  const row = coordinate(snapshot.row, "anchor.from.row", 1_048_576, image);
  const column = coordinate(snapshot.column, "anchor.from.col", 16_384, image);
  const rowOffsetPx = pixels(snapshot.rowOffsetPx, "anchor.from.rowOffsetPx", image);
  const columnOffsetPx = pixels(snapshot.columnOffsetPx, "anchor.from.colOffsetPx", image);
...
export function spreadsheetImageFromWire(sheet, source, assets) {
  ...
  if (!anchor) fail(source, "has no one-cell anchor.");
  ...
  const image = sheet.images.add({
    ...
    anchor: {
      from: {
        row: anchor.row,
        col: anchor.column,
        rowOffsetPx: numberFromWire(anchor.rowOffsetEmu, "row offset") / EMU_PER_PIXEL,
        colOffsetPx: numberFromWire(anchor.columnOffsetEmu, "column offset") / EMU_PER_PIXEL,
      },
      extent: {
